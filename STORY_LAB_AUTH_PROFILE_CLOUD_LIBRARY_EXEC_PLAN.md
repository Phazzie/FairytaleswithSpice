...
- The Clerk auth adapter has a real verifier wired into the production `configuredAuthPort` singleton (`@clerk/backend`'s `verifyToken`, gated on `CLERK_SECRET_KEY`), and the Angular app has a live sign-in flow: an unauthenticated `GET /account/auth-config` endpoint reports whether a deployment has a provider, publishable key, and secret key all configured together; `AuthService` lazy-loads `@clerk/clerk-js` only when it does, and `authInterceptor` attaches a freshly-fetched session token to authenticated account requests (as `Authorization: Bearer`) and, since the "Story Lab content-boundaries safety preference" worst-to-best PR (#329), to the direct genesis/continuation routes and the job routes too — as a dedicated `X-Story-Lab-Session` header there rather than `Authorization`, because those routes also run `enforceApiAccessControl`, which reads `Authorization: Bearer` as an `API_KEYS` candidate whenever a deployment configures one; `clerkAuthPort.ts`'s `readClerkSessionToken` reads either header. Neither of those routes gates on auth the way the account routes do (they call `getCurrentUser`, never `requireUser`, unless durable job storage is enabled), so this only lets a signed-in reader's stored content-boundaries preference reach generation — it does not require sign-in. A deployment that configures both `API_KEYS` and Clerk still cannot reach any paid route from the browser at all (`API_KEYS` predates Clerk auth and the frontend has never had a way to satisfy it) — tracked as [#330](https://github.com/Phazzie/FairytaleswithSpice/issues/330), not part of this contract's live-provider claim. Deploying this against a real Clerk tenant with `STORY_LAB_AUTH_PROVIDER=clerk`, `CLERK_PUBLISHABLE_KEY`, and `CLERK_SECRET_KEY` set has not been proven end to end against a live Clerk tenant in this environment (no credentials available here) — that live-provider proof is still open.
- Token verification passes `authorizedParties` (the configured origin allowlist plus this deployment's own `VERCEL_URL`/`VERCEL_BRANCH_URL`/`VERCEL_PROJECT_PRODUCTION_URL` — platform-injected, never a request header) so a correctly-signed token whose `azp` claim names an untrusted sibling origin is rejected rather than accepted on signature validity alone; if that allowlist parses to empty (an all-invalid `STORY_LAB_ALLOWED_ORIGINS` value with no platform URL available), the verifier fails closed instead of handing Clerk an unchecked allowlist, and `GET /account/auth-config` shares that same check so it never advertises `provider: 'clerk'` in the one case the verifier refuses to exist. `AuthService.signOut()` propagates a rejected Clerk sign-out instead of clearing local state anyway, and every cloud-library request (`refreshCloudLibrary`/save/load/delete, mutually exclusive via `isCloudLibraryBusy`) has its in-flight subscription cancelled *before* `signOutOfCloudAccount()` awaits Clerk's own sign-out call, not after, so a response authenticated under the old session cannot resurrect that account's data even if it arrives while sign-out is still in flight — and again on `App.ngOnDestroy()`, so navigating away mid-request has the same guarantee. The constructor effect driving all of this tracks a client-side-decoded `accountId`, not just signed-in/out, so a multi-session account swap with no intermediate sign-out is caught too; because Angular's effect scheduling is itself asynchronous, every cloud-library response callback additionally re-checks a live snapshot of `isSignedIn`/`accountId`/`sessionEpoch` (the last advancing synchronously the instant Clerk announces *any* session change, closing the gap before `isSignedIn`/`accountId` themselves finish updating) before applying anything, rather than relying on cancellation timing alone. `AuthService.refreshSessionToken()` similarly guards against two overlapping refreshes resolving out of order: a superseded call returns `null` whenever `sessionEpoch` has moved since it started — sign-out and an account switch both bump it — and only defers to the newer refresh's own settled result when the epoch is unchanged, meaning it was superseded by an ordinary same-identity refresh; never a stale in-between snapshot an interceptor could send as a wrong bearer token. `sessionEpoch` advancing synchronously still leaves a narrower gap of its own: a *new* save/load/delete that starts after the epoch bump but before that refresh settles would capture the already-advanced epoch and be found current, not stale, by that same comparison, while `sessionTokenState`/`accountId` (and the UI) still reflect the outgoing account — letting its interceptor fetch the incoming account's token for a request built against the outgoing account's data. `AuthService.identityTransitionPending` closes that: true synchronously the instant a session-change listener fires, false again only once its own refresh settles, and `App`'s four cloud-library methods refuse to start while it is true — the listener is registered with Clerk's own `skipInitialEmit: true` option, since the real client otherwise invokes it immediately on registration and that immediate call would itself get permanently stuck pending, before `AuthService`'s own explicit initial `refreshSessionToken()` call ever has a chance to supersede and clear it. `AuthService.isConfigured()` requires both that the deployment reports `provider: 'clerk'` *and* that the Clerk client itself finished loading (`clientReadyState`) — not the former alone — so a client-load failure (blocked script, network error) is distinguishable from a working deployment and `App.showCloudAccountSetupStatus()`'s retry path can actually surface it, proven by a client-load-failure-then-successful-retry test. All of this is proven with mocked/DI'd Clerk clients (a fake reproducing the real client's initial-emission behavior, among other documented contract points) — see the "Not live yet" section below for what still lacks live-Clerk-tenant proof. See `PR70_RECOVERY_CHANGELOG.md`'s 2026-09-03 entries for the finding-by-finding history, including a since-corrected round that briefly trusted a caller-supplied forwarded-host header for this same boundary.
...
- The "Story Lab Profile" worst-to-best PR (#342) added a `StoryLabProfilePanelComponent` editor for favorite creatures/tones, the default heat contract, content boundaries, and library sort order, reachable from the account panel's "Profile" action once `cloud_synced`. Saved favorite creature/tone and default heat contract are applied to a still-blank blueprint both at the moment of saving and once per signed-in account on a later visit (`App.syncStoryLabProfileDefaultsWithAuthState`) — never onto a blueprint that already has a story in progress, and never once the reader has touched `creature`/`tone`/`heatContract` directly (a permanent `blueprintTouchedByReader` latch, set by `updateBlueprint`/`updateHeatContract`/`toggleTheme`), so a slow background fetch can no longer clobber a manual pick made before it resolves. The panel guards its own requests and auto-closes on an actual signed-in account change (not an ordinary same-account token refresh), mirroring `CloudLibraryService`'s stale-response pattern; the sign-in-time fetch has the same `sessionEpoch` staleness guard, retrying under the current identity rather than applying a stale account's defaults (safe here since it's a GET run at most once per account, and reset on sign-out so the same account refetches on a later sign-in), and both the panel's `loadProfile()`/`save()` release their loading/saving lock unconditionally on a stale response rather than leaving the panel stuck. `librarySort` continues to be read only by `handleProjectsRoute`, as before; the panel just gives it (and the other three fields) a writer. Non-claim: the panel has no live-Clerk-tenant proof beyond what the rest of this file already tracks as open.
...
- The "Story Lab cloud storage" worst-to-best PR (#344) gave `createStoryLabCloudStorage()` an actual `STORY_LAB_CLOUD_STORAGE` env-driven mode switch — it previously always built Postgres-only stores with no way to opt into the non-durable in-memory adapter this file's own scaffold (line 24/29 above) had built. Default stays `postgres` (unchanged production behavior); `STORY_LAB_CLOUD_STORAGE=non_durable_memory` is a local/test convenience, not a supported production fallback — the account/job/genesis/continuation route handlers now share one process-local in-memory store instance in that mode (not a fresh one per call, and still not shared across separate Vercel function invocations), and `/api/health`'s `services.storyLabCloudStorage` reports that mode as `degraded` (not just `configured: true`), matching this file's own line above: the Angular client already treats `non_durable_memory` account storage as cloud-unavailable, so `/api/health` reporting it healthy would have been the same false-capability claim this routine exists to catch.
