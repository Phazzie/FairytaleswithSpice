...
- Token verification passes `authorizedParties` (the configured origin allowlist plus this deployment's own `VERCEL_URL`/`VERCEL_BRANCH_URL` — platform-injected, never a request header) so a correctly-signed token whose `azp` claim names an untrusted sibling origin is rejected rather than accepted on signature validity alone. `AuthService.signOut()` propagates a rejected Clerk sign-out instead of clearing local state anyway, and every cloud-library request (`refreshCloudLibrary`/save/load/delete, mutually exclusive via `isCloudLibraryBusy`) cancels its in-flight subscription on sign-out so a response authenticated under the old session cannot resurrect that account's data after the UI has cleared it. See `PR70_RECOVERY_CHANGELOG.md`'s 2026-09-03 entries for the finding-by-finding history, including a since-corrected round that briefly trusted a caller-supplied forwarded-host header for this same boundary.
