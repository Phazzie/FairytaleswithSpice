...
- [ ] `In review` (PR #293, not merged) — repair three blind spots in the deterministic story-quality scan the Phase 6 evals read. The scan is what this plan's "judged by evidence instead of vibes" rests on, so what it cannot see is what no eval can report:
  - `cliffhanger_quality`'s explicit-language lexicon was inverted. `cliff` could not match `cliffhanger` — `wordFormAlternationPattern` accepts an inflection, not a compound — so it only ever matched the rock face, while `continued` scored on any ordinary "he continued down the hall". The lexicon is now `['cliffhanger', 'to be continued']`.
  - The same dimension was the only one of the seven reading text whose whitespace was never collapsed, so a label the generator wrapped between its words was invisible to the phrase named for it. Repaired for a break **inside** one block: `To be\ncontinued` now reads as the label it is. **Known limit, deliberate:** a `<br>` puts the break *between* blocks — `splitStoryIntoTextBlocks` renders it the same `\n\n` a `<p>` produces — so `<p>To be<br>continued</p>` is still missed. A reconstruction that joined trailing blocks was written for that case and withdrawn: with the boundary's provenance already erased, joining cannot tell a torn label from two ordinary paragraphs, and it synthesised the label out of prose (`<p>She wanted to be</p><p>continued through the next trial.</p>`). A miss is the safe direction; a synthesised signal is the defect this dimension was being repaired for. Closing the gap properly needs boundary provenance in `splitStoryIntoTextBlocks`, which every scanner reads — a slice of its own.
  - `continuity`'s theme-word floor dropped words of three letters or fewer, and `sin` is one of the eighteen `CLASSIC_STORY_THEMES` — so it was filtered out of its own word list and could never be echoed.
- [ ] `In review` (not merged) — close the `<br>` gap the entry above records, by restoring the information the splitter was throwing away rather than by making the joiner cleverer. `shared/storyTextBlocks.ts` gains `splitStoryIntoRenderedParagraphs`, the same blocks grouped by the paragraph a reader sees, with `splitStoryIntoTextBlocks` defined as its flattening so the two cannot drift; the cliffhanger scan reconstructs within the final rendered paragraph only, and a join can no longer cross a `</p><p>`. The withdrawn reconstruction's rules were correct and are unchanged — what they lacked was the input they assumed. Stacked on PR #293, whose withdrawal it builds on; the prose-synthesis assertions that withdrawal added still pass unmodified.
- [ ] `Parked` (follow-up slice, not started; surfaced by PR #293 and deliberately not fixed there) — the still-open evaluator limit those repairs exposed: `scoreContinuity` matches a configured theme through the generic inflection matcher, which reaches `sin`/`sins` but not `sinful`, `sinner`, or `damnation` — and equally not `betrayed` for `betrayal`. `storyContentAnalysis.ts`'s `THEME_KEYWORDS` already holds the richer per-theme vocabulary. Wiring the two together is a slice of its own; see the Surprises entry below.
...
- The Phase 6 evals are only as good as what the deterministic scan behind them can see, and three of its signals were unreachable or inverted for reasons no eval could surface — an eval asserts on the report, and a report that cannot see a thing reports its absence indistinguishably from its absence in the story. Two of the three had been latent since the scan was written; the third was masked by a false positive, so removing the false positive is what exposed it. Worth treating "can this dimension see the thing it names?" as its own review question, separately from "does the eval assert on it".
- `scoreContinuity` reads a *configured* theme, which arrives as an arbitrary `string[]` from the request rather than from a closed set, so it matches through a generic stem-plus-inflection rule. That rule is deliberately inflectional-only (`WORD_INFLECTION_SUFFIXES` excludes `less` and `ly` because those build different words), and derivational forms are outside it in both directions: `sinful`/`sinner`/`damnation` for `sin`, and `betrayed` for `betrayal`. `storyContentAnalysis.ts`'s `THEME_KEYWORDS` holds the curated vocabulary for the eighteen classic themes and would fix this for those — but not for the twelve Story Lab seeds or for a caller-supplied value, so a lexicon-aware theme match needs a decision about the other two vocabularies before it is written. Not a defect introduced by PR #293; a limit it made visible.
