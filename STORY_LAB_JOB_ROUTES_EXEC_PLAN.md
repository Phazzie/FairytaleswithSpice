...
- Later follow-up (2026-09-04, PR #329): `POST /api/story-lab/jobs` continuation requests gained a new refusal case. A signed-in caller with a stored `contentBoundaries` profile preference and no `heatContract` on the request body is now answered `400 INVALID_REQUEST` (checked before the job is created, so it never reaches `queued`) rather than silently proceeding without honoring the boundary — `heatContractPolicyError` treats any *present* `heatContract` as needing `adultOnlyConfirmed: true`, so manufacturing one just to carry the boundary would reject a continuation that used to succeed; proceeding unchanged would mean the boundary never reaches the model. A signed-in caller with no stored boundaries, or any anonymous caller, is unaffected — see `api/_lib/story-lab/contentBoundaries.ts`'s `resolveContinuationHeatContract` and `PR70_RECOVERY_CHANGELOG.md`'s 2026-09-04 entries for the full history.
