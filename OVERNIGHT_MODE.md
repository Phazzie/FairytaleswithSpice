...
Overnight Mode is the operating guide for long autonomous coding sessions on Fairytales with Spice. Its job is to let an agent keep working after the first assigned task is done, without drifting into random unsafe changes.

The mode should create forward motion in four ways:

- Fix real issues.
- Advance the current product roadmap.
- Research adjacent products and mine ideas.
- Try small weird experiments that might improve story output or the creative workflow.
...
2. `gh pr list --state open --json number,title,headRefName,baseRefName,url`
...
If checks are already failing before edits, record the baseline failure in `OVERNIGHT_HANDOFF.md` and prioritize repair before feature work.
...
4. P0 items from `STORY_LAB_APP_AUDIT.md`.
5. The next unchecked gate in the active execution plan.
6. P1 items from `STORY_LAB_APP_AUDIT.md`.
7. Source-backed research ideas from `STORY_LAB_IDEA_BOARD.md`.
8. One bounded Weird Lab experiment from `STORY_LAB_IDEA_BOARD.md`.
9. Documentation cleanup that improves future autonomous runs.

## Lane Weights

When nothing is urgent and several tasks are valid, use this rough mix:

- 40 percent repair, tests, review comments, cleanup that removes real risk.
- 25 percent planned product/platform progress.
...
If no issue exists, log the task in `OVERNIGHT_HANDOFF.md` and, if it is an idea or experiment, add it to `STORY_LAB_IDEA_BOARD.md`.

## Research Mining Rules

Research is allowed and encouraged, but it must be source-backed and transformed into this app's language.

For each research pass:

1. Search current web sources. Do not rely on memory for product features or pricing.
2. Capture links and dates in `STORY_LAB_IDEA_BOARD.md` or a dedicated research note.
...
- Never auto-merge a weird experiment unless it proves value and passes normal gates.
...
- run function-count check when route files or `vercel.json` change;
- update docs/handoff if the work changes plan state;
...
Before the session ends, update `OVERNIGHT_HANDOFF.md` with:
...
If work continues after the handoff, keep the handoff fresh after each coherent slice.
