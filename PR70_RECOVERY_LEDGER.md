...
| #198 | close-as-superseded (maintainer call) | In review | Superseded by `STORY_LAB_LIVING_BOOK_AND_DURABLE_JOBS_EXEC_PLAN.md`, which already directs this PR be closed; branch also has unrelated git history vs. current `main`. Two comments posted 2026-09-06 (original status comment, then a corrective follow-up); see "Stale-PR triage cycle" below. |
...
None qualified for the "small and mechanical" auto-merge bar (single same-major dependency bump / docs-only / single-purpose cleanup with green CI and no open design questions). All three landed in the "larger or riskier" bucket, so each got a status comment rather than a merge or close, per this routine's instructions. #322 and #316 each got one; #198 ended up with two, because a Codex review of the PR carrying this ledger entry (#343) caught that the first #198 comment missed the authoritative source for its disposition — the second is a correction, not a second independent finding (see the "Self-review addendum" below).
