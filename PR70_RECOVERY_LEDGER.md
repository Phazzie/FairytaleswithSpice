...
| #198 | close-as-superseded (maintainer call) | In review | Superseded by `STORY_LAB_LIVING_BOOK_AND_DURABLE_JOBS_EXEC_PLAN.md`, which already directs this PR be closed; branch is stale against current `main`, not unrelated history. As of 2026-09-06 that was two ordinary merge conflicts (`AGENTS.md`, `PR70_RECOVERY_CHANGELOG.md`); as of 2026-09-10 `main` has grown a third, `STORY_LAB_COMPLETION_HARDENING_EXEC_PLAN.md` (still ordinary content conflicts, not unrelated history). Three comments posted 2026-09-06 (original status comment, a disposition-source correction, and a git-ancestry retraction); see "Stale-PR triage cycle" below. |
...
| #358 | merge (routine's own output) | In review | This routine's own prior-cycle output (re-verification of #322/#316/#198, no material change); docs-only ledger update on `claude/funny-bardeen-desw3j`. Opened 2026-09-09T08:19Z; inside the 3-day window at every subsequent cycle so far. |
| #360 | merge (routine's own output) | In review | This routine's own prior-cycle output (re-verification of #322/#316/#198, no material change); docs-only ledger update on `claude/funny-bardeen-i570b5`. Opened 2026-09-09T10:20Z; inside the 3-day window at every subsequent cycle so far. |
| #361 | merge (routine's own output) | In review | This routine's own prior-cycle output (re-verification of #322/#316/#198, no material change); docs-only ledger update on `claude/funny-bardeen-kaoiff`. Opened 2026-09-09T14:24Z; inside the 3-day window at every subsequent cycle so far. |
| #364 | merge (routine's own output) | In review | This routine's own prior-cycle output (re-verification of #322/#316/#198/#325/#327, no material change); docs-only ledger update on `claude/funny-bardeen-y49xak`. Opened 2026-09-10T00:18Z; inside the 3-day window at every subsequent cycle so far. |
| #367 | merge (routine's own output) | In review | This routine's own current-cycle output (re-verification of #322/#316/#198/#325/#327, no material change); docs-only ledger update on `claude/funny-bardeen-pcclaf`. |
...
## Stale-PR triage cycle - 2026-09-10 20:18 UTC

Scope per the automated stale-PR recovery routine: open PRs with no activity in the last 3 days, excluding `claude/dreamy-bardeen-*` branches (owned by the separate hourly quick-wins routine). Ten PRs were open at cycle start: #341, #358, #360, #361, #364, #327, #325, #322, #316, #198. `#358`, `#360`, `#361` and `#364` are this same routine's own prior-cycle output PRs (docs-only ledger updates on `claude/funny-bardeen-*` branches), created 2026-09-09T08:19Z through 2026-09-10T00:18Z — roughly 20 to 36 hours old at this cycle's 20:18 UTC start (not "within the last 20 hours" as first written; only #364 was that recent). All four are still inside the 3-day window regardless, so none qualified for this cycle's scope and none were touched. `#341` (last activity 2026-09-09T00:18:13Z) was also inside the window. The remaining five — `#327`, `#325`, `#322`, `#316`, `#198` — were all last touched 2026-09-06 and so were stale. (These are not the same five every cycle has rechecked — the 2026-09-06 04:52 cycle rechecked only #198/#322/#316, its 18:16 cycle only #325/#327, and the 2026-09-09 cycle found only #341 stale and left this five untouched as in-window. This is simply the set that happened to be stale at this cycle's start.)

Working copy was a shallow clone at cycle start; ran `git fetch --unshallow` before trusting any `git merge-tree` result, per the standing lesson above. Re-verified each of the five stale PRs against current `main` (`93f09ce7`, which had advanced by PR #366, a Proving Grounds access-control fix, since the last cycle's check). #366 is not blanket-unrelated to this set: it touches `story-generator/src/app/app.ts`, `app.config.ts`, and `app.spec.ts`, all files #325 also modifies — see the #325 bullet below for what that does and doesn't change:

- **#316** (bearer-token redaction rewrite): `git merge-tree` against current `main` is still a clean merge with no conflicts; the two owner-facing open decisions named in the PR's own description are still open, unresolved by anyone. No material change since the 2026-09-06 status comment. No re-comment.
- **#198** (Story Lab two-plan freeze, draft): `git merge-tree` shows the conflict set **grew by one file** since 2026-09-06: `AGENTS.md` and `PR70_RECOVERY_CHANGELOG.md` as before, plus `STORY_LAB_COMPLETION_HARDENING_EXEC_PLAN.md` now (exactly three files — verified via `git merge-tree origin/main FETCH_HEAD | grep '^CONFLICT'`, no others). `STORY_LAB_LIVING_BOOK_AND_DURABLE_JOBS_EXEC_PLAN.md` still exists on `main` and still supersedes this PR's Plan 2 scope per its own recorded direction. Disposition unchanged (close-as-superseded, maintainer call) but the exact conflict set was not previously re-verified file-by-file each cycle — corrected here and in the index table above. No re-comment (the growth doesn't change the recommendation already posted).
- **#322** (35-advisory lockfile-only remediation): `git merge-tree` still shows the same two conflicts (`PR70_RECOVERY_CHANGELOG.md`, `story-generator/package-lock.json`) already documented in-thread. Verified directly against the **resolved lockfile versions** on `main` (not just `package.json` ranges, which don't prove what's actually installed): `package-lock.json` still resolves `axios` to `1.16.1` and `form-data` to `4.0.5`; `story-generator/package-lock.json` still resolves `@angular/core`/`@angular/compiler` to `20.3.22`, `tar` to `7.5.15`, `undici` to `6.26.0`, `ws` to `8.17.1`, `engine.io` to `6.6.4`, `socket.io-parser` to `4.2.4`, `brace-expansion` to `1.1.15`, and `body-parser` to `2.2.2` — all still the pre-remediation versions this PR's description names as vulnerable. So the advisories this PR clears are confirmed still open on `main`, and #327 still repeats the major-bump-bundling mistake this PR exists to avoid. No material change to disposition. No re-comment.
- **#325** (Story Lab Cloud Account/Auth): confirmed `api/_lib/story-lab/auth/clerkSessionVerifier.ts` still exists on `main` from #326/#328 with materially different content than this PR's version of the same path. **Correction to this bullet's first draft:** it originally claimed "no further auth-related commits landed on `main` since the last comment" — false. #365 (`42d9b09`, merged 2026-09-10) added per-owner access control to the Story Lab transient story cache (`api/_lib/story-lab/stateStore.ts`, `contentBoundaries.ts`, `jobRouteHandlers.ts`, `storyLabEngine.ts`, the stories/continue routes) — real, recent, access-control-relevant work in the same Story Lab auth space this PR occupies, but touching none of the files #325 modifies. **Second correction:** #366 (the Proving Grounds access-control fix that also landed since the last comment, and that this cycle's opening paragraph first described as "unrelated to any of these five") is *not* unrelated to #325 specifically — it touches `story-generator/src/app/app.ts`, `app.config.ts`, and `app.spec.ts`, all of which #325 also modifies and which were already in #325's conflict set as of the 2026-09-08 cycle (before #366 existed). Re-ran `git merge-tree` against current `main`: the full conflict list is `PR70_RECOVERY_CHANGELOG.md`, `STORY_LAB_AUTH_PROFILE_CLOUD_LIBRARY_EXEC_PLAN.md`, `api/_lib/story-lab/auth/clerkSessionVerifier.ts` (add/add), `api/_lib/story-lab/auth/configuredAuthPort.ts`, `api/health.ts`, `package.json`, `story-generator/src/app/app.config.ts`, `story-generator/src/app/app.spec.ts`, `story-generator/src/app/app.ts`, `story-generator/src/app/auth.service.spec.ts` (add/add), `story-generator/src/app/auth.service.ts` (add/add), and `story-generator/src/app/contracts.ts` — real content conflicts throughout, not narrowed or widened in a way that changes the picture; #366 didn't introduce new conflicting files here, it landed on top of app files that were already conflicting. **Third correction:** this list omits two files the 2026-09-06 comment on this PR named as conflicts, `README.md` and `.env.example`, and that omission needed checking rather than silent dropping. Verified directly: `README.md` now merges cleanly (`Auto-merging README.md`, no `CONFLICT` line) — the specific hunks that conflicted on 2026-09-06 no longer overlap, most likely because `main`'s own README churn since then moved past them. `.env.example` doesn't appear in the merge-tree output at all because **`main` has never had this file** — `git show origin/main:.env.example` returns "does not exist," and none of the commits in this repository's history that touch `.env.example` (all on `#325`'s own branch or other unmerged branches) are ancestors of `main`. Why the 2026-09-06 comment recorded it as a conflict is unresolved — either something on `main` briefly had a colliding `.env.example` that was removed since, or that earlier check was itself wrong — but as of this cycle, `.env.example` is not a real conflict for #325 because `main` has no version of it to conflict with. Disposition unchanged (close-as-superseded, maintainer call). No re-comment.
- **#327** (Dependabot 20-update npm/yarn group): `main`'s `package.json` files are still untouched by any of these 20 updates (see #322 check above); the Vercel deployment check on this PR's head commit is still red from 2026-09-03 (`dpl_2kXnwpKVqHc9im5nAqRWTS7C1kZx`, confirmed via a fresh `pull_request_read get_status` call). Still the same major-Angular-bump-bundling problem the existing comment already names. No material change. No re-comment.

None of the five were merged or closed this cycle. Their *dispositions* are unchanged from the 2026-09-06 status comments (needs-a-human-decision / close-as-superseded / needs-to-be-split, per PR), which is what "do not repeat unless something material changed" turns on — a comment restates a recommendation, and none of the recommendations changed. That's distinct from their *supporting facts*, some of which did move: #198's conflict set grew from two files to three, and #325 now sits under two more unrelated `main` commits (#365, #366) than it did on 2026-09-06, one of which (#366) touches files already in its conflict set. Neither delta changes what a maintainer would be told to do, so no new comment was posted on either PR — but the ledger text above was corrected to reflect the current facts rather than silently carrying forward stale ones. `#341` and this routine's own four prior-cycle PRs (`#358`, `#360`, `#361`, `#364`) were all inside the 3-day window at cycle start and were left untouched under the literal rule — no re-check performed.

**Self-review addendum.** Codex's review of the PR carrying this ledger entry (#367) caught seven real gaps across two rounds, all corrected in place rather than left as first written.

Round 1: the #198 conflict-set claim didn't note it had grown to three files (fixed above and in the index table); the #322 evidence cited unchanged `package.json` ranges, which don't prove what's actually resolved in the lockfiles — replaced with the actual resolved lockfile versions; the #325 bullet's "no further auth-related commits landed on `main`" was flatly false (#365 landed 2026-09-10) even though it doesn't change #325's disposition since it touches none of #325's files; and this routine's own four still-open prior-cycle PRs (#358, #360, #361, #364) were missing from the `PR Status Index` despite the index's stated purpose of tracking every currently open PR — added above, along with this cycle's own #367.

Round 2, after the round-1 fixes were pushed: the opening paragraph's claim that #366 was "unrelated to any of these five" was false for #325 specifically — #366 touches `app.ts`/`app.config.ts`/`app.spec.ts`, files already in #325's conflict set — corrected to call this out and re-verify the full conflict list rather than dismiss #366 wholesale; the closing paragraph's "all five... unchanged" conclusion contradicted the #198 conflict-set growth documented two paragraphs earlier — reworded to distinguish unchanged *disposition* (what determines whether a new comment is warranted) from changed *supporting facts* (which still get corrected in the record); and the "(the same five identified by every cycle since 2026-09-06)" parenthetical was checked against this ledger's own history and found false — three different cycles rechecked three different subsets — removed and replaced with an accurate note.

Round 3, after the round-2 fixes were pushed: the "created within the last 20 hours" claim about #358/#360/#361/#364 didn't hold arithmetic up — #358 was ~36 hours old at this cycle's start, not ~20 — corrected to state the actual ~20–36 hour range; and the round-2 #325 conflict list silently dropped `README.md` and `.env.example`, which the 2026-09-06 comment on that PR had recorded as conflicts, without explaining the discrepancy — checked directly and corrected: `README.md` now merges cleanly (`main`'s own churn moved past the conflicting hunks), and `.env.example` was never on `main` at all, so it was never a real merge conflict regardless of what the earlier comment said.
