...
| #198 | close-as-superseded (maintainer call) | In review | Superseded by `STORY_LAB_LIVING_BOOK_AND_DURABLE_JOBS_EXEC_PLAN.md`, which already directs this PR be closed; branch is stale against current `main` with three ordinary merge conflicts (`AGENTS.md`, `PR70_RECOVERY_CHANGELOG.md`, `STORY_LAB_COMPLETION_HARDENING_EXEC_PLAN.md`), not unrelated history. Three comments posted 2026-09-06 (original status comment, a disposition-source correction, and a git-ancestry retraction); see "Stale-PR triage cycle" below. |
...
## Stale-PR triage cycle - 2026-09-09 10:16 UTC

Scope per the automated stale-PR recovery routine: open PRs with no activity in the last 3 days, excluding `claude/dreamy-bardeen-*` branches (owned by the separate hourly quick-wins routine). Seven PRs were open at cycle start: #358, #341, #327, #325, #322, #316, #198. **#358** is this same routine's own docs-only ledger PR recording a still-unmerged "2026-09-09 08:16 UTC" cycle (branch `claude/funny-bardeen-desw3j`, created 2026-09-09T08:19:27Z) — that cycle exists only in #358's diff, not yet as a section in this committed document. #358 is not stale by a wide margin, left untouched, not merged here (a separate concern from this triage pass). Computed staleness against the cycle-start timestamp (2026-09-09T10:16:35Z, cutoff 2026-09-06T10:16:35Z): **#322, #316 and #198** again fell before the cutoff (last activity 2026-09-06T05:21:52Z, 04:56:25Z and 05:21:48Z respectively) — the same three named in every cycle since 2026-09-08. #327 and #325 (2026-09-06T18:21Z) and #341 (2026-09-09T00:18Z) all had activity inside the 3-day window and were left untouched.

Re-verified all three against current `main` (`22fa4608`), which advanced by one PR — **#359** ("Export fidelity: `<em>`/`<strong>` silently vanished from 4 of 5 export formats") — since the base commit #358's still-unmerged diff was checked against (`888f3dd`). `git log 888f3dd..origin/main` touches only `CHANGELOG.md`, `api/_lib/services/exportSanitizer.ts`, `api/_lib/services/exportService.ts`, `api/_lib/services/imageService.ts`, `api/_lib/utils/textExcerpt.ts`, `shared/storyDownloadFilename.ts`, `tests/export-sanitizer.test.ts`, `tests/export-service.test.ts` — none of which intersect the files these three PRs' dispositions depend on.

This cycle re-verified mergeability with a real `git merge --no-commit --no-ff` test-merge on each PR branch against `origin/main` rather than reading `git merge-tree`'s raw three-way diff, after that raw diff falsely flagged #316 as newly conflicting in `tests/log-redaction.test.ts`; the actual merge auto-resolves that file cleanly (git's rename/context-aware merge succeeds where a naive three-way tree diff reports "changed in both"). Each test-merge branch was created, inspected, and deleted; nothing was pushed.

- **#322** (35-advisory lockfile-only remediation): real test-merge still conflicts, same two files as every prior cycle — `PR70_RECOVERY_CHANGELOG.md` and `story-generator/package-lock.json` (root `package-lock.json` merges cleanly on its own). CI unchanged, still 5/5 green (Vercel Preview Comments, SonarCloud, SonarCloud Code Analysis, Sourcery review, Validate Vercel recovery build). No new PR comments since 2026-09-06T05:21:52Z. No re-comment — nothing material changed.
- **#316** (bearer-token redaction rewrite): real test-merge auto-resolves cleanly with no conflicts (confirms `mergeable_state: clean` despite the raw `git merge-tree` false flag above). CI unchanged, still green (SonarCloud ×2 `success`, Vercel Preview Comments `success`, Sourcery review `skipped` as before). The two owner-facing open decisions named in the PR description are still open. No new PR comments since 2026-09-06T04:56:25Z. No re-comment.
- **#198** (Story Lab two-plan freeze, draft): real test-merge now conflicts on **three** files — `AGENTS.md`, `PR70_RECOVERY_CHANGELOG.md`, and, newly enumerated this cycle, `STORY_LAB_COMPLETION_HARDENING_EXEC_PLAN.md`. This is a more complete accounting of an already-dirty PR, not a new conflict introduced this cycle: the third file's conflicting hunks trace to two "Fix ... findings on #357" commits (`d53c886e`, `2fcc9f4c`) that were already on `main` before commit `888f3dd` (`git log 888f3dd..2fcc9f4c` is empty, i.e. `2fcc9f4c` is not newer than `888f3dd`) — prior cycles simply didn't enumerate this file. CI unchanged (still carries the two known-failing "Validate Vercel recovery build" runs from 2026-07-31; PR remains draft). `STORY_LAB_LIVING_BOOK_AND_DURABLE_JOBS_EXEC_PLAN.md` still supersedes it with the same closure directive. No new PR comments since 2026-09-06T05:21:48Z. No re-comment.

None of the three were merged or closed this cycle, for the same reasons as every prior cycle: #322 is a multi-package dependency bundle outside the "single dependency bump within the same major version" auto-merge bar and its lockfile conflict needs an actual regeneration on rebase; #316 carries two open questions the PR's own description defers to the repo owner; #198 is a draft architecture/planning document explicitly outside this routine's close authority, even though the superseding exec plan already calls for its closure. All three dispositions match the prior committed cycles' findings (2026-09-08 22:16 and 2026-09-09 00:16 UTC, both above) exactly, and also match the still-unmerged #358's 08:16 UTC re-check, so per the "don't repeat unless something material changed" instruction, no new status comments were posted on any of the three PRs — this entry exists only to record that the re-check happened and found no change.
