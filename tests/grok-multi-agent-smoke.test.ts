...
async function main(): Promise<void> {
  const shouldRun = process.env['RUN_REAL_GROK_MULTI_AGENT_SMOKE'] === '1' && Boolean(process.env['XAI_API_KEY']);

  if (!shouldRun) {
    console.log('SKIP: Set RUN_REAL_GROK_MULTI_AGENT_SMOKE=1 and XAI_API_KEY to run the real Grok multi-agent smoke test.');
    return;
  }
  ...
  assert(response.text.trim().length > 20, 'Expected non-empty Grok multi-agent text.');
