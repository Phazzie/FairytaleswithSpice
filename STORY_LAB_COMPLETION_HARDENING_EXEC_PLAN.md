...
- As of 2026-09-03, PR #328 hardened the Clerk verifier's `authorizedParties` origin check through an extended run of Codex review rounds, each finding a real gap the prior round left: no origin check at all; a static-only allowlist that would 401 legitimate Vercel-preview sign-ins; a fix for that which trusted a caller-supplied `X-Forwarded-Host` header for a security boundary where CORS-safe reasoning doesn't transfer; the same stale-response race `refreshCloudLibrary()` had, unfixed on save/load/delete; a fail-open gap where an explicit-but-entirely-invalid origin config (`STORY_LAB_ALLOWED_ORIGINS=*`, say) combined with no platform URL produced an empty `authorizedParties`, which Clerk treats as "no restriction" rather than "reject everything"; a split between the verifier's own fail-closed check and the unrelated auth-config endpoint; an account-switch-without-sign-out race the constructor effect didn't track; and a missing stable-production-origin platform variable. Lesson for future auth-boundary changes here: verify what the underlying library does with an edge-case *empty* value for a security-relevant option, not just its non-empty behavior. **Do not restate the round count here** — it goes stale every time Codex finds one more; `PR70_RECOVERY_CHANGELOG.md`'s dated, numbered "Clerk auth hardening round N" entries are the current source of truth for both the count and the finding-by-finding history.
