...
## 2026-08-26 UTC - A Theme Seed Counted But Never Measured, A Stream That Quotes Its Own Crash, And A Title Only Four Exports Agree On

Actions:

- Added `maxThemeIdLength` (80), `maxThemeLabelLength` (80), and `maxThemeDescriptionLength` (280) to `STORY_BLUEPRINT_LIMITS`, and made `parseThemes` refuse a seed past any of them. `themes` was the one blueprint field checked for shape and never for size: the parser counted the entries against `maxThemes` and confirmed each carried `id`, `label`, and `description` strings, then accepted a megabyte under any of the three. That module's own comment names the fields it bounds — logline, world details, narrative directives, no-go content — because each is interpolated into a paid Grok prompt; a theme seed is free text arriving on the same two routes and was not on the list. `buildContinuityPrompt` interpolates `themes.map(theme => theme.label)` uncapped beside `existingState.threads`, whose `label` and `description` `buildInitialThreads` seeds from these same strings, so an oversized seed is written into the story state, persisted with it, and re-sent on every continuation for as long as the serial runs — while the chapter prose next to it in that prompt is capped at 2,200 code points precisely because prompt size is billed. The numbers are the repository's own: 80 and 280 are `StoryService`'s `STORY_LAB_THEME_LABEL_MAX_LENGTH` and `STORY_LAB_THEME_DESCRIPTION_MAX_LENGTH`, which it silently truncates seeds to before building a prompt, and 80 is what `STORY_EVALUATION_LIMITS` already calls "one theme id or creature name, not a paragraph wearing the field's name".
- Stopped `/api/story-lab/stream/genesis` quoting its own exception to the browser, and made it log one. The route sent `error.message` verbatim in its SSE `error` frame — whatever threw, with nothing between the engine and the frame to narrow it — and `StoryService` turns that into `observer.error(new Error(chunk.error.message))`, so a driver quoting a failing statement or a client naming an internal host reached the reader. `redactSensitiveLogData` blanks this class of text on the way into the log; nothing was blanking it on the way to the client. The branch also logged nothing at all, so the operator was the one party who never saw the failure. Its POST twin `/api/story-lab/stories` has answered a fixed message and called `logError` for the same failure since it was written.
- Gave that route the `createStoryLabGenesisStreamHandler(generateGenesis = generateStoryLabGenesis)` factory the two Story Lab POST routes already use, default export unchanged. The throw branch was untestable while the engine was imported and called directly: a blueprint that parses is one the mock engine generates a story for, so no input could make the real call reject — which is why the branch shipped sending the raw message.
...
- Extended `tests/story-lab-blueprint-parser.test.ts` (per-field caps at and past the limit, on both the body and the query form the stream parses), `tests/story-lab-stream-genesis.test.ts` (a thrown generation is not quoted into the frame and still ends the stream), and `tests/export-service.test.ts` (the PDF heading reads as the `.txt` export's title does). No new test files, so no new `test:all` entries.

Self-review:

- One semantic counterfactual per fix, run and reverted: weakening the theme-seed length comparison, restoring `error.message` on the stream's error frame, and restoring the raw `input.title` in the PDF each made the matching new assertion fail. None is committed.
- The PDF assertion compares against the `.txt` export rather than a literal, since the defect was precisely that the repository's two plain-text renderings of one story disagreed; the stream assertion looks for the thrown text's own distinctive contents rather than comparing the whole frame, so it keeps holding if the fixed message is later reworded.
- The theme caps are enforced in the parser rather than at either route, so the POST body and the genesis stream's query string get the same answer — the reason that module says the numbers are defined once.
- `npm test` passes in full; `npx tsc -p story-generator/tsconfig.json --noEmit` is clean, which covers `shared/` and, through the Node server's route table, the `api/` tree; the Vercel function count is unchanged at 9/12.
- Non-claim: no generated story changes, and no prompt the server sends changes for any input the app can produce — the app's theme picker offers the twelve fixed seeds in `shared/storyLabThemeSeeds.ts`, all far inside the new caps. What changes is what a direct API caller is refused, what a failed stream tells the reader and the log, and the heading on one export format.
