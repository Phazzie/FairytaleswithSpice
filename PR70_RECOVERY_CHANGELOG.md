...
## 2026-08-28 UTC - The Two Chapter-Heading Readers Ended A Heading At The First `>` Rather Than At The Heading's Own

Actions:

- Fixed both `<h3[^>]*>` readers named by #296. `extractChapterTitleAndBody` (`storyContentAnalysis.ts`) ended the opening tag at the first `>` rather than at the tag's own, so a `>` inside a quoted attribute value left the remnant in the capture group — `<h3 data-x="a>b">Real Title</h3>` produced the chapter title `b">Real Title`, and `stripHtml` could not clean it because the truncation leaves no tag in it to strip. The same truncation left the rest of the real opening tag in `headingMatch[0]`, and therefore in the body sliced by removing it.
- **#296's stated premise for the second reader does not reproduce.** It predicted `renderChapterForAppend` (`storyService.ts`) fails "for the same reason"; it does not, because the lazy `.*?<\/h3>` after the truncated opening tag runs on to the real `</h3>` and the heading is stripped anyway. It stops recovering only when a newline falls between the truncation point and the `</h3>`, since `.` does not match a newline: the pattern then matches nothing at all, the existing heading survives, and the caller appends its own in front of it, so the chapter goes out carrying two headings with the second as raw markup. Real defect, different cause than filed.
- Added `TAG_ATTRIBUTES_PATTERN` to `shared/storyTextBlocks.ts` and composed it in both readers. Moved the strip out of `StoryService`'s private `renderChapterForAppend` into an exported `stripLeadingChapterHeading` in `storyContentAnalysis.ts`, beside the other reading of the same heading — it was previously reachable only through a model call, which is why it had no test.
- Scope held deliberately: `BLOCK_BOUNDARY_PATTERN` and `stripInlineTags` in the same shared module keep their own `[^>]*>` / `[^<>]*` forms. Those are the block splitter, they are where #295's "finding 8" (a pattern cannot know whether its match sits in HTML's data state or inside an enclosing tag's attribute value) actually bites, and they are the row #296 wants a tokenizer for. #296 stays open: its `shared/storyTextBlocks.ts` row and its literal-`<` row are untouched.

Self-review:

- **A first version of this pattern paired quote characters wherever they appeared, and that was a regression against `main`, not merely a missed improvement.** Codex review found it: in `<h3 a=b">c">Real Title</h3>`, `b"` is an unquoted value that merely contains a quote, so HTML ends the tag at the very next `>` and a reader sees `c">Real Title`; pairing `b"` with `c"` read the tag past its own `>` and returned `Real Title`, deleting text `[^>]*>` had kept. Rewritten as HTML's own attribute grammar — a list of names, each optionally followed by `=` and a value — so a quoted run opens only directly after an assignment. This is the same failure class that produced findings 2, 4 and 5 on #295 and rounds 1 and 2 on #297.
- **The rewrite introduced catastrophic backtracking twice, both caught before pushing.** An optional separator between attributes let an n-character name be divided into attributes in 2^n ways, and an unquoted value permitted to *begin* with a quote gave `a="b"` one reading per alternation branch. A 1,600-attribute unclosed tag did not finish in two minutes. Fixed by requiring at least one separator and by excluding the quote from an unquoted value's first character only; both are now pinned by timing assertions sized so a regression fails in under two seconds rather than hanging CI. Current cost at 80,000 attributes is ~14ms, linear.
- Whitespace is spelled `[ \t\n\f\r]` rather than `\s`, because JavaScript's `\s` matches NBSP and HTML's tag whitespace does not — #295's finding 7. The conservative consequence (an NBSP between attributes takes the fallback) is pinned by its own test.
- **Correction to a measurement method, recorded because it nearly became a false finding.** The first append-path metric counted any prose word that disappeared as a loss and reported 51,975 of them. Removing the heading is the point of that function, so every one was the heading being correctly stripped; re-measured against the body, the number is 0. The same trap #297 recorded hitting.
- **A second Codex round found a second regression of the same class**, on the fix for the first. `/` was excluded from an unquoted value and accepted as an attribute separator; HTML does the opposite, giving the slash a meaning only in the state *before* an attribute name. So in `<h3 a=//b="c>d">Real Title</h3>`, `//b="c` is one unquoted value and the tag ends at the very next `>` — but this read `b="c>d"` as a second attribute, ran through the second `>`, and deleted the `d">` a reader sees. Fixed by moving the slash into the value and making the separator whitespace-only. The deliberate cost: a trailing self-closing slash (`<h3 a="b"/>`) is no longer a separator, so that markup falls back and answers as `[^>]*>` does — coverage, not correctness.
- **A third Codex round found a P1 catastrophic backtracking**, and this is where the approach stops. With whitespace permitted after `=`, a value could begin past the separator and consume the next attribute's name, so ` a= a=` reads either as one attribute valued `a` or as two valueless ones — 2^n partitions once the tag never closes. Measured here: 32 repeats 275ms, 36 repeats 2.0s. Fixed by forbidding whitespace between `=` and the value (whitespace *before* `=` is still read); linear afterwards, 100,000 repeats in 12ms. The narrow cost is that `a = "b>c"` takes the fallback.
- **The third round's fix carried a sixth defect, of the same class it was fixing, and it was found by measurement rather than by review.** Forbidding whitespace after the `=` disambiguated ` a= a=`, but it also meant `a= b="c>d">` could no longer read `b="c` as `a`'s value — so the *optional* value took the empty reading instead, the space became a separator, `b="c>d"` was read as a second attribute, and the scan ran through the tag's own `>`. A browser shows `d">Real Title`; that revision returned `Real Title`. Identical in kind to the quote defect and the slash defect: reader-visible prose deleted, reached through a third character. The commit that introduced it is the one titled "and stop".
- **Fixed by requiring the value rather than by narrowing another separator, which is the first fix in this sequence that removes a class instead of an instance.** The empty reading was what carried all three: it is available after any bare `=`, so whatever follows reads as the next attribute. Narrowing separators moves the loss between characters (slash → whitespace → the `>` itself) and cannot exhaust them. HTML admits an empty value in exactly one position — before-attribute-value skips whitespace, and a `>` there ends the tag — so that position is now named by a lookahead and no other is admitted.
- **Requiring the value also removes the third round's 2^n, which means the whitespace it cost can come back.** ` a= a=` had two readings only because the value could be empty; with it required there is one, so whitespace is legal on both sides of the `=` again, as a browser reads it. `a = "b>c"` is read whole rather than surrendered to the fallback. Measured against a reference HTML tokenizer, that is **11,366 more tags ended where a browser ends them, and none ended worse.**
- **A quadratic was introduced by the gate itself and caught before pushing.** With the lookahead placed after the `=`'s whitespace, the run gives a character back and the lookahead rescans the rest of it, though the answer cannot differ: `a=` and 40,000 spaces took 761ms, quadrupling per doubling. The two branches are mutually exclusive, so trying the empty reading first — before any whitespace is consumed — is answer-preserving and linear. Pinned by its own timing guard.
- **Adopted an HTML tokenizer as the oracle, because "never worse than `[^>]*>`" cannot see this defect class.** That metric scores any deleted prefix as a remnant correctly removed, so deleting `d">` — text a browser shows — reads as an improvement; it scored the sixth defect clean. Implementing the WHATWG tag states (~40 lines, tests only) and comparing tag *end positions* measures the property that actually matters. Over every tag interior of length 1..9 over `{a = " > space /}` (11,525,019 emitting a tag), the shipped pattern ends the tag where a browser does on **98.404%**, against 98.382% for `[^>]*>`. Every remaining disagreement is the documented recovery class — `a="b"c="d"`, missing whitespace between attributes — where the pattern deliberately falls back.
- **This does not overturn the recommendation below; it is the sixth data point for it.** The defect was in the commit that concluded the sequence, the metric in use could not detect it, and it took a different oracle to find. What the round changes is the count and the diagnosis — the blowups and the data-loss defects are one fault, ambiguity, and the empty value was its last carrier here — not the conclusion. Eleven counterfactual mutations, including the exact branch-head combination, now fail a suite; `npm run test:all` exits 0, typecheck clean, function count 8/12.
- **Recommendation: do not merge this slice; use a tokenizer.** Five defects across four revisions of one pattern — two shipped data-loss regressions (a quote inside an unquoted value, a slash after an assignment) and three catastrophic-backtracking blowups (an optional separator, an unquoted value that could begin with a quote, whitespace after `=`). All three blowups are the *same* underlying fault: two constructs able to consume the same characters. Each was found reactively, one review round at a time, and nothing here proves the fourth was the last — there is no method in this approach for showing a regex is unambiguous, only for testing the shapes someone thought of. A left-to-right scanner consumes each character once and cannot have the fault at all. That is a structural guarantee rather than a property to keep testing for, and it is what #296 concluded before this slice began; the slice is the evidence for it, not against it.
- The defect being repaired is cosmetic and has never been observed in production output. The same is true of every input that broke this pattern — which is the point: it cannot be right to dismiss the availability risk as unlikely while justifying the change on inputs of exactly the same likelihood. On that comparison the trade is negative, and the honest recommendation is the tokenizer or nothing.
- Suggested path for whoever takes it: `api/_lib/services/exportSanitizer.ts` already has a `tokenizeHtml`/`parseHtmlTag` scanner, and #297 fixes the same first-`>` truncation in it. Once that lands, both chapter-heading readers should call it rather than a pattern. The corpora, counterfactuals and linearity bounds recorded here transfer directly as its acceptance criteria.
- **Replaced generated fuzzing with exhaustive enumeration**, because the generator's blind spots are exactly what let the first regression through — it could not emit a quote inside an unquoted value followed by a `>`. Character-level, every tag interior over three alphabets — `{a = " ' > space < /}` to length 7 (2,396,744 inputs), `{a = " > space}` and `{a = / " >}` to length 8 (488,280 each): **0 cases where the new reader loses prose `main` keeps**, every difference a strictly cleaner title.
- **Added fragment-level enumeration after character-level missed the slash defect**, which needs 12 characters of tag interior and so lay past any feasible character enumeration. 30 attribute fragments covering every grammar state, in all combinations of three (27,000 interiors, reaching ~30 characters): 0 losses on the title path and 0 on the strip path.
- **The strip metric was wrong a second time, in the same way.** Scoring the strip path on all prose reported 433 losses; every one was heading text being correctly removed, which is the function's purpose. Scored on the body alone it is 0. Recorded because this is the identical error to the 51,975 above, made again after being written down.
- Eleven counterfactual mutations applied, each confirmed failing a suite, then reverted; none committed. Two initially survived and the tests gained inputs that discriminate. Two of the eleven are the shipped data-loss regressions themselves, and the exact pre-fix combination for the slash one is pinned separately because neither half reproduces it alone.
- Final verification on the last revision, all clean: character-level exhaustive over three alphabets (2,396,744 + 488,280 + 488,280 inputs) and fragment-level enumeration (32 fragments, all triples, 32,768 interiors reaching ~30 characters) show **0 cases where either reader loses prose `main` keeps**, on the title path and the strip path alike. Ten adversarial shapes all linear at 32,000 repetitions. `npm run test:all` exits 0; typecheck clean; function count 8/12.
- `npm run test:all` exits `0`. API function typecheck clean under preflight's exact command; `check-vercel-function-count.sh` 8/12 unchanged; `git diff --check` clean. No test reaches a store, an auth port, or a model provider.
- Not claimed: no evidence either defect occurs in production output today. The generator is prompted to emit bare `<h3>Chapter N: Title</h3>`. Both are worth fixing because the failure is silent and reader-visible, not because it is common. These readers are also not correct in the sense a tokenizer would be — they read a tag correctly but still *find* it by pattern match, so finding 8 remains true of the unanchored one exactly as it was before.
