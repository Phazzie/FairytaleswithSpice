...
- Non-claim: `StoryLabProfilePreferences.defaultHeatContract`, `favoriteCreatures` and `favoriteTones` are stored, validated, redacted and persisted with no reader in either tree — only `librarySort` is read, by `handleProjectsRoute`. Recorded here because it was found while tracing the blueprint defaults; applying or retiring those three is a feature, not a cleanup.
