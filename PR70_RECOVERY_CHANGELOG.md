...
- Made `shared/storyTextBlocks.ts` read a tag past its quoted attribute values. A tag does not end at the first `>` — it ends at the first `>` that is not inside a quoted attribute — and both readers in the module used `[^>]*`, which stops early. On `<p title="a>b">Hello.</p>` the boundary pattern matched `<p title="a>` and the tag's own remaining text came back as prose: `["b\">Hello."]`. That is reader-visible rather than internal, because `stripStoryHtmlToText` is the module's other export — it is what the `.txt` and `.pdf` exports render and what gets pasted into the next continuation prompt, so the attribute remnant is shown to the reader and then sent to the model as part of the story so far.
