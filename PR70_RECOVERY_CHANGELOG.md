...
Thirteenth review round (Codex, on `14912e2`) — two P2s, **one a containment leak the previous round introduced, and it is the third time this exact trap has been sprung here**:

- **Fixed: the breakout must not fire inside an integration point.** `<svg><foreignObject><p>secret</p></foreignObject></svg>` exported `secret` in every format. HTML parses HTML content normally inside `foreignObject`, `desc` and `title` (SVG) and `mtext`, `mi`, `mo`, `mn`, `ms` and an HTML-encoded `annotation-xml` (MathML), so a `<p>` there is *inside* the SVG rather than a boundary. Reading it as a breakout ended the skip early and put the foreign element's own contents into the export — precisely what dropping `<svg>` exists to prevent, and a regression against both `main` and the previous commit.
- **Fixed by requiring depth 0, not by listing the integration points.** Recognising them exactly needs an element stack and, for `annotation-xml`, the value of an attribute — a parser, which this module is not. Requiring the breakout to sit at the foreign element's own content level is the containment-safe *subset* of the same rule: every integration point is nested inside the foreign element, so no breakout can fire inside one. **The property that matters is the failure direction** — a case this rule gets wrong costs prose, and can never leak. A name list gets the opposite failure direction, and this module has now had three rounds (`closesItself`, raw-text nesting, integration points) where a missed name in an enumeration was a leak.
- The cost is asserted rather than left silent: plain nested foreign content (`<svg><g><a>…</svg!><p>`) does break out in a browser and does not here, so the prose is kept inside an unclosed foreign element instead of after it. That is the direction this module's policy already commits to when the two conflict — keep the reader's words *except* where keeping them would export a dangerous element's contents.
- **Declined: the conditional `font` breakout.** `<svg>hidden</svg!><font color="red">Story.</font>` loses `Story.` where `main` keeps it. HTML breaks out at `<font>` only when it carries `color`, `face` or `size`, and `parseHtmlTag` exposes no attributes, so the rule cannot be expressed here. Adding `font` unconditionally is not a clean win either — it would break out at a bare `<font>`, exporting text a browser hides from inside a dropped container. Recorded on #296 with the other malformed-markup policy rows rather than guessed at.
- **Re-fuzzed with a generator that can produce the shape it missed.** The previous round's 1,980-input containment pass reported 0 leaks and was *right about every input it could generate* — it simply had no integration points and no plain foreign nesting in it, which is the same failure as the well-formed-attributes generator four rounds earlier. Extended to emit both: 3,575 inputs, **0 leaks on `main`, 0 on the pre-breakout branch, 0 here, and 0 new leaks introduced**, with 0 prose lost against the pre-breakout branch and 8 against `main` — all eight the documented nesting cost above.
- **The honest read on convergence.** One fix drew two findings, one of them a leak. That is the non-converging pattern this log named at round eight, now with a safety cost attached, and it is why the fix here closes the *class* by failure direction rather than adding another enumeration. The remaining divergences — conditional `font`, and exact integration-point recognition — are recorded on #296 as the parser-shaped work they are, not taken here.
