...
Validation: added `tests/critical-alert-sink.test.ts` (8/8: sink selection, webhook payload formatting/redaction-safety, non-throwing behavior on delivery failure, default-console regression, `dispatchCriticalAlert` routing), extended `tests/health-route.test.ts` for the new field (2 new cases); registered as `npm run test:critical-alert-sink` and wired into `test:all`; full `npm run test:all` clean; API typecheck (`tsc --noEmit` over `api/**/*.ts`, the exact preflight invocation) clean; `check-vercel-function-count.sh` 9/12, unchanged; `git diff --check` clean.
