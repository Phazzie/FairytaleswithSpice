...
- Wired `showCloudAccountSetupStatus()`/the account-panel action to `AuthService.signIn()`/`signOut()` when Clerk is configured. The unconfigured *observable* behavior is unchanged — `showCloudAccountSetupStatus()`'s "not configured yet" message is byte-for-byte the original text, and `app.spec.ts`'s pre-existing assertions all pass unmodified — but the suite itself gained an `AuthService` double and `tick()`-based async handling to exercise the new constructor path, and the `App` constructor now always calls `AuthService.initialize()` (one `/api/health` request and a new backend code path even when unconfigured). Not a byte-for-byte-identical runtime; see the exec plan's Acceptance section for the same correction.
...
- Codex review (7 rounds across the PR's life) caught real issues past what the unit tests alone did: a missing `authorizedParties` restriction (fixed, with a negative test); the `/api/health` payload duplication above (fixed); `AuthService.initialize()` caching a failed health check forever, and the click handler never re-calling it (both fixed — see Self-review); no UI path to sign out, and sign-out being gated on cloud-storage success rather than auth state (both fixed — see Self-review); local dev's documented `.env` never actually loading, across two rounds of fix (fixed — see Self-review); `cloudAccountAuthenticated` being inferred from HTTP status alone, which a gateway failure or a pre-auth-gate `ACCOUNT_ROUTE_NOT_FOUND` could spoof (fixed — see Self-review); an `.env.example` template whose optional credential placeholders were truthy, defeating the mock-mode fallback for an untouched copy (fixed - commented out); this file and Phase 6 itself going stale relative to the PR's own later rounds (fixed — this entry, and the Phase 6 edits it describes); `cloudAccountAuthenticated` only being reset by the library-refresh path and not by save/load/delete's own 401s, and `cloudAccountStatusLabel` not reading that signal at all (both round 6, fixed — see Self-review); no session token refresh mechanism once a Clerk session expires (round 7, documented as an explicit non-claim rather than fixed — see Self-review). One finding (claiming `verifyToken` returns the bare payload rather than `{ data }`) was checked against the installed `@clerk/backend@3.17.1` source and found incorrect; replied on the PR thread with the exact source lines instead of changing already-correct code.

Self-review:

- Correction, caught by Codex round 2, fixed same round (`09bf50b`): fixing "`initialize()` caches a failed health check forever" by clearing the cached promise in `loadAuthConfig()`'s catch block was necessary but not sufficient — nothing then re-called `initialize()` from the click handler, so the retry path was unreachable from the UI. `showCloudAccountSetupStatus()` now awaits `initialize()` again before deciding to sign in.
- Correction, caught by Codex round 2, fixed same round (`09bf50b`), then found still incomplete by Codex round 3 and fixed again (`e5b3361`): the sign-out affordance was gated on `cloudLibrarySyncState().mode === 'cloud_synced'`, which requires both a successful auth check *and* a successful cloud-storage listing — an authenticated user hitting a storage outage would see "Connect account" and get redirected back to sign-in instead of being able to sign out. Round 2's fix added a `cloudAccountAuthenticated` signal but only updated it in the `next` RxJS handler; round 3 found `StoryService.handleHttpError` rethrows every non-2xx response, so a real 401/5xx from this backend always lands in `error`, never `next` — round 2's own test used `of({ success: false })`, a shape this backend never sends, which is why it didn't catch the gap. Fixed by moving the check into the `error` callback and rewriting the tests to `throwError(() => new HttpErrorResponse(...))`.
- Correction, caught by Codex round 5, fixed same round: that `error`-callback fix itself inferred "authenticated" from HTTP status alone (`status !== 401`), which a gateway/proxy failure (a 502, carrying no JSON body) or `accountRouteHandlers.ts`'s own `ACCOUNT_ROUTE_NOT_FOUND` (answered *before* the auth gate runs) could both spoof into a false "Sign out" state for a browser that was never signed in. Fixed by reading the account route's own envelope `error.code` instead of the status, and excluding `ACCOUNT_ROUTE_NOT_FOUND` specifically since it proves nothing about the gate either way; added regression tests for both spoofing cases.
- Finding via empirical proof, not just reasoning, across two rounds: a first attempt at loading `dotenv` (a plain `config()` call placed textually first in `server.ts`) did not actually fix the "local `.env` never loads" bug (Codex round 1). Building the real `server.mjs` and running it against a `.env` with only `STORY_LAB_AUTH_PROVIDER=clerk` set (no secret key) showed the process starting instead of crashing — proving ES module import-graph evaluation order (every import's own module-scope code runs before the importing file's own body, regardless of source position) made the fix a no-op for module-scope `process.env` reads. Fixed by switching to the side-effecting `import 'dotenv/config'` as the literal first import (`f9c8a35`), re-verified the same way. Codex round 4 then caught that this still resolved `.env` relative to `process.cwd()`, which is `story-generator/` rather than the repo root under this package's own `start:prod`/`serve:ssr:*` scripts — fixed by replacing it with `load-root-env.ts` (`fa6d1a4`), which resolves the path from its own compiled location (`import.meta.url`) instead, and re-verified with `story-generator/` as the working directory for both the pass and fail-fast cases. Recorded here per this plan's testing policy: "a mocked auth token cannot prove provider configuration" — both rounds required proof against the compiled artifact, not `tsx`.
- Round 6, caught by Codex round 6, fixed same round (`eefd278`): `cloudAccountAuthenticated` was only ever cleared by `refreshCloudLibrary` — a session that expired *after* the initial library check, and was instead caught by `saveActiveProjectToCloud`/`loadCloudProject`/`deleteCloudProject`'s own error handling, left the signal stuck `true`, so the reader kept seeing "Sign out" while every real request silently 401d underneath them. Factored the existing auth-signal logic into two shared helpers and wired them into all three operations, with a regression test. Same round: `cloudAccountStatusLabel` derived only from `cloudLibrarySyncState`, not the auth signal `cloudAccountActionLabel` already checked, so an authenticated reader hitting a storage outage read "Not connected" next to a "Sign out" button — fixed to check the auth signal first, matching the action label, with a regression test. A third round-6 finding narrowed the exec plan's "byte-for-byte unchanged" default-behavior claim, since `AuthService.initialize()` now always fires one `/api/health` request even when unconfigured.
- Non-claim, raised by Codex round 7: no session token refresh mechanism. Clerk's session JWTs are short-lived by design and normally kept fresh by the browser SDK's background polling, which this design deliberately omits (see the "considered and rejected `@clerk/clerk-js`" note above). Without it, an expired session isn't silently renewed — every account request 401s until the reader revisits the hosted portal. Round 6's own fix (above) already proves this degrades gracefully rather than getting stuck or spoofing a signed-in state, so not fixing it here is a scope decision, not an oversight: a real fix needs either the rejected SDK or a hand-rolled Clerk handshake this repo has no live credentials to verify safely. Documented in `auth.service.ts`'s doc comment, the README, and this plan's non-claims.
- Non-claim: no live sign-in proven against a real Clerk instance (no live credentials in this environment); no cloud project storage claim (`DATABASE_URL`/schema remain separate prerequisites, tracked in `STORY_LAB_STORAGE_PORT_EXEC_PLAN.md`); no session token refresh (see above).
...
- `ng test` (full karma suite): 265/265 passed (259 + 2 round-6 regression tests + pre-existing count correction).
...
- CI on PR #325: `Validate Vercel recovery build` and both SonarCloud checks green through round 6 (`eefd278`); round 7 CI pending at the time of this entry.
