...
- **Replaced generated fuzzing with exhaustive enumeration**, because the generator's blind spots are exactly what let the regression above through — it could not emit a quote inside an unquoted value followed by a `>`. Every tag interior of length 1..7 over `{a = " ' > space < /}` (2,396,744 inputs) and of length 1..8 over `{a = " > space}` (488,280 inputs, long enough to contain the shape Codex found): **0 cases where the new reader loses prose `main` keeps**, and on the second corpus all 192 differences are a strictly cleaner title.
- Seven counterfactual mutations applied, each confirmed failing a suite, then reverted; none committed. Two of the seven (the attribute separator, and the whitespace class) initially survived, so the tests were at fault and gained the inputs that discriminate.
