...
- Named the genesis stream's replay interval once and stopped its countdown finishing early. `/api/story-lab/stream/genesis` does not stream the generation — `generateStoryLabGenesis` is awaited whole — and then replays the finished chapters 500ms apart so the reader gets one at a time. That interval was written three times: the delay of chapter `n`'s timer, the `estimatedMsRemaining` those frames report, and the delay of the completion timer. The countdown was the copy that would be forgotten, since nothing in this repository reads it. It was also already wrong: it counted the chapters still to be replayed and stopped there, but the replay does not end on the last chapter — the payload the reader is waiting for goes out one interval later in the completion frame. So the final `chapter_progress` frame reported `0`, "arriving now", while the story it announces had not been sent. All three now derive from `CHAPTER_REPLAY_INTERVAL_MS` through one `replayFrameDelayMs`, and the estimate counts frames rather than chapters. The `90000` on the `connected` frame is named `ESTIMATED_MS_PER_GENERATED_CHAPTER` beside it, because it measures the model call rather than the replay and two bare numbers in one handler gave a reader no way to tell them apart.
- Added `tests/story-prompt-tables.test.ts`, registered in `test:all`, and extended `tests/story-content-analysis.test.ts` and `tests/story-lab-stream-genesis.test.ts`, both already registered.

Self-review:

- Both behavioural fixes were reproduced against the old code before being kept. Reverting `analyzeEmotionalTone` fails the new past-tense case with `past-tense prose should still read as passionate, got "angsty"` — the old scan found one of five registers in a chapter carrying all five. Reverting the genesis countdown to `totalChapters - frameIndex` fails with `a chapter frame should not report the story as already arrived, got 0`. Both mutations were reverted immediately and neither is committed.
...
- The genesis countdown is not read by anything in this repository — the Angular streaming panel derives its own estimate from `percentage` and a clock — so no screen changes. It is part of the published `StreamingProgressChunk` contract, which is why the off-by-one was fixed rather than the field removed.
- Validation: `npm run test:all` (60 suites) passes, `npx ng test --watch=false` passes (211 specs), and `scripts/recovery/preflight.sh --skip-status --skip-tests` completes — Angular app and spec type checks, the Vercel API type check, the Node 20 production build, and build-output verification. The `proving-grounds.css exceeded maximum budget` warning in that build is pre-existing and untouched by this slice. Root and Angular `npm install` were needed to run any of it; the root install dirtied the tracked `node_modules` tree and rewrote the `name` field of `package-lock.json`, and both were deliberately left unstaged.
