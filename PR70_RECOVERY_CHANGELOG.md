...
- **Two rounds, two data-loss regressions of the same family** (the pattern reading past the tag's real `>`). That is the non-convergence #296 predicted for a regex, and it is recorded here as evidence for that conclusion rather than against it. Stopping rule stated on the PR: if a third round produces another reshaped finding of this family, stop revising the pattern and recommend the tokenizer.
...
- Ten counterfactual mutations applied, each confirmed failing a suite, then reverted; none committed. Two initially survived and the tests gained inputs that discriminate. Two of the ten are the two shipped regressions themselves, and the exact pre-fix combination is pinned.
