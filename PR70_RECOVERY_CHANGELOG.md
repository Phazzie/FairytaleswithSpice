...
## 2026-09-09 UTC - Story Lab durable job store made its job-row and event-log writes atomic, closing #125 (PR #357)
...
Rewrote both operations as a single atomic `WITH ... AS (...)` CTE statement each — one Postgres statement is atomic by definition, so a failure and a partial write can no longer disagree, and no executor-interface or schema change was needed. This closes issue #125 (transaction-capable Postgres job mutations), scoped to the two writes these methods already made — not the broader transactional-outbox/idempotency/dispatcher work `STORY_LAB_LIVING_BOOK_AND_DURABLE_JOBS_EXEC_PLAN.md` Phase B and this plan's own Slice 6 still scope (#126, #131, #135 remain open).

Posted the plan to `#claude-routines` tagging Codex for critique first; no critique came back within the wait window, consistent with every prior run in this workspace. Codex's review of the pushed PR found three real issues across two review rounds, all fixed in the same PR:
