...
- Non-claim: **the account route file writes no log lines today**, so the id there is currently an echo and nothing more — the value a caller can quote for a request the platform's own logs recorded, not one this file files anything under. The settle call is deliberately not bound to a variable and says so, and the comment tells the first log line added to this file to take the id from there rather than reading the header again. Giving that route lines of its own (its store failures answer `503` and `403` silently) is a logging slice, not this one.
