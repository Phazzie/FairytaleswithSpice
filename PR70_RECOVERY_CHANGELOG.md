...
Validation: `npx tsc -p tsconfig.app.json --noEmit` / `tsconfig.spec.json --noEmit` clean; `ng build` clean; full Angular unit suite 335/335 (`story-lab-profile-panel.component.spec.ts` new, plus `app.spec.ts` additions for the Profile-button routing, the applied/not-applied default-seeding cases, and the unchanged pre-existing suite); `npm run test:all` clean (`story-lab-picker-vocabulary.test.ts` updated as noted above; `story-generator-app-line-budget.test.ts` still passes at 3,003/3,100 lines — net smaller than before this slice, since the extracted picker copy left `app.ts` more than the new methods added back); `git diff --check` clean.
