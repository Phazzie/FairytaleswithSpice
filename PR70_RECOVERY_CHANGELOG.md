...
- PR #206 checks: Validate Vercel recovery build passed and the Vercel preview deployed. SonarCloud's quality gate failed on `B Maintainability Rating on New Code` where the gate requires A; its analysis API is blocked by this environment's egress proxy and it left no inline comment, so the finding could not be read directly. A follow-up commit cleared every small readability point the new lines plausibly raise — a redundant `??` over an index TypeScript already types as non-nullable, a non-null assertion and a four-times-repeated literal in the auth test, `as const` tuples in the cliffhanger test, and a duplicated title literal in the export test.
