...
- The route-level redaction case drives `/api/story/stream` with an unrecognised theme and reads the entry back out of `logger.getRecentLogs`, so a route that went back to passing `input.themes` straight through would fail even though the helper-level assertions still passed.
