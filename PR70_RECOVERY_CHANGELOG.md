...
## 2026-09-09 UTC - Story Lab durable job store: job-row and event-log writes made atomic, addressing #125 (PR #357, in review)

**Status note:** as of this entry, PR #357 is in review and not yet merged to `main`. This entry describes what that PR does; #125 and the checklist items it addresses in `STORY_LAB_LIVING_BOOK_AND_DURABLE_JOBS_EXEC_PLAN.md`/`STORY_LAB_COMPLETION_HARDENING_EXEC_PLAN.md` stay open until it actually merges (Codex review finding on this PR itself — the first draft of those doc updates said "closed"/"merged" prematurely; corrected there and here).

`postgresStoryLabJobStore.ts`'s `createJob()` and `updateJob()` each wrote the job row (`INSERT`/`UPDATE story_lab_jobs`) and its append-only event-log row (`INSERT story_lab_job_events`) as two separate, un-transacted `query()` calls against the Neon HTTP executor, which has no cross-call transaction — each call commits independently. If the second write failed for any reason, the whole operation threw a generic storage error, which every route in `jobRouteHandlers.ts` turns into a client-facing 503 — but on the real payoff path (`finishJob()` completing a genesis or continuation generation), the job row had by then already been durably committed as `status: 'completed'` with the full, already-billed result. The client was told the write failed while the database said otherwise; a retry or a poll of `GET /jobs/{id}` would find the job already completed with a result the caller never received. `createJob()`'s equivalent failure instead left an orphaned job row.

Rewrites both operations as a single atomic `WITH ... AS (...)` CTE statement each — one Postgres statement is atomic by definition, so a failure can no longer mean a *partial* write, and no executor-interface or schema change was needed. This does not resolve a separate, pre-existing ambiguity Codex's review of this PR also raised: the Neon HTTP executor is a single request/response round trip, so a statement that commits on the server but whose response is lost afterward still surfaces as a failure to the caller — inherent to this driver, predating this PR, and applying equally to every other store built on `StoryLabCloudQueryExecutor`. Resolving that would need an idempotent outcome-reconciliation check, related to but broader than the createJob idempotency-key work tracked as #135, and is not attempted here. Once merged, this addresses issue #125 (transaction-capable Postgres job mutations) scoped specifically to eliminating a *partial* write between these two rows — not the broader transactional-outbox/idempotency/dispatcher work `STORY_LAB_LIVING_BOOK_AND_DURABLE_JOBS_EXEC_PLAN.md` Phase B and this plan's own Slice 6 still scope (#126, #131, #135 remain open).

Posted the plan to `#claude-routines` tagging Codex for critique first; no critique came back within the wait window, consistent with every prior run in this workspace. Codex's review of the pushed PR found five real issues across three review rounds, all addressed on the same PR:

- **P2, real — a job-ID conflict would have silently mutated an unrelated job.** The first draft gave `createJob`'s insert an `ON CONFLICT (job_id) DO UPDATE`, on the mistaken belief that retrying the whole atomic statement on an event sequence-number race needed the insert leg to be idempotent. It didn't — a failed CTE statement already rolls back its job insert along with everything else, since nothing survives a failed attempt for a retry to collide with — and the upsert instead meant a genuine `job_id` collision would silently attach a new caller's event onto a pre-existing, possibly different-owner job. Removed it; a duplicate `job_id` fails loudly again, as before this change.
- **P2, real (found in two passes) — recursive null-stripping, then unconditional nulls, in the built event.** Round 1: wrapping the whole built event object in `jsonb_strip_nulls` (meant only to drop an absent top-level `result`/`error` key) recursively stripped every `null` anywhere inside `result_json`'s own structure too, including required fields like `StoryIterationPayload.batch.stateDelta.fromRevision: number | null`. Round 2: removing the wrapper outright made `result`/`error` unconditionally present as an explicit JSON `null` instead, unlike the create-path and in-memory event shapes, which omit the key entirely when absent. Fixed by conditionally merging in each of `result`/`error` via `||` on two small `jsonb_build_object()`s, only when its column is not null — omitting the key again while leaving anything nested inside a *present* `result_json` untouched.
- Declined (with rationale posted on the PR, thread left open): a live-PostgreSQL integration test exercising the actual CTE. This repository has no existing live-database test harness anywhere (no `testcontainers`/embedded-Postgres dependency, no DB service in CI); adding one is a CI/infrastructure decision beyond this bug fix's scope. The atomicity guarantee itself rests on standard, documented single-statement Postgres semantics, not on anything this codebase's own SQL needs a live run to prove; the existing fake-executor tests verify the part that is this codebase's responsibility — that a permanent failure surfaces after exactly one write call, never a second one that could have partially applied.
- **P1, real — the code's own doc comment overclaimed what atomicity buys.** A doc comment said "the caller's failure and the database's state can no longer disagree," which is only true for the *partial*-write failure mode this PR actually fixes. It does not hold for a Neon HTTP `.query()` call whose response is lost after the server has already committed — a separate, pre-existing ambiguity inherent to this driver, unrelated to the two-writes bug, and not attempted here. Corrected the doc comment (and this entry, and the two exec-plan updates below) to state precisely what is and isn't guaranteed.
- **P1, real — the doc updates claimed this PR was already merged.** The first pass at updating `STORY_LAB_LIVING_BOOK_AND_DURABLE_JOBS_EXEC_PLAN.md` said PR #357 was "merged after this plan's last revision" while it was still in review; the completion plan and this changelog's own first draft of this entry likewise said #125 was "closed." Corrected all three to say "in review, not yet merged" and left the completion-plan checklist item unchecked, so a later reader (or this recovery process itself) does not treat a not-yet-landed guarantee as already in production.

Also updated `STORY_LAB_LIVING_BOOK_AND_DURABLE_JOBS_EXEC_PLAN.md`'s and this plan's own Slice 6/issue-list entries for #125 to reflect the above, with accurate in-review status.

Validation: `npm run test:all` (90 backend scripts) green; standalone `npx tsc --noEmit` clean on the changed file; SonarQube Quality Gate passed (0 new issues). This sandbox's Karma/Chrome runner hit the same pre-existing ~31s `ping timeout` disconnect documented on #348-#356 across three pushes on this PR, confirmed independently by re-running the identical check on `main`'s own tip (unrelated to this diff) before standing down on it once per this changelog's established practice.

## 2026-09-09 UTC - Story Lab batch generation stopped discarding already-generated chapters on a partial shortfall (PR #356)

**Correction to the 2026-05-27 22:52 EDT PR90 entry below:** that entry records "converted partial real-engine batch failures into explicit Story Lab errors instead of silently returning a shorter completed batch" as a fix. This PR reverses that behavior and the record needs to say so plainly, since the old line now describes a discard this codebase no longer performs.

`StoryService.generateChaptersForStory`/`continueChapter` already generate chapter-by-chapter and deliberately stop early (via `hasBudgetForAnotherChapter`) when the serverless time budget runs low — the expected path under real load — returning `success: true` with the chapters that did generate plus `partialFailures` naming the rest. `storyLabEngine.getPartialGenerationError` (now `getZeroChapterGenerationError`) discarded all of that on any shortfall, before continuity extraction or persistence ever ran: a 5-chapter batch that generated 4 real, already-billed chapters and timed out on #5 returned nothing. The guard now only hard-fails when zero chapters generated; any shortfall above zero keeps its chapters, runs continuity extraction/persistence as normal, and reports `partialFailures` on the batch. `ChapterBatchEnvelope` gained the field; `BatchProgressStatus` gained `'partial'`; the UI shows a warning instead of an unqualified success.

Codex's review of the initial push (`bea35d4`) found two more real issues, both fixed in the same PR:

- **P1, real — a non-contiguous chapter sequence could reach the reader.** Neither generation loop's per-chapter `catch` block stopped the loop: on a mid-batch chapter error it recorded the failure and moved on to the *next* chapter number, which is generated from `aggregatedRawHtml` (only ever the chapters that actually succeeded) — so chapters 1 and 3 could both generate with chapter 2 missing, numbered as if nothing were wrong, and the next continuation would resume from 4 with no way to ever fill the gap. Discarding the whole batch on any shortfall had been silently masking this every time, since a batch with a gap never got the chance to be checked for contiguity — it was already gone. Both catch blocks now `break` immediately after recording the failure (mirroring the existing time-budget `break` just above them) and mark every later requested chapter number skipped, so only a contiguous run starting at 1 (or at the continuation's starting chapter) is ever kept.
- **P2, real — a malformed `partialFailures` value could crash the job-completion handler.** `hasRenderableIterationPayload` validated `batch.chapters` and `summary.storyId` but not the new `batch.partialFailures` field it now also dereferences (via `describeBatchCompletionNotice`'s `.map()`), so a non-array value with a truthy `.length` from a stale stored job row would throw inside the job event stream's callback — the exact "stuck in progress forever" failure mode that guard exists to prevent. Added `isValidPartialFailures()` (`batch-progress.ts`) and folded it into the same guard.

Validation: `npm run test:all` (90 backend scripts) green; `npx tsc --noEmit` clean on both `tsconfig.app.json`/`tsconfig.spec.json`; `story-generator-app-line-budget.test.ts` passes at 3,096/3,100. This sandbox's Karma/Chrome runner hit the same pre-existing ~31s `ping timeout` disconnect documented on #348–#353 (confirmed not diff-specific: this exact CI check shows `conclusion: failure` on the last 5 pushes to `main` itself) — every spec that executed passed with zero failures across two full local runs (146/148, 100/148) and one CI run (361/362) before each disconnect, and the new/changed specs were additionally isolated with Jasmine `fit()` and run alone to a clean pass.

## 2026-09-07 UTC - Post-merge Codex round found 4 more races in #342's profile-defaults sign-in path

A third Codex review round landed after PR #342 already merged (`d6bedb0`), too late to fold into that PR, finding 4 real bugs in the sign-in-time profile-defaults fetch (`App.syncStoryLabProfileDefaultsWithAuthState`) and the panel's own save/load paths — all stemming from that fetch never getting the same request-identity rigor `CloudLibraryService` and the panel's own guards already had. Per this changelog's own established practice of not reopening a merged PR, the fix shipped as a new commit on the same branch/PR history rather than amending #342.

Per explicit instruction, the fix was planned first, independently adversarially critiqued and red-teamed by two separate subagents before any code was touched, then implemented incorporating both passes' findings, then independently reviewed by a third subagent against the actual committed diff:

- **Real — no staleness guard on the sign-in-time fetch.** A slow response from one account could apply its defaults (including private no-go-content notes) onto whichever account is signed in by the time it resolves. Fixed by capturing `sessionEpoch` before the request and, on a stale response, retrying under the current identity rather than discarding — safe here since it's a GET run at most once per account, unlike a PUT.
- **Real — `storyLabProfileDefaultsFetchedForAccountId` was never cleared on sign-out.** Signing back in as the same account skipped the fetch entirely for the rest of the session. Now reset whenever `signedIn` is false.
- **Real — a same-account token refresh mid-save could permanently lock the profile panel.** The stale-response early-return in `save()` skipped resetting `isSaving`, leaving the panel's fields and Save button disabled forever. Now released unconditionally, with an explicit "session changed, please review and save again" message rather than silently discarding a response that might have actually succeeded (blind-retrying a PUT isn't safe the way retrying a GET is). `loadProfile()` had the identical bug shape, stuck permanently on `loadState() === 'loading'`; fixed the same way there via retry, since a GET is safe to repeat.
- **Real — sign-in-time and save-time defaults could silently overwrite a creature/tone/heat-contract the reader had already picked**, even without a story started yet, since the existing guard only checked `workbench().story`. Added a permanent `blueprintTouchedByReader` latch, set by every blueprint-mutating entry point (`updateBlueprint`/`updateHeatContract`/`toggleTheme`), checked by both defaults-application call sites alongside the existing guard.
- **Real, minor — the sign-in-time fetch had no error handler**, so a failed request would escape to RxJS's global unhandled-error path instead of failing quietly like the rest of this best-effort background sync. Added one; left empty since `StoryService.handleHttpError` already logs the failure, avoiding double-logging.

The independent review subagent confirmed all fixes above were implemented correctly, with one blocking gap: the new retry-on-stale branch in `syncStoryLabProfileDefaultsWithAuthState` had no test exercising it. Added `describe('App sign-in-time profile-defaults staleness', ...)` to `app.spec.ts`, with a fully-controllable fake `AuthService` (a real `signal()` for `sessionEpoch`, so a test can advance it mid-flight) and two RxJS `Subject`s standing in for the first and retried `getStoryLabProfile()` calls — asserting the first (now-stale) response's `creature` is discarded, a second request fires under the current identity, and only the second response's value lands on the blueprint.

Self-review:

- Good: the retry-vs-discard split (GET retries, PUT discards with a message) matches the same tradeoff `CloudLibraryService` already made for its own guards, rather than inventing a third pattern for this fetch.
- Non-claim: this does not close the cross-account edge case `blueprintTouchedByReader`'s own doc comment already disclaims — it is a permanent latch, not scoped to account or story, by design.

Validation: `npx tsc -p tsconfig.app.json --noEmit` / `tsconfig.spec.json --noEmit` clean; `ng build` clean (pre-existing bundle/CSS budget warnings only, unrelated); `npm run test:all` clean; `git diff --check` clean; `story-generator-app-line-budget.test.ts` still passes (`app.ts` at 3,092/3,100 lines). This sandbox's Karma/Chrome runner disconnected on a `ping timeout` at a consistent ~31-second wall-clock mark on every full-suite attempt, independent of which test Jasmine's randomized order placed last, which behavior points at an environment/container characteristic rather than a test defect — not a full clean run, so recorded as a non-claim rather than a pass. Every test that did execute before each disconnect passed with zero failures across multiple attempts (144/145 running only `app.spec.ts`; 126/159 running it together with `story-lab-profile-panel.component.spec.ts`), including the new staleness/retry test and the full pre-existing suite, and the two affected spec files were each run to a clean 100% completion in isolation (`story-lab-profile-panel.component.spec.ts` 14/14; both together not yet achieved end-to-end in this environment).

## 2026-09-06 UTC - Proving Grounds' own default configuration could never generate a story (PR #345)

Proving Grounds (`story-generator/src/app/proving-grounds/*`), the prompt A/B-testing tool, packed the selected template's system+user prompt into `narrativeDirectives` — capped at 1,200 characters by `STORY_BLUEPRINT_LIMITS` — for every template, including "Current Production." That template's real system+user prompt runs to about 10,500 characters, so the page's own default configuration hit the cap and refused to generate on every fresh load. The prompt "under test" was also never actually used as a system prompt: it was flattened into one bullet inside the Story Lab context block while the real production system prompt ran unconditionally server-side regardless, so even a template that fit wasn't testing what the UI claimed. An earlier run's sweep had listed this page as "already well-covered" without digging in; the one prior fix here (`describeNarrativeDirectivesOverflow`) only made the refusal arrive with a friendlier message, and its own docblock documented the defect rather than fixing it.

The unmodified "Current Production" template now sends no `narrativeDirectives` override at all — a request with no override *is* what "current production" means, since that prompt already runs unconditionally server-side, so it can actually generate. Experimental templates (and "Current Production" once the reader edits its own text, since that's what turns a baseline into a variant) send only `system + user` through `narrativeDirectives`, dropping the redundant `'PROVING GROUNDS TEST'` label and a generation-logic summary the server was never going to honor anyway. A live character-count warning now disables Generate before submission instead of only failing after it. Replaced `globalThis.alert()` in `viewPrompts()` with an in-page preview panel, relabeled the Generation Logic panel's hint to stop implying it affects the actual run, and routed the page's `console.error` calls through the existing `ErrorLoggingService`.
...
Codex's review of the initial push (`06f872e`) found four issues (two P1, two P2), all fixed in the same slice:

- **P1, real — a stored result's `promptPreview` misattributed generated prose to a prompt the model never received.** For the unmodified "Current Production" baseline, `createTestResult()` still stored the client's independently-filled (and randomly Chekhov-drawn) prompt as `configuration.promptPreview`, even though that exact text is now never sent — the server's own `buildProductionSystemPrompt()` ran instead, with its own independently-drawn author styles, beat structure, and Chekhov ledger. Exported/compared baseline results would show a specific prompt as "what generated this," when it wasn't. Rather than having the API return the resolved prompt it actually used (a backend seam this PR deliberately avoided adding, for the same content-safety-review reason it avoided a real system-prompt override), `createTestResult()` now records a `BASELINE_PROMPT_PREVIEW_NOTICE` descriptor for the baseline case instead of the unused sampled prompt. `viewPrompts()`'s live preview panel is unaffected — it's answering a different question ("what would this look like"), not attributing a specific result to it.
- **P1, docs — this changelog entry itself was the required update.** Recorded here, in the same slice.
- **P2, real — the live overflow check and the actual request could draw two different, disagreeing Chekhov ledgers.** `narrativeDirectivesOverflowMessage()` (the Generate button's `[disabled]` and the overflow banner) and `generateStory()`'s own guard each called `getFilledPrompts()` independently, which draws a fresh random Chekhov ledger for "Current Production" every call. For a custom-edited near-the-cap variant that still contained the ledger token, the button's validation could disagree with the request `generateStory()` actually built — a false-disable that never clears, or a false-enable whose click gets locally rejected. Replaced the two independent call sites with `resolveCurrentRequest()`, which computes `{ prompts, directives, overflow }` once per distinct configuration (memoized by a signature of every field that affects it) and is what both the live UI and `generateStory()` now read — so the button always validates the exact request it's about to send. The cache is cleared once a real generation is sent, so the next click still draws its own fresh ledger, preserving the "each real run plants its own elements" behavior `CHEKHOV_LEDGER_TOKEN` documents.
- **P2, real — the click-to-dismiss backdrop and the panel it wraps had no keyboard equivalent, and opening the panel never moved focus into it.** `aria-modal="true"` alone doesn't move focus, trap it, or support Escape (SonarCloud's Quality Gate independently failed "B Reliability Rating on New Code" on the same two `<div>`s, for the same underlying gap). Rather than inventing a second focus-trap implementation, matched `StoryLabProfilePanelComponent`'s existing one exactly: `role="presentation"` backdrop, a `tabindex="-1"` `<section role="dialog">` focused after `viewPrompts()` sets the panel signal, `onPromptPreviewKeydown`'s Tab-wrap trap and Escape-to-close, and focus restored to whatever was focused before `viewPrompts()` opened it. (An `effect()` watching the panel signal was tried first and dropped: it ran before Angular refreshed the `@if`-conditional `ViewChild` query against the newly-created DOM, so the panel reference was still `undefined` at the moment it fired — confirmed with a temporary debug log before switching to a `setTimeout` queued right after the signal is set, which reliably runs after that render.)

Codex's re-review of that push (`5315afd`) found three more issues (one P1, two P2), all fixed in the same slice:

- **P1, real — a mock generation was stored and compared exactly like a real one.** When no model provider is configured, `/api/story-lab/stories` answers `buildGenesisResponse()`'s canned mock chapters (`GenerationTelemetry.engine: 'custom'`), which ignore whatever prompt was actually under test. `createTestResult()` discarded that telemetry and stored the result as an ordinary run, so it could be evaluated and placed in a side-by-side comparison as if a real generation had produced it. Added `isMockGeneration` to `ProvingGroundsTestResult` (`engine === 'custom'`), a status message on generation ("Generated with offline mock chapters — not real AI output...") when it's true, and a badge everywhere a result is shown — the current-test view, comparison cards, and a compact `🎭 mock` chip in history — mirroring the existing `aiEvaluation.isMockEvaluation` badge pattern rather than inventing new wording for the same kind of warning.
- **P2, real — a failed generation was logged twice.** `StoryService.beginStory()`'s `handleHttpError` already logs an HTTP failure through `ErrorLoggingService` before rethrowing it; the component's own `error` callback logged the same error again (a holdover from routing what used to be a plain `console.error` through the shared service — correct for the page's other three `console.error` call sites, which had no such upstream logger, and wrong for this one). Removed the component's duplicate call; the service's own log is now the only one, so the Debug Errors panel's ten-entry display no longer fills twice as fast on repeated failures.
- **P2, real — the overflow warning wasn't announced to assistive technology.** The warning appears earlier in the DOM than the field a screen-reader user was just editing, and Generate silently becomes disabled with nothing connecting the two. Added `role="alert"` to the warning `<p>` and wired `aria-describedby` from the Generate button and both custom-prompt textareas to its `id`, so its text is both announced when it appears and read as part of why the disabled control and the fields under edit are what they are.

Validation: `ng test` full Angular suite green (350/350, including new specs for the baseline-sends-no-override, experimental-payload, custom-prompt-overflow, prompt-preview-panel, focus-trap, mock-generation, and no-duplicate-logging behaviors); `npm run test:all` (backend/shared) green, including `test:proving-grounds-production-prompt`; `ng build --configuration production` compiles clean.

SonarCloud's Quality Gate failed again on that push (still "B Reliability Rating on New Code") — reverting the panel from a native `<dialog>` to `role="dialog"` on a `<section>`, to match `StoryLabProfilePanelComponent`'s convention for Codex's focus-trap finding, reintroduced the exact two findings the first SonarCloud fix had resolved (the backdrop `<div>`'s `onClick` with no `onKeyDown`, and `role="dialog"` instead of the real element) — matching an existing pattern elsewhere in the codebase doesn't exempt *new* code from the same rule, since SonarCloud scores only the diff. Also flagged: a nested ternary in the mock-generation status message, and a generic `.length` assertion in the new duplicate-logging test. Fixed in the same slice: switched back to a native `<dialog>` while keeping the exact same focus-on-open, Tab-wrap trap, Escape-to-close, and focus-restore behavior (`onPromptPreviewKeydown` unchanged, now bound to the `<dialog>` instead of a `<section>`); added a `(keydown.escape)` to the backdrop `<div>` alongside its existing `(click)`; extracted the nested ternary into a `chapterWord` local; and changed the test assertion to Jasmine's `toHaveSize`. Codex's two asks (real keyboard focus management) and SonarCloud's two asks (a real `<dialog>`, every `onClick` paired with a keyboard equivalent) turned out to be fully compatible — the native element was never the obstacle to the focus-trap; assuming otherwise cost one extra round trip.

Validation (final): `ng test` full Angular suite green (350/350); `npm run test:all` green; `ng build --configuration production` compiles clean.
...
Added a `STORY_LAB_CLOUD_STORAGE` env-driven mode switch to `createStoryLabCloudStorage`, reusing `durableStoreEnvResolution.ts` like its siblings, wiring the previously-dead in-memory stores in as the `non_durable_memory` mode. Unlike the siblings — whose *only* prior implementation was in-memory, so defaulting to memory there was zero-risk — this store's only prior implementation was Postgres, implicitly selected whenever `DATABASE_URL` was set with no way to opt out. Defaulting the mode to `non_durable_memory` here (matching the siblings' convention) would have silently downgraded any deployment that already relies on `DATABASE_URL` to ephemeral, per-instance storage the moment this shipped. So the default stays `postgres` — byte-for-byte the same behavior as before when `DATABASE_URL` is set, and, for a deployment with no `DATABASE_URL`, now correctly reported as `degraded` by `/api/health` instead of silently erroring on every request with no operator-visible signal. `STORY_LAB_CLOUD_STORAGE=non_durable_memory` opts a deployment into the non-durable stores explicitly, the same way `RATE_LIMIT_STORE=postgres` opts the rate limiter into durability in the other direction. Updated the five call sites (`accountRouteHandlers.ts`, `jobRouteHandlers.ts` ×2, `stories.ts`, `continue.ts`) to match the config's widened return shape (`mode`/`requestedMode`/`errorCode`, mirroring the sibling configs) — a drop-in change since each only reads `.profileStore`/`.projectStore`/`.isConfigured()`. Added `services.storyLabCloudStorage: { mode, configured }` to `/api/health`, feeding the existing `isServiceDependencyDegraded` check the same way the other two stores already do.

Posted to `#claude-routines` for critique before implementing, specifically flagging the postgres-vs-non_durable_memory default choice for a second opinion; no response arrived in the review window, so it shipped as planned.

Codex's review of the initial push (`2949548`) found four issues (four P1), three fixed in the same slice:

- **P1, real — every one of the 5 call sites created a fresh in-memory `Map` on each `createStoryLabCloudStorage()` call.** In `non_durable_memory` mode, the account handler, job handler, genesis handler, and continuation handler each got their own store instance, so a profile saved through `/api/story-lab/account/profile` was invisible to the generation routes — the exact "claims a capability with no working code path" defect this routine exists to catch, reintroduced by the fix itself. Added module-level `sharedNonDurableStoryLabProfileStore`/`sharedNonDurableStoryProjectStore` singletons, mirroring `sharedInMemoryRateLimitStore` in `rateLimitStoreConfig.ts` and the exported `nonDurableStoryLabJobStore` singleton in `jobStore.ts`. Still process-local only (not shared across separate Vercel function invocations), identical to and no worse than the accepted limitation on the other two non-durable defaults.
- **P1, real — the new `STORY_LAB_CLOUD_STORAGE` variable was undocumented.** An unset default now degrades `/api/health` without `DATABASE_URL`, but README's Environment Setup never mentioned the variable and its Health Check section still attributed 503s to only the other two stores. Added the variable to Environment Setup (with the same default-preserves-behavior rationale as this changelog entry) and widened the Health Check section's wording to name Story Lab cloud storage and its postgres-by-default exception explicitly.
- **P1, real — several unrelated health-route tests didn't isolate cloud storage's new default.** `testDegradedOnUnsupported{RateLimit,Job}StoreMode`, `testDegradedOnUnreachablePostgres{RateLimit,Job}Store`, and `testCriticalAlertingDegradedOnMalformedWebhookUrl` asserted `degraded`/`503` without pinning `STORY_LAB_CLOUD_STORAGE=non_durable_memory`, so each would keep passing even if the assertion's actual target were accidentally removed from `isServiceDependencyDegraded` — cloud storage's own default-postgres degradation would mask the regression. Pinned all five.
- **P1, declined with rationale — "verify database readiness before reporting healthy."** Real in the sense that a syntactically valid `DATABASE_URL` only proves an executor was constructed, not that the database is reachable or schema-migrated — but this is the exact same shallow `isConfigured()` check (`databaseUrl && executor`, no live query) already shared by `postgresRateLimitStore.ts` and `postgresStoryLabJobStore.ts`, and `/api/health`'s own top-of-file doc comment states this is deliberate ("without making a live network call"). Fixing it only for cloud storage would be inconsistent with its two siblings; fixing it for all three would be widening this PR into a live-health-check redesign well beyond its scope. Replied on the PR thread rather than pushing a fix.

Validation: extended `tests/story-lab-cloud-storage-config.test.ts` (+4 cases: default mode stays `postgres`, explicit `non_durable_memory` opt-in round-trips profile/project saves in-process, an unsupported mode value fails closed with its error code, and the new env var doesn't fall through to `process.env` — all 5 pre-existing cases pass unchanged, confirming the default is byte-for-byte the prior behavior); extended `tests/health-route.test.ts` (+4 cases: unconfigured-by-default reports `degraded`/`503`, an unsupported mode reports `degraded`, an explicit `non_durable_memory` opt-in reports `healthy`, a reachable `postgres` store reports `healthy`; the 5 pre-existing "healthy" cases were updated to opt into `non_durable_memory` explicitly, since their premise — an unconfigured deployment being healthy — no longer holds for this store by design); fixed one incidental break in `tests/story-lab-account-routes.test.ts` (`testHealthReportsTheOriginTheCorsPolicyResolved` called the real health handler without pinning this store's mode). Full `npm run test:all` clean; API typecheck (`tsc --noEmit` over `api/**/*.ts`, the exact preflight invocation) clean; `git diff --check` clean.

Codex's re-review of that push (`98da642`) found three more issues (two P1, one P2), all fixed in the same slice:

- **P1, real, and the deepest one — `non_durable_memory` reporting `healthy` contradicted the app's own existing UI.** `CloudLibraryService` (`story-generator/src/app/cloud-library.service.ts:172-177`) already treats a `storageMode: 'non_durable_memory'` response as `cloud_unavailable` — read-only-for-inspection, save/load/delete disabled — a defensive check written for this exact mode back when the Phase C storage-port scaffold was built, years before this PR ever wired a real caller to it. So `/api/health` reporting `healthy` for that mode would have been the identical "claims a capability with no working code path" defect this whole routine exists to catch, one layer deeper than the fix itself. Unlike the rate-limit and job-store non-durable defaults — which keep doing their actual job, just without cross-instance sharing — cloud storage's non-durable mode means the signed-in library itself doesn't work for its purpose. Added `isStoryLabCloudStorageDegraded` (`api/health.ts`), a store-specific check requiring a reachable `postgres` store for `healthy`; `non_durable_memory` and `unsupported` both now degrade it. Rewrote `tests/health-route.test.ts`'s cloud-storage-healthy cases accordingly (only a reachable `postgres` config reaches overall `healthy` now) and reconciled every other test's now-required cloud-storage pin to a valid `postgres`+`DATABASE_URL` config instead of `non_durable_memory` (two tests — the rate-limit/job-store unreachable-postgres cases — can't fully isolate cloud storage this way since all three configs share one `DATABASE_URL` env var; left a comment explaining why their direct per-service field assertions, not the overall status, are the real regression guard there). Updated README's Environment Setup and Health Check sections and `STORY_LAB_AUTH_PROFILE_CLOUD_LIBRARY_EXEC_PLAN.md` (the active plan, not just the historical storage-port one) to state plainly that `non_durable_memory` is a local/test convenience, not a supported production fallback.
- **P2, real — the shared singleton silently ignored an injected clock.** `createStoryLabAccountRouteHandler({ now })` forwards a custom clock into `createStoryLabCloudStorage`, honored per-call on the Postgres branch, but the new module-level singletons were constructed once with the real clock regardless — deterministic callers (tests) would get mixed real/injected timestamps. Fixed: an explicit `options.now` now builds its own isolated stores instead of reusing the singleton (matching the rest of this config's per-call clock behavior); the shared singleton is still what every real, clock-less caller gets. Added `testNonDurableMemorySharesStoreAcrossSeparateCalls` to `tests/story-lab-cloud-storage-config.test.ts` to prove the singleton-sharing fix directly (two independent `createStoryLabCloudStorage()` calls, no `now` override, one call's save visible to the other's load) now that the existing clock-injecting test no longer exercises it.
- **P1, docs — the active auth/profile/cloud-library plan didn't record the new switch.** `STORY_LAB_AUTH_PROFILE_CLOUD_LIBRARY_EXEC_PLAN.md`'s "Current Reality" list is this repo's source of truth for signed-in profile/project behavior; only the historical, already-closed `STORY_LAB_STORAGE_PORT_EXEC_PLAN.md` had been updated. Added a bullet there too, cross-referencing this changelog entry.

Validation (final): `npx tsx tests/story-lab-cloud-storage-config.test.ts` and `npx tsx tests/health-route.test.ts` both green with the new/rewritten cases; full `npm run test:all` clean; API typecheck clean; `check-vercel-function-count.sh` 9/12, unchanged; `git diff --check` clean.

Codex's third review of that push (`cf2e6fd`) found one more issue (P2), fixed in the same slice — and this one caught a real regression in the P2 fix immediately above:

- **P2, real, and a direct regression from the previous round's clock fix — the previous round's "honor an injected clock" fix broke sharing for the one route that matters most.** `createStoryLabAccountRouteHandler()` (`accountRouteHandlers.ts:103-104`) always constructs its own `now` closure and passes it to `createStoryLabCloudStorage({ now })` — even when its own caller supplied none — while every other route (`jobRouteHandlers.ts`, `stories.ts`, `continue.ts`) calls it bare, with no `now` at all. The previous round's fix branched on "was `now` provided," so the account route — the one route that actually saves profiles — always took the newly-added isolated-store path, while every other route kept the shared singleton. A profile saved through `/api/story-lab/account/profile` would load as `null` from every generation route in the same process, silently dropping its content boundaries — reintroducing this PR's own original defect, one review round after fixing it. Reverted the branch: the `non_durable_memory` stores now always use the shared singleton regardless of `options.now`, deliberately matching `storyLabJobStoreConfig.ts`'s own `nonDurableStoryLabJobStore` singleton, which also ignores any `now` option. A caller that genuinely wants a controlled clock for this mode now has to ask for it explicitly via `nonDurableProfileStore`/`nonDurableProjectStore` — the interface already supported this; `testNonDurableMemoryModeUsesInMemoryStores` was rewritten to use it instead of relying on `now` alone. Added `testAccountHandlerShapedCallStillSharesTheSingleton`, which reproduces the account handler's exact call shape (`{ now: () => new Date().toISOString() }`) and proves it still shares state with a bare call — confirmed this test actually fails against the previous round's code before restoring the fix, and passes after.

Validation (final): all three `story-lab-cloud-storage-config.test.ts` sharing/clock tests green, including the new one confirmed to fail against the prior round's code; full `npm run test:all` clean; API typecheck clean; `check-vercel-function-count.sh` 9/12, unchanged; `git diff --check` clean.

Codex's fourth review of that push (`d69c704`) found the "always ignore `options.now`" workaround from the round above traded one real problem for the original one back — a genuinely deterministic caller (a test injecting `now` through `createStoryLabAccountRouteHandler({ now })`) once again got mixed real/injected timestamps, since the singleton no longer honored `now` at all. Rather than choosing between the two rounds' fixes, found and fixed the actual root cause both were dancing around:

- **P2, real, root cause — `createStoryLabAccountRouteHandler()` defaulted `now` *before* deciding whether to pass it through, conflating "no override" with "has a value."** `const now = dependencies.now ?? (() => new Date().toISOString()); createStoryLabCloudStorage({ now })` meant `createStoryLabCloudStorage` always received a real function, whether or not its own caller supplied one — there was never a way for it to distinguish "the account route's own default" from "a test's genuinely explicit clock." Fixed at the source: `accountRouteHandlers.ts` now passes `createStoryLabCloudStorage({ now: dependencies.now, env: dependencies.env })` — `dependencies.now` directly, undefined unless the account handler's *own* caller supplied one — while the always-defaulted closure is kept, unchanged, for `context.now` (used elsewhere in the route, unrelated to cloud storage construction). This let `storyLabCloudStorageConfig.ts` go back to branching on `options.now` (restored from the second round, effectively reverting the third round's blanket-ignore workaround): the default, `now`-less path (every real route, account route included) shares the singleton; an explicitly-injected `now` builds its own isolated, deterministically-clocked store — matching the same tradeoff as passing `nonDurableProfileStore`/`nonDurableProjectStore` directly, and honoring the Postgres branch's own per-call clock behavior. Also found and fixed a second, previously-latent instance of the identical defaulting bug: `dependencies.env` was never forwarded to `createStoryLabCloudStorage` at all (`env: dependencies.env` was missing even before this PR touched the file), so a caller supplying a custom `env` to control this route's other behavior had no way to make cloud storage's own mode resolution honor it either — harmless in production (nobody constructs this route with a custom `env`) but it directly blocked writing a real regression test for the `now` fix above, which is how it was found.
- Replaced `testAccountHandlerShapedCallStillSharesTheSingleton` (which hand-simulated the account handler's call shape inside `story-lab-cloud-storage-config.test.ts`, and silently stopped matching the real code once that shape changed — exactly what happened in this round) with `testDefaultAccountHandlerSharesNonDurableStorageWithGenerationRoutes` in `tests/story-lab-account-routes.test.ts`: it builds the real, unmodified `createStoryLabAccountRouteHandler()` (no `profileStore`/`projectStore` override) against `STORY_LAB_CLOUD_STORAGE=non_durable_memory`, saves a profile through it, and checks a separate bare `createStoryLabCloudStorage()` call (as a generation route makes) sees it — immune to the exact kind of call-shape drift that broke the previous test. Confirmed this test fails against the pre-fix code (by temporarily reverting just the singleton branch) and passes with the fix, the same way the previous round's own regression test was verified. Added `testExplicitNowIsolatesFromTheSharedSingleton` to `story-lab-cloud-storage-config.test.ts`, proving the other half of the contract: an explicit-clock caller must *not* see the shared singleton's records either.

Validation (final): `testDefaultAccountHandlerSharesNonDurableStorageWithGenerationRoutes` confirmed to fail against the pre-fix code and pass after; all `story-lab-cloud-storage-config.test.ts` and `story-lab-account-routes.test.ts` cases green; full `npm run test:all` clean; API typecheck clean; `check-vercel-function-count.sh` 9/12, unchanged; `git diff --check` clean.

## 2026-09-06 UTC - The Story Lab profile had a validated, tested backend and zero frontend UI to ever set it (PR #342)

`StoryLabUserProfile` (favorite creatures/tones, a default heat contract, content boundaries, library sort) has had a full backend since it was built — Postgres + in-memory stores, field-size validation — with `StoryService.getStoryLabProfile()`/`updateStoryLabProfile()` already on the Angular service. Both methods were called only from their own spec file: no settings screen, no form, no route to it anywhere in the frontend. It wasn't just inert — `accountRouteHandlers.ts`'s `readLibrarySort()` already read `preferences.librarySort` off the stored profile to order every Cloud Library list, so every signed-in user was permanently stuck on `updated_desc` with no way to know a sort preference existed, and the one UI element with the word "Profile" on it (`cloudAccountActionLabel()` → `'Profile'` when `cloud_synced`) was wired to `showCloudAccountSetupStatus()`, which only popped an "Account connected." toast.

Extracted the creature/tone/heat-contract picker copy (`creatureOptions`, `toneOptions`, `heatTensionOptions`, `heatBoundaryOptions`) out of `app.ts` into a new `story-lab-option-copy.ts`, so a second screen could read the same vocabulary tables instead of restating them a fourth time (`story-lab-picker-vocabulary.test.ts` was updated to read both files as one source for its "the form should build its picker from `CREATURE_ARCHETYPES`" assertions, since the literal mentions moved). Added `StoryLabProfilePanelComponent` — its own `.ts`/`.html`/`.css`/spec, not added to `app.ts`/`app.html` directly, since the previous slice (#340) was specifically about `App` regrowing past its own god-component fix. Rewired the "Profile" action to open the panel in `cloud_synced` mode only, leaving every other sync mode's behavior unchanged, and refresh the cloud library on save so a changed `librarySort` is visible immediately.

Posted to `#claude-routines` for critique before implementing; no response arrived in the review window, so it shipped as planned.

Codex's review of the initial push (`cbcd2e8`) found five issues (three P1, two P2), all fixed in the same slice:

- **P1, real — the saved "default heat contract" and favorite creature/tone were still never read anywhere.** This changelog's own prior entry (below) already recorded `StoryLabProfilePreferences.defaultHeatContract`, `favoriteCreatures`, and `favoriteTones` as "stored, validated, redacted and persisted with no reader in either tree," noting that applying or retiring them was "a feature, not a cleanup." Making them editable without making them apply would have been the same false affordance the "Profile" button itself used to be — a screen presenting values as your "defaults" that influence nothing. Added `App.applyStoryLabProfileDefaults()`, called from `onStoryLabProfileSaved()`: seeds `blueprint().creature`/`.tone`/`.heatContract` from the just-saved profile, but only while `workbench().story` is still empty — a story already in progress is never silently rewritten out from under the reader. `contentBoundaries` needed no equivalent fix: `api/_lib/story-lab/contentBoundaries.ts`'s `loadAuthenticatedContentBoundaries` already merges it server-side into every genesis request (the #329 fix), which Codex's finding conflated with a nonexistent frontend function of the same description.
- **P1, real — a profile response could populate the panel under an account that had since signed out or switched.** `CloudLibraryService` guards every one of its own requests against `AuthService.sessionEpoch()` advancing mid-flight, for exactly this reason (see its own extensive doc comments); the new panel had none of that. Added the same guard directly against `AuthService.sessionEpoch()`: both `loadProfile()` and `save()` capture the epoch before the request and discard the response if it has since changed, and a constructor `effect()` closes the panel the moment the epoch moves while it's open — so one account's profile, including its private "no-go content" notes, can never render under another.
- **P2, real — the panel stayed fully interactive while a save was in flight.** Only the Save/Cancel buttons were disabled; every field, the header Close button, and the backdrop remained clickable, so a slow save could be edited out from under itself or dismissed mid-flight with its response never handled. Added `[disabled]="isSaving()"` to every `<fieldset>` (natively disables all descendant controls) and the header Close button, and `close()`/`dismissBackdrop()` now no-op while saving.
- **P2, real — the dialog had no real keyboard focus behavior.** `aria-modal="true"` alone does not move focus, trap it, or support Escape. Added focus-on-open (`ngAfterViewInit` focuses the panel), a minimal Tab-wrap focus trap and Escape-to-close (`onDialogKeydown`), and focus restoration to whatever was focused before the panel opened.
- **P1, docs — this changelog entry itself was the required update.** Recorded here, in the same slice.

Separately, the `handleCloudAccountAction()` this push added called `showCloudAccountSetupStatus()` from a template `(click)` binding without awaiting or handling its returned promise, which failed SonarCloud's "C Reliability Rating on New Code" gate. Changed the method's return type to `void` and marked the ignored promise explicitly with `void` rather than returning it to a template that was never going to await it either way — the same fire-and-forget shape as the binding it replaced, which predates this PR's "new code" window and so was never gated.

Codex's re-review of that push (`8fe600a`) found four more issues (two P1, two P2), all fixed in the same slice:

- **P1, real — the applied defaults covered only the moment of saving.** `applyStoryLabProfileDefaults` (added for the finding above) only ran from `onStoryLabProfileSaved`, so a returning signed-in user's saved defaults were never loaded on a fresh visit — only if they reopened the profile panel and saved again in the same session. Added `App.syncStoryLabProfileDefaultsWithAuthState`, called from a new constructor effect alongside the existing `cloudLibrary.syncWithAuthState` one: fetches the profile at most once per signed-in account (a plain `storyLabProfileDefaultsFetchedForAccountId` field, not a signal, for the same reason `CloudLibraryService.syncWithAuthState`'s own `wasSignedIn` is a plain field) and applies it the same way, silently, on failure.
- **P1, real — the account-identity auto-close effect and the save-in-flight lock fought each other.** The constructor effect closing the panel on identity change called `close()`, which refuses to run while `isSaving()` — exactly the moment the close is most needed, since the alternative is the outgoing account's private profile staying rendered, locked open, under the incoming account for as long as the save takes to fail or land. Extracted `handleAccountIdChange`, which calls a new `forceClose()` that bypasses the save lock deliberately (the lock exists to stop an accidental reader dismissal, not to protect this).
- **P2, real — the same effect closed the panel on an ordinary same-account token refresh, discarding unsaved edits.** It compared `AuthService.sessionEpoch()`, which — per that signal's own doc comment — advances on every token refresh, not only a real identity change. Switched the auto-close comparison to `AuthService.accountId()` (`null` when signed out, otherwise stable across an ordinary refresh); the response-staleness guards in `loadProfile()`/`save()` keep comparing `sessionEpoch()`, unchanged, since discarding an occasional needlessly-stale response is the correct trade there.
- **P2, real — the focus trap's backward boundary missed the panel's own initial focus target.** `ngAfterViewInit` focuses the dialog container (`#panel`), not its first control, so the very first Shift+Tab pressed right after opening compared `document.activeElement` against `first` and missed, escaping to the page behind the dialog. `onDialogKeydown` now treats the container itself as an equivalent backward boundary.
- **P1, docs — `STORY_LAB_AUTH_PROFILE_CLOUD_LIBRARY_EXEC_PLAN.md` was untouched.** Updated its "Current Reality" list alongside this changelog entry, per `AGENTS.md`'s documentation map.

Validation (final, after both Codex rounds above): `npx tsc -p tsconfig.app.json --noEmit` / `tsconfig.spec.json --noEmit` clean; `ng build` clean; full Angular unit suite 342/342 (`story-lab-profile-panel.component.spec.ts` covers load/save/error/staleness/account-identity-close/focus-trap paths; `app.spec.ts` covers Profile-button routing, save-time and sign-in-time default application, and the unchanged pre-existing suite); `npm run test:all` clean (`story-lab-picker-vocabulary.test.ts` updated as noted above; `story-generator-app-line-budget.test.ts` still passes at 3,049/3,100 lines); `git diff --check` clean.

## 2026-09-05 UTC - `App` god-component regrew past its own #324 fix, with nothing to catch it (PR #340)

PR #324 cut `story-generator/src/app/app.ts` from 3,212 to 3,018 lines specifically to end "the entire Story Lab frontend crammed into one class." In the four commits since (#326 Cloud Account/Auth plus its #328 follow-up, the chapter-illustrations fix, #336 job-progress streaming), nearly all new state landed straight back on `App` instead of a service, and it grew to 3,339 lines — past its own original, pre-fix size, with no lint rule or test ever failing along the way.

Extracted the cloud-library sync/staleness-guard machinery (auth-transition tracking, request-identity staleness guards, list/save/load/delete lifecycle plumbing) into a new component-scoped `CloudLibraryService`, following the same pattern `MemoryCardService`/`FormValidationService`/`StoryWorkspaceStorageService` already established in this file. `saveActiveProjectToCloud`/`loadCloudProject`/`deleteCloudProject` stay on `App` since they touch its workbench/blueprint editing state directly; they now delegate the shared guard/lifecycle plumbing to the service instead of reimplementing it. `App`'s public signals/methods are preserved as aliases/delegates, so the template and every existing caller are unchanged. Added `tests/story-generator-app-line-budget.test.ts` — a hard 3,100-line ceiling on `app.ts`, wired into `test:all`, modeled on the existing `story-generator-component-style-budget.test.ts`/`check-vercel-function-count.sh` structural checks — the piece the #324 fix was missing, so a third regrowth now fails CI instead of drifting silently.

Posted to `#claude-routines` for critique before implementing; no response arrived in the review window, so it shipped as planned. Codex's review of the initial push (`b205021`) found one issue, a documentation gap: this changelog entry was missing, which is the one finding fixed in this slice.

Validation: `npx tsc -p tsconfig.app.json --noEmit` / `tsconfig.spec.json --noEmit` clean; `ng build` clean (pre-existing `proving-grounds.css` budget warning only, unrelated); full Angular unit suite 320/320 (18 new `cloud-library.service.spec.ts` cases, plus the existing `app.spec.ts` cloud-account suite passing unchanged in substance); `npm run test:all` clean; `git diff --check` clean.

## 2026-09-05 UTC - Critical-error alerting stops being a stub that pages no one (PR #339)

`Logger.critical()`'s own doc comment says it's for "system failures, data corruption, security issues," and `ERROR_LOGGING_IMPROVEMENTS.md` showed the intended usage as `logCritical('Security breach detected', ...)`. But `sendToExternalLogger`, the method that was supposed to make "critical" mean something beyond another `console.error`, was a literal `// TODO: Implement external logging service integration` stub — it just re-printed the entry to the console, identical in effect to the `error()` level it's supposed to exceed. That gap stopped being theoretical once the API route crash guard (`expressApiRoutes.ts`'s `apiErrorHandler`) and the process crash guard (`unhandledProcessFailureLogger.ts`) were both wired to call `logCritical` in recent slices — the app's two last lines of defense for "something is on fire" funneled into a severity tier whose distinguishing behavior was never built, with zero test coverage on that path.

Added `api/_lib/utils/criticalAlertSink.ts`: a `ConsoleCriticalAlertSink` (byte-identical to the prior placeholder's console output, so an unconfigured deployment sees zero behavior change) and a `WebhookCriticalAlertSink` that POSTs a Slack-incoming-webhook-compatible `{ text }` body to `CRITICAL_ALERT_WEBHOOK_URL` when set. `logger.ts`'s `sendToExternalLogger` now delegates to `dispatchCriticalAlert`, which resolves the sink fresh from the live environment on each call — the same no-network-until-actually-sending pattern `api/health.ts` already uses for the rate-limit and Story Lab job stores. Extended `/api/health` with `services.criticalAlerting: { mode: 'console' | 'webhook', configured: boolean }` for the same operator-visibility reason the health check itself was fixed for in #338. Not touching a full Sentry/Datadog SDK integration (a new credentialed dependency this environment has no way to verify end-to-end) or the frontend `error-logging.ts` `logCritical` (identical in-memory-only shape, but a separate auth/endpoint surface and bigger scope) — both flagged as known follow-ups rather than silently left as-is.

Posted to `#claude-routines` for critique before implementing; no response arrived in the review window, so it shipped as planned.

Codex's review of the initial push (`a8fea4b`) found five real issues (four P1, one P2), all fixed in the same slice rather than left as follow-ups:

- **P1, real — the process-crash guard's alert could never leave the process.** `story-generator/src/server.ts`'s `uncaughtException` handler called `logUnhandledProcessFailure(...)` (fire-and-forget) and then immediately `process.exit(1)` — the one call site this whole feature was built to cover had no guarantee the webhook request even started before the process ended. Added `Logger.criticalAndFlush()`/`logCriticalAndFlush` (awaits delivery, still bounded by the sink's own timeout, still never throws) and `CriticalAlertSink.sendAndWait()` on both sink implementations; `logUnhandledProcessFailure` now returns a promise, and the `uncaughtException` handler awaits it (via `.finally()`) before exiting.
- **P1, real — arbitrary error text reached the external webhook.** `formatCriticalAlertText` included `entry.error.message`, but both crash guards accept *any* thrown error, and the logger's redactor only strips recognizable credentials/emails/URLs — not arbitrary prose. A validation error stringifying story text or a raw provider payload would have been a new third-party disclosure path console-only logging never had. Fixed: the webhook payload now carries only the error's `name` (bounded, operational), never its `message`.
- **P1, real — a malformed `CRITICAL_ALERT_WEBHOOK_URL` reported `configured: true`.** Any truthy value (`"not-a-url"`, whitespace, an `ftp:` URL) selected webhook mode and reported it as working, while every delivery would fail — a typo indistinguishable from live alerting until the first real emergency. Added `isValidHttpWebhookUrl` (must parse as `http:`/`https:`); an invalid URL now reports `{ mode: 'webhook', configured: false }` (mirroring how `rateLimitStoreConfig` reports an unreachable `postgres` mode) and is folded into `/api/health`'s `degraded` computation, renamed `isServiceDependencyDegraded` to cover it alongside the two durable stores.
- **P1, real — the health field could report "webhook" when nothing would ever dispatch to it.** `Logger.critical()` only escalates beyond the console when `NODE_ENV === 'production'` (a pre-existing gate, unchanged) — but `/api/health` reported `mode: 'webhook'` for a valid, configured URL regardless of environment, including the README's own documented `NODE_ENV=development` local setup. `createCriticalAlertSinkConfig` now folds in that same production check, reporting `console` (not `webhook`) outside production so the field always reflects what will actually happen right now.
- **P1, real and the deepest finding — the two crash guards `logCritical` reaches are unreachable on the Vercel deployment.** `vercel.json`'s `functions: { "api/**/*.ts": {...} }` deploys every file under `api/` as its own isolated serverless function, invoked directly with no Express error middleware wrapped around it; `expressApiRoutes.ts`'s `apiErrorHandler` and the process-level `unhandledProcessFailureLogger.ts` are reachable only through the standalone Express/Docker server. So on Vercel, `services.criticalAlerting` could report a fully working webhook while zero backend code path was capable of calling it — the same "claims a capability with no reachable code path" defect this whole routine exists to catch, just one level deeper than the original pick. Added `api/_lib/http/withUnhandledRouteFailureLogging.ts`: wraps a route handler so anything escaping its own try/catch (a bug in a CORS/method check before the handler's inner try, or in the catch block itself) is logged at `critical` severity and answered with a generic `500`, without double-writing a response if headers are already sent (an SSE stream mid-flight). Wired into the `export default` of all nine Vercel entrypoints (`health.ts`, `export/save.ts`, `image/generate.ts`, `audio/convert.ts`, `story-lab/account.ts`, `story-lab/evaluate.ts`, `story-lab/jobs.ts`, `story-lab/stories.ts`, `story-lab/stories/[storyId]/continue.ts`) — since `expressApiRoutes.ts` imports these same default exports for the Express/Docker deployment too, `apiErrorHandler`'s own `logCritical` call becomes a genuinely-last-resort second net there rather than the *only* one.
- **P2, real — same environment-mismatch as the P1 above, different field.** Covered by the same production-gating fix.

Validation: `tests/critical-alert-sink.test.ts` (17 cases: sink selection now production-gated, malformed-URL handling, non-production console fallback, redacted webhook payload, `sendAndWait` success/failure paths, `dispatchCriticalAlertAndWait`); new `tests/with-unhandled-route-failure-logging.test.ts` (5 cases: pass-through, sync throw, rejected promise, headers-already-sent, logged request context); extended `tests/health-route.test.ts` (5 cases total for `criticalAlerting`: console default, webhook in production, console outside production despite a valid URL, degraded on a malformed URL); `tests/logger-recent-logs.test.ts` and `tests/express-server-process-guard.test.ts` re-verified unaffected by the `log()`/`recordLogEntry` refactor and the now-async `logUnhandledProcessFailure`. Registered as `npm run test:critical-alert-sink` and `npm run test:with-unhandled-route-failure-logging`, both wired into `test:all`; full `npm run test:all` clean; API typecheck (`tsc --noEmit` over `api/**/*.ts`, the exact preflight invocation) clean; Angular app typecheck (`tsc -p story-generator/tsconfig.app.json --noEmit`, for the `server.ts` change) clean; `check-vercel-function-count.sh` 9/12, unchanged (no new route file — `withUnhandledRouteFailureLogging.ts` lives under `_lib`); `git diff --check` clean.

Codex's re-review of that push (`a1c4f4e`) found four more issues (two P1, two P2), all fixed in the same slice:

- **P1, real — a credential could still leak through the endpoint field.** `api/story-lab/jobs.ts`'s SSE route accepts its `API_KEYS` credential as an `apiKey` query-parameter fallback (its `EventSource` client cannot set custom headers) — see `withEventStreamAuth`. `readRequestPath`/`context.endpoint` carried the full URL including that query string, and `redactSensitiveLogData` blanks a *key* it recognizes, not an arbitrary token sitting inside a query string, so an escaped failure on that route could have forwarded a live credential to the webhook verbatim. Added `sanitizeEndpointForAlert` to `criticalAlertSink.ts`, stripping everything from the first `?` onward — applied once, in `formatCriticalAlertText`, protecting every `logCritical` caller rather than requiring each one to sanitize its own endpoint first.
- **P1, real — the new Vercel wrapper had the identical "may never leave the process" gap already fixed for the process-crash guard.** `withUnhandledRouteFailureLogging.ts`'s `handleEscapedFailure` called fire-and-forget `logCritical` before answering the response; a Vercel Node function stays alive only until the promise its `export default` handler returns settles, and nothing there waited for that. Switched to `logCriticalAndFlush`, sending the response first (so the failing request's own latency is unaffected) and then awaiting the flush before the wrapper's own returned promise resolves — keeping the invocation alive long enough for delivery.
- **P2, real — `error.name` was still an uncontrolled disclosure path after the `message` fix.** `Error.prototype.name` is an ordinary writable string; nothing stops a dependency or handler from setting it to arbitrary text. Added `sanitizeErrorNameForAlert` (an identifier-shaped allow-list pattern; anything else becomes a generic `Error` label) alongside the endpoint sanitizer.
- **P2, real — a mid-stream escape left the connection open until the platform's function timeout.** `sendGenericFailureEnvelope`'s `headersSent` branch was a no-op once a response (an SSE stream, say) had already started. Now calls `res.end?.()` in that branch, closing the connection instead of stranding the caller.

Validation: extended `tests/critical-alert-sink.test.ts` (+2 cases: query-string stripped from the endpoint, unsafe error names replaced with a generic label — 19 total) and `tests/with-unhandled-route-failure-logging.test.ts` (+1 case: the connection is closed exactly once on a mid-stream escape — 6 total); full `npm run test:all` clean; API typecheck clean; `check-vercel-function-count.sh` 9/12, unchanged; `git diff --check` clean.

Codex's third review of that push (`f5d6fd7`) found four more issues (one P1, three P2), all fixed in the same slice:

- **P1, real — a non-string error name could crash critical-alert delivery entirely.** `extractErrorDetails` in `logger.ts` did `error?.name || 'Error'` — a `Symbol` is truthy, so it passed through unconverted even though `LogEntry['error'].name` is typed as `string`. `sanitizeErrorNameForAlert`'s `RegExp.test` coerces its argument with an implicit `ToString` that *throws* for a `Symbol`, and that call happens *before* `WebhookCriticalAlertSink.sendAndWait`'s own `try` block — so a single malformed error name could throw synchronously out of `formatCriticalAlertText`, defeating this whole sink's "never throws" contract for exactly the unanticipated-bug case it exists to alert on. Fixed at both ends: `extractErrorDetails` now only accepts a genuine non-empty string, falling back to `'Error'` otherwise; `sanitizeErrorNameForAlert` was also widened to accept `unknown` (not `string`) with its own `typeof` guard, so it can never throw regardless of what any caller — this one now fixed, or a future regression — hands it.
- **P2, real — the correlation id was settled after the response was already committed.** `handleEscapedFailure` read (and, for a caller with no incoming id, minted) the correlation id only when building the critical-log context, *after* `sendGenericFailureEnvelope` had already sent the `500` with no `X-Request-ID` header. A caller with no incoming id therefore got a 500 with no id to quote, while a different, unreturned id was the one that showed up in the alert. Now calls `settleRequestCorrelationId` (mint-if-missing, echo via `X-Request-ID`) before sending the response, reusing that exact value for the alert — skipped in favor of a read-only `readRequestCorrelationId` when headers are already sent (a mid-stream escape), since `res.setHeader` throws past that point and there is no header left to echo onto anyway.
- **P2, real — the webhook config/health/README claimed a currently-untested guarantee.** `ERROR_LOGGING_IMPROVEMENTS.md` marked "set up alerting" as flatly done; every delivery test in this PR replaces Axios with an injected `postFn`, so nothing here has actually proven a real webhook (Slack or otherwise) receives and displays the payload — no credential-safe way to do that exists in this environment. Reworded as an explicit non-claim rather than silently overstating verified behavior.
- **P2, real — README's webhook documentation didn't reflect the production-gating or invalid-URL-degrades-health fixes from the second round.** Updated the Environment Setup comment and the Health Check section to state plainly that the webhook only takes effect when `NODE_ENV=production`, and that a malformed URL degrades `/api/health` (`503`) rather than merely being invisible.

Validation: extended `tests/critical-alert-sink.test.ts` (+2 cases: a `Symbol` error name doesn't throw synchronously or reject `sendAndWait` — 21 total), `tests/logger-recent-logs.test.ts` (+1 case: `extractErrorDetails` falls back to `'Error'` for a non-string name), and `tests/with-unhandled-route-failure-logging.test.ts` (+2 cases: a minted id is echoed via `X-Request-ID` and matches the alert; a caller-supplied id survives and is echoed unchanged — 8 total); full `npm run test:all` clean; API typecheck clean; `check-vercel-function-count.sh` 9/12, unchanged; `git diff --check` clean.
...
Threaded a narrow `continuityState` field (`characterNames`, `openThreads`, `latestChapterExcerpt`) and a separate `continuityGuidance` field across the seam, and gave the hidden guidance its own `CONTINUITY REQUIREMENTS (do not deviate):` heading apart from the reader's own `CREATIVE DIRECTION:` brief.

Codex's review of the initial push (`7c2484b`) found one CI-blocking issue (SonarCloud, not a Codex finding — see below) and five review findings, four real:

- **P1, real — mid-batch tone staleness.** A multi-chapter continuation call generates chapter 2, 3, ... from a growing `existingContentOverride`, but `continuityState` is computed once, before the whole batch — reading its `latestChapterExcerpt` for every chapter told chapter 3 the tone of the chapter before the batch even started, even if chapter 2 changed register moments earlier in the same call. Fixed: tone now reads the override's own tail once one exists (`createContextExcerpt`); the batch's first chapter still reads the accurate pre-batch excerpt, since its override equals `input.existingContent`.
- **P1, real — state alone can silently stop growing.** `continuityExtractor.ts` sometimes skips extraction or falls back after a provider error and returns the pre-extraction state completely unchanged, nothing prose-derived merged in. `continuityState` being preferred outright meant a run stuck on that path permanently lost the ability to notice a character or thread introduced in prose alone. Fixed: `characterNames`/`activePlotThreads` are now a merge (union) of state and the regex scan, state's own entries first — safe in the accurate case too, since the scan can only add a name/thread the text actually contains, never remove or override what state already knows.
- **P2, real — the merged list needed a bound.** Nothing capped how many characters/threads a long story could accumulate before they were joined wholesale into every future prompt. Capped to 12 characters (80 chars each) / 8 threads (180 chars each), the same order of magnitude the Courtroom already bounds its own hidden guidance to.
- **P2, real — a request-supplied `storyState` missing `characters`/`threads` entirely threw a 500.** The route/job normalizer only truthy-checks `storyState`, never schema-validates it. Fixed by reading through `getStateCharacters`/`getStateThreads`, exported from `continuationGuidance.ts` where the same `Array.isArray` guard already existed for this same `storyState`, rather than a bare `.map()`/`.filter()`.
- **P1, docs — this exec-plan section**, added in that same push.

Also fixed, not a Codex finding: `storyLabEngine.ts` found `previousChapters.reduce(...)` with no initial value to locate the latest chapter — SonarCloud's Quality Gate flagged it as a New Code Reliability bug (throws on an empty array; nothing at that call site lets a static reader see the `previousChapters.length === 0` guard earlier in the same function already rules that out). Replaced with `.find()` against the already-computed `currentChapterCount`, with a defensive fallback.

Codex's review of that push (`1f1f1ee`) found three more issues, two real:

- **P2, real — the new cap re-broke the merge fix above.** `mergeUniqueStrings` puts state's entries first and the prose scan's after; a plain prefix cap over that combined list starves the prose scan to zero whenever state alone already reaches the cap — exactly the long-running, many-characters story where the merge fix matters most. Fixed by reserving slots (3 of 12 for characters, 2 of 8 for threads) exclusively for prose-only discoveries: state is capped to `max - reserved` first, then the prose scan fills the rest.
- **P2, real — a malformed array *entry* still slipped through.** `getStateCharacters`/`getStateThreads` guard a missing array, not a malformed entry inside a present one (`storyState.characters: [{}]` passes `Array.isArray` but yields `undefined` from `.map(c => c.displayName)`), which would have thrown one call site deeper, inside `capPromptStringLengths`' string helpers. Fixed by filtering through `isNonEmptyString` (exported from `continuityExtractor.ts`, where the same check already existed) right where `continuityState` is built.
- **P1, docs — this changelog entry**, added in this same push.

Validation: `npm run test:all`, plus the exact `tsc --noEmit` invocation `scripts/recovery/preflight.sh` runs over `api/**/*.ts` — both re-run clean before each push. New/updated coverage: `tests/story-service-prompt-guards.test.ts` (state-vs-prose merge, the reserved-slot cap, mid-batch tone freshness), `tests/story-lab-continuation-guidance.test.ts` (`buildContinuationHiddenGuidance`), `tests/story-lab-real-engine.test.ts` (engine wiring, the malformed-entry case), `tests/story-quality-evals.test.ts`.

## 2026-09-04 UTC - Content boundaries reach the direct genesis/continuation routes, not just the job routes (PR #329)

A signed-in reader's stored "no-go content" safety preference (`profile.preferences.contentBoundaries`) was folded into the Heat Contract on the Story Lab job routes (`jobRouteHandlers.ts`) but not on `api/story-lab/stories.ts` (direct genesis) or `api/story-lab/stories/[storyId]/continue.ts` (direct continuation) — the two routes the Proving Grounds UI actually calls. Whether a reader's boundary was honored depended on which of two parallel routes served the request. Extracted the job route's `loadAuthenticatedContentBoundaries`/`withMergedContentBoundaries`/`capNoGoSource` into a shared `api/_lib/story-lab/contentBoundaries.ts` and wired both direct routes to use it.

Codex's review of the initial push (`3bb87fb`) found four issues, three fixed in this slice:

- **P1, real and severe — the frontend never attached a session token to either generation route, so the backend fix above was unreachable from a real signed-in browser session.** `auth.interceptor.ts`'s `ACCOUNT_AUTH_ROUTE_PATTERN` only matched `/api/story-lab/account/(profile|projects)`, deliberately excluding generation. Added a second `GENERATION_ROUTE_PATTERN` matching `/api/story-lab/stories` (covers both the genesis route and the `.../continue` sub-route) and attach the token there too — neither route gates on auth (`getCurrentUser`, never `requireUser`), so an anonymous caller is served exactly as before. Added interceptor spec coverage for both routes, signed-in and signed-out, plus a still-untouched-route case (`/api/story-lab/jobs`) to keep proving the interceptor doesn't blanket-apply.
- **P1, real — a continuation with no Heat Contract at all silently dropped a signed-in caller's stored boundaries rather than refusing.** `withMergedContentBoundaries` has nothing to merge a boundary into when the request supplied no contract, so the previous `contentBoundaries && normalized.heatContract` guard let those requests through unchanged — indistinguishable, from the generated story, from a caller with no boundary set. Added `resolveContinuationHeatContract` to `contentBoundaries.ts`: unchanged when there is nothing to fold in, merged when a contract is present, and `{ ok: false }` when a signed-in caller has stored boundaries and no contract to carry them — refused as `400 INVALID_REQUEST` on both the direct continuation route and the job route (moved earlier, before the job is created, so the refusal is a request-level 400 rather than a job that starts and then fails). Updated the existing job-route test that asserted the old (now-wrong) silently-proceeds behavior, and added a paired test proving a signed-in caller with *no* stored boundaries still proceeds normally with no contract — the refusal is about the boundary being unhonorable, not about being signed in.
- **P1, docs — this changelog entry**, per the Documentation Update Map's requirement that any recovery slice changing behavior gets a `PR70_RECOVERY_CHANGELOG.md` entry.
- **P1, evaluated and NOT applied — fail-closed on a profile-store error.** Codex proposed that `loadAuthenticatedContentBoundaries` should return an unavailable response instead of `undefined` when the profile lookup itself errors (a transient DB failure), so it isn't indistinguishable from an intentionally-empty preference. Not applied: this is the job route's own pre-existing, documented design (unchanged by this PR, only relocated) — "a reader's boundary is a courtesy layered onto generation, not a gate that should ever turn a working request into a failed one" — and inverting it would mean any transient profile-store hiccup blocks *every* signed-in caller's story generation outright, including the majority who have no boundary configured at all, in exchange for closing a narrower gap (one generation proceeds without a boundary it would otherwise have honored, the same outcome an anonymous caller or a caller who simply has no boundary set already gets today). Replied on the PR thread with this reasoning rather than applying the suggested change.

Validation: `npx tsx tests/story-lab-route-status.test.ts`, `npx tsx tests/story-lab-job-routes.test.ts`, `npx tsx tests/story-service-prompt-guards.test.ts`, full `npm run test:all`, the `auth.interceptor.spec.ts` suite headless, and the full Angular karma suite headless (287/287) — all confirmed clean before each push.

### Round 2: SonarCloud duplication (two passes) and three more Codex findings (PR #329)

Codex's review of the round-1 push (`92e7605`) found three more issues, all fixed:

- **P1, real and severe — attaching the Clerk token via `Authorization: Bearer` on the generation/job routes collides with `API_KEYS`.** Every route `GENERATION_ROUTE_PATTERN` matches also runs `enforceApiAccessControl`/`authenticateRequest` (`beginPostRoute`), which reads `Authorization: Bearer` as an `API_KEYS` candidate whenever a deployment configures one — a Clerk JWT sent that way would be misread as an invalid key and answered `401 INVALID_API_KEY` before ever reaching Clerk verification, breaking every signed-in generation request in a deployment that combines `API_KEYS` with Clerk auth. The account routes never hit this because `accountRouteHandlers.ts` never calls `enforceApiAccessControl` — confirmed by reading both call graphs directly. Fixed by adding a dedicated `X-Story-Lab-Session` header: `clerkAuthPort.ts`'s `readClerkSessionToken` reads it first (falling back to `Authorization`, then the `__session` cookie, unchanged), and the interceptor sends the token there for generation/job routes while continuing to use `Authorization` for the account routes. Added coverage in `story-lab-clerk-auth.test.ts` (header read, case-insensitivity, precedence over `Authorization`) and rewrote the interceptor spec's route-parametrized test to assert `X-Story-Lab-Session` is set and `Authorization` is not, on all three routes.
- **P1, real — the job route (`/api/story-lab/jobs`), not just the two direct routes, needed the token too.** `App.startGenesis()`/`continueSaga()` — the app's actual primary generation flow — call `StoryService.createStoryLabJob()`, which posts to `/api/story-lab/jobs`, not the direct genesis/continuation routes this PR started with (those are reached by the Proving Grounds/debug-panel UI instead). Confirmed by reading `app.ts` and `story.service.ts` directly. Extended `GENERATION_ROUTE_PATTERN` to `/api/story-lab/(stories|jobs)`. This also matters for durable job storage, which gates job creation behind `requireUser` — without a token, that would 401 for every signed-in caller once durable storage is enabled.
- **P2, real — a whitespace-only stored `contentBoundaries` value incorrectly triggered the round-1 refusal.** `normalizeStoryLabProfilePreferences` preserves a whitespace-only value and the `PUT` length check accepts it, but `capNoGoSource` (inside `withMergedContentBoundaries`) trims it to nothing — so `resolveContinuationHeatContract`'s truthiness check disagreed with the rest of the module about whether such a value is a real restriction, refusing continuations that had nothing to honor in the first place. Fixed by trimming before the check. Added a route-level regression test with a whitespace-only stored boundary, proving the continuation now succeeds unchanged.

Also two SonarCloud Quality Gate failures on new-code duplication, found and fixed before merge, neither from a Codex finding: round-1's push initially duplicated `createStaticAuthPort`/`createRejectingAuthPort`/`createStubProfileStore` between `story-lab-job-routes.test.ts` and the new coverage in `story-lab-route-status.test.ts` (24.2%, required ≤3%) — extracted to `tests/helpers/storyLabAuthFixtures.ts`. That still left a third, older copy of `createStaticAuthPort` in `story-lab-account-routes.test.ts` undetected by the first pass (10.0%, then 6.5% after diluting with round-2's own new lines) — pointed that file at the same shared helper, which is what actually cleared the gate.
...
- **P1, real and severe — every real deployment would have finished initialization with `identityTransitionPending()` stuck true forever, silently blocking all four cloud-library methods.** Verified against the actually-installed `@clerk/clerk-js@6.31.0` types (`addListener(callback: ListenerCallback, options?: ListenerOptions)`, `ListenerOptions.skipInitialEmit` — "If `true`, the callback will not be called immediately after registration. Defaults to `false`"): the real client invokes a freshly-registered listener immediately, which our test fakes never reproduced. That immediate call would start a *tagged* refresh (`clearsIdentityTransitionOnSettle: true`) before `loadConfigAndClient`'s own explicit, untagged `refreshSessionToken()` call ran — and since that untagged call's higher generation supersedes the tagged one, the tagged call's `.finally()` generation check would always fail, so `identityTransitionPendingState` would never clear. Fixed by registering the listener with `{ skipInitialEmit: true }` (the explicit call right after it already handles the initial load) and by fixing both test fakes (`auth.service.spec.ts`, `app.spec.ts`) to actually reproduce Clerk's real immediate-emission behavior — so if this regressed again, tests would catch it, which they could not before. Added a regression assertion in the existing "loads the Clerk client..." test.
- **P1, docs — the exec plan's round-13 summary didn't mention `clientReadyState`.** Added a paragraph covering the `isConfigured()`/`clientReadyState` distinction, its retry proof, and the `skipInitialEmit` fix above, plus an explicit pointer to the "Not live yet" section for what's still unproven against a real Clerk tenant.
- **P1, evaluated and NOT applied — false positive.** Codex claimed the installed `@clerk/backend` export returns the decoded JWT payload directly and throws on failure rather than a `{ data, errors }` envelope, and that `clerkSessionVerifier.ts` reading `result.data` would make every real verification return `undefined`. Checked against the actually-installed `@clerk/backend@3.17.1`: both its `.d.ts` (`verifyToken(...): Promise<JwtReturnType<JwtPayload, TokenVerificationError>>`, where `JwtReturnType<R, E> = { data: R; errors?: undefined } | { data?: undefined; errors: [E] }`) and its compiled runtime source (`dist/index.js`'s `verifyToken`, which returns `{ errors }` or the `{ data, errors? }` result of `verifyJwt` — never throws, never returns a bare payload) confirm the code's existing `result.errors || !result.data?.sub` / `result.data.sub` handling already matches the real contract exactly, and `clerkSessionVerifier.ts`'s own docstring already documents this. Not changed; replied on the thread with the verified evidence instead of applying the suggested "fix", which would have broken every real token verification.

Validation: `tsc --noEmit` on both app and spec configs, full karma suite headless (284/284, confirmed stable across two runs), `ng build`, `npm run test:all` (backend, unaffected but re-run for safety) — all confirmed clean before push.

## 2026-09-03 UTC - Clerk auth hardening round 13: `isConfigured()` no longer masks a Clerk client-load failure (PR #328)

Codex's review of the round-12 push (`416ae42`) found one more issue: `AuthService.isConfigured()` was `computed(() => this.authConfigState()?.provider === 'clerk')` — true as soon as the auth-config response reported a Clerk provider, regardless of whether `loadConfigAndClient()`'s later `await this.client.load()` actually succeeded. When the deployment was correctly configured but the Clerk script was blocked or the network failed, `authConfigState` was already set before that load was attempted, so `isConfigured()` stayed true forever after — `App.showCloudAccountSetupStatus()`'s retry path (`if (!this.authService.isConfigured()) { ...show the setup-pending message... }`) never saw it go false, so clicking "Connect account" silently did nothing on every retry instead of surfacing an actionable error. Fixed by adding `AuthService`'s `clientReadyState`, set `true` only once the Clerk client has actually finished loading (client construction, `load()`, listener registration, and the initial token refresh all succeeded) and `false` on any load failure, and folding it into `isConfigured` (`authConfigState()?.provider === 'clerk' && clientReadyState()`) — the only call site. Updated a stale test comment that asserted the old (buggy) behavior and added assertions proving `isConfigured()` goes false after a client-load failure and true again after a successful retry, plus an `App`-level test proving `showCloudAccountSetupStatus()` now shows the setup-pending message instead of silently doing nothing.
...
- **A superseded refresh could hand an interceptor the *new* account's token to authenticate a request already built for the *old* account.** Round 10's `resolveSupersededRefresh` distinguished "superseded by `signOut()`" (return `null`) from "superseded by an ordinary newer refresh" (defer to that refresh's own settled result) using only `signOutGeneration`/`sessionRefreshGeneration` — but an account switch (a Clerk multi-session swap, not a sign-out) also supersedes an in-flight call, and that case fell into the "ordinary newer refresh" branch: the caller who started before the switch — an interceptor already carrying a request payload for account A — would receive account B's token and send A's data authenticated as B, letting the server persist it into B's library before any response-level identity guard runs. Fixed by capturing `sessionEpoch` (not just the generation counter) at the moment each `refreshSessionToken()` call starts, and rejecting with `null` whenever the epoch has moved by the time a superseded call resolves — sign-out and an identity change both bump it, so this subsumes the old `signOutGeneration` check, which was removed as redundant. Added a negative test: an earlier call in flight when `fireSessionChange` swaps the account mid-await must resolve `null`, not the new account's token.
- **A stale `error`/`complete` left `isCloudLibraryBusy` stuck true forever.** `guardStaleCloudResponse`/`guardStaleCloudComplete` dropped a stale callback entirely, including `error`/`complete` — but those are the only callbacks that ever clear the busy lock for a given request, so discarding one because the response was stale left every cloud control disabled permanently, with no later callback to release it. Fixed by splitting `error` into a new `guardStaleCloudError` that always releases the lock first, then still guards the error's own state-mutating side effects (logging, `cloudLibrarySyncState`) behind the staleness check, and making `complete` an unconditional `isCloudLibraryBusy.set(false)` (its only job) rather than a guarded call. Added two tests covering a stale `error` and a stale `complete`.

Validation: `tsc --noEmit` on both app and spec configs, full karma suite headless (281/281), `ng build`, `npm run test:all` (backend, unaffected but re-run for safety) — all confirmed clean before push.
...
- **A superseded refresh could still return a wrong (not just stale) token.** Round 9's `sessionRefreshGeneration` guard made a superseded `refreshSessionToken()` call return `sessionTokenState()` instead of overwriting it — correct when superseded by `signOut()`, but not when superseded by an *ordinary* overlapping refresh that hadn't itself resolved yet: the signal could be holding a value that belonged to neither the old nor the new state, and an interceptor could send it as the bearer for an outgoing request. Fixed by distinguishing the two cases: added `signOutGeneration` (the generation `signOut()` last invalidated) so a superseded call can tell whether sign-out is what invalidated it — answered with `null` — versus an ordinary newer refresh — answered by awaiting and returning that refresh's own settled result via a tracked `latestRefreshPromise`, never a snapshot. Added the negative concurrency test requested, forcing two overlapping refreshes to resolve out of order.
- **The live-signal comparison guard (round 9) had its own timing gap.** `isSignedIn`/`accountId` don't update until `refreshSessionToken()`'s `await session.getToken()` resolves — not the instant Clerk's listener fires — so a stale cloud-library response arriving in that window would still compare equal against the *outgoing* identity and pass the guard. Fixed by adding `AuthService.sessionEpoch`, a counter advanced synchronously inside the listener callback (and in `signOut()`) before any `await`, and folding it into the same comparison `App`'s cloud-library callbacks already made. Deliberately coarser than `accountId` — it also advances on an ordinary same-account refresh — trading an occasional needlessly-discarded in-flight response for never risking cross-account data leakage.
- **`ngOnDestroy()` never cancelled the cloud-library subscription.** Navigating to `/proving-grounds` while a save/load/delete/refresh was in flight left it subscribed; Angular does not auto-unsubscribe manually-created RxJS subscriptions on destroy. Fixed by calling `cancelInFlightCloudLibraryRequest()` from `ngOnDestroy()`. Added `cancels an in-flight cloud-library request on destroy`.
- **The auth exec plan still described the pre-round-9 behavior.** Updated `STORY_LAB_AUTH_PROFILE_CLOUD_LIBRARY_EXEC_PLAN.md`'s summary paragraph with the `sessionEpoch`/`sessionRefreshGeneration`/`ngOnDestroy` behavior above.

Validation: full karma suite headless (278/278, confirmed stable across two runs), `tsc --noEmit` on both app and spec configs, `ng build`, `npm run test:all` (backend, unaffected but re-run for safety) — all confirmed clean before push.
...
- **A pending `refreshSessionToken()` call could resurrect a session after sign-out.** `AuthService.signOut()` awaits `client.signOut()` then sets the session token to `null`. If a `refreshSessionToken()` call (from the Clerk session-change listener, or an interceptor's `getRequestToken()`) was already awaiting `session.getToken()` when sign-out landed, that older call's `await` could resolve afterward, with a token fetched before sign-out, and unconditionally overwrite the `null` sign-out had just set. Fixed with a request-scoped `sessionRefreshGeneration` counter: every `refreshSessionToken()` call increments it and captures its own value, `signOut()` also increments it, and only the call whose captured value still matches the counter when it resolves is allowed to write `sessionTokenState` — an older call resolving late (from either a sign-out or a newer overlapping refresh) discards its result and defers to whatever superseded it. Added `discards a session refresh that was already in flight when sign-out lands` in `auth.service.spec.ts`, holding `getToken()` open with a controlled promise to force the exact ordering.
- **Cancelling a cloud-library subscription is not sufficient by itself.** Every stale-response fix through round 6 relied on unsubscribing before a response could arrive — correct for the direct `signOutOfCloudAccount()` path (cancellation runs synchronously before any `await`), but not for the constructor effect's path: Angular's `effect()` is scheduled asynchronously, so an external session change (Clerk revoking a session, or a multi-session account switch in another tab) can leave a real window where an already-queued response's callback runs *before* the effect ever gets a chance to cancel it. Cancelling afterward stops nothing that already ran. Fixed by adding a second, timing-independent defense: `refreshCloudLibrary`/`saveActiveProjectToCloud`/`loadCloudProject`/`deleteCloudProject` now each snapshot the signed-in identity (`AuthService.isSignedIn()`/`accountId()`, read live) when the request starts, and every response callback (`next`/`error`/`complete`) compares that snapshot against a fresh live read before doing anything — a signal's current value is always correct the instant it's read, regardless of when any effect depending on it next gets to run, so this closes the window cancellation alone cannot. Added `discards a cloud response whose identity no longer matches the live signed-in state, independent of the constructor effect` in `app.spec.ts`, which changes `AuthService`'s live signal directly (bypassing `signOutOfCloudAccount()` and the constructor effect entirely) to prove the guard works without either of them having run.

Validation: `npm run test:all` (backend, unaffected but re-run for safety), `tsc --noEmit` on both app and spec configs, full karma suite headless (276/276, confirmed stable across two runs), `ng build` — all confirmed clean before push.
...
- **Correction to the entry directly below**: that entry's "current request's own dynamic target origin (`getRequestTargetOrigin`, now exported from `corsPolicy.ts`)" was itself the flaw. `getRequestTargetOrigin` reads `X-Forwarded-Host`/`Host` — a caller-supplied request header — and `authorizedParties` is not a CORS response header (where trusting that header is safe, since CORS only constrains a browser and a non-browser caller who forges it gains nothing it didn't already have): it is the actual boundary deciding which origins a session token may claim to come from. A caller holding a valid token from an untrusted sibling origin could set `X-Forwarded-Host` to that origin and pass. Fixed by switching to `resolvePlatformOrigins(env)`, reading `VERCEL_URL`/`VERCEL_BRANCH_URL` — set by the Vercel platform on the deployment's own runtime environment, never from anything in the request, so a caller cannot influence them. `getRequestTargetOrigin` reverted back to private in `corsPolicy.ts`, where its original CORS use remains legitimate. Added `tests/story-lab-clerk-session-verifier.test.ts::testVerifierIgnoresAForgedForwardedHostHeader`, a negative test proving a forged forwarded-host header never reaches `authorizedParties`.
- **`saveActiveProjectToCloud`/`loadCloudProject`/`deleteCloudProject` had the same unfixed stale-response race** the entry below fixed only for `refreshCloudLibrary()`: none of the three held or cancelled their HTTP subscription, so a response authenticated just before a sign-out could still arrive afterward — a late save silently re-adding the previous account's project, a late load hydrating that account's full story into the UI after it had already cleared its signed-in state. Fixed by reusing the same `cloudLibrarySubscription` field and `cancelInFlightCloudLibraryRequest()` `refreshCloudLibrary()` already had: `isCloudLibraryBusy` gates all four cloud-library requests against each other, so at most one is ever in flight and one field is enough to cancel whichever it is. Added matching `app.spec.ts` coverage (`discards a cloud-save/-load/-delete response that arrives after sign-out`), each using a controllable `Subject` and a storage mode chosen so the assertion actually distinguishes "discarded" from "processed" rather than matching either outcome.

Validation: `npm run test:all` (backend, all green, including the new negative forged-header test), `tsc --noEmit` on both app and spec configs (clean), `ng build` (clean), full karma suite headless (271/271, confirmed stable across two consecutive runs).
...
Review round 1 (Codex on `7319a58`) — **two P1s, both false negatives this slice introduced, both fixed:**

- **The whole-candidate score did not allow the inflections the tokens did.** `Blood pact` against `Settle the blood pacts tonight` scored **8 under the substring reading and 2 after the first draft** — the six-point phrase score is most of the signal, and `selectScoredCourtroomThreads` keeps three entries, so that is a demotion out of the prompt and the preview rather than a rounding difference. The phrase now asks the same question the tokens do: a normalized phrase is its words with single spaces between them, so an ending on the phrase is an ending on its final word.
- **The endings do not cover the consonant English doubles.** `plan` → `planed`, `plot` → `ploted`, `commit` → `commited` are spellings nobody writes, so a brief saying `planning`, `plotting` or `committed` scored the thread it names **0** where the substring reading scored 7. `inflectedWordForms` now also generates the doubled final consonant before the vowel-initial endings.

Review round 2 (Codex on `616c98b`) — **one P1, the same family one letter further in:**

- **A stem ending in `c` repeats as `k`, not as a second `c`.** The doubling rule added last round generated `panicced` and `paniccing`, which are not words and so match nothing, and `Panic` against `She panicked at dawn` scored **0** where the substring reading scored 7 — `mimic`/`mimicking` and `picnic`/`picnicked` likewise. `DOUBLED_CONSONANT_SPELLING` maps the final letter to what it is repeated as, so a second exception is a row rather than another branch.

**Why this one is not the same kind of simplification as the doubling itself.** Applying doubling to the last character rather than to a stress-tested stem is deliberate and safe because it can only generate a form too *many* — `courtted` is inert. Spelling `panicced` is different in kind: it generates the wrong form *instead of* the right one, so it does not merely add a useless comparison, it loses the only spelling a brief would carry. That distinction is now in the docblock, because it is the line between the licence this function takes and the defect it produced.

**Both are worse than the collisions this slice is for**, and that is the part worth recording. A false positive mis-ranks an item; a false negative sends it to the fallback and out of a three-entry list entirely — so the repair was failing in the direction its own argument says matters most. My own self-review pass an hour earlier found a false *positive* (`cove` for `cover`) and missed both of these, because it probed the direction the fix was designed for and not the direction it broke.
...
- **The heuristics' pattern form still covers only the appended endings.** A keyword in `storyQualityHeuristics` does not match its doubled-consonant inflection. That is pre-existing rather than introduced here, and closing it changes what those scans match, which is a separate slice.
...
Review follow-up (Codex, one P2 on `a447eaa`, real and fixed in the same slice):

- **The merged bound was only correct if both sources were themselves capped, and neither is guaranteed to be.** `withMergedContentBoundaries` appends the profile's boundaries *after* the request's text, so a continuation job supplying 700 characters produced a 1,021-character merge that the new 641 bound cut back to the request's half and **none** of the profile's — measured, `R 641/700, P 0/320`, on both builders. That is the very deletion the bound was introduced to stop, reappearing one source further up, and it is reachable precisely because of this slice's own first finding: no continuation route caps the request's half. Codex's second remedy is the one taken — cap each source before merging — because the two halves are still separate there, and by the time the prompt reads the field it is one string and any bound on it has to choose a half to lose. `capNoGoSource` holds each to `STORY_LAB_PROFILE_LIMITS.maxNoGoContentLength`, which makes the sum a bound by construction rather than by assumption.
- **The profile's half needed the same treatment, for a reason the finding did not name.** `describeOversizedStoryLabProfileField` refuses an oversized `contentBoundaries` on `PUT`, but `normalizeStoryLabProfilePreferences` deliberately does not, because it "runs on every *read* as well as every write — a profile that predates this cap has to keep loading". So a stored profile wider than its cap is a shape the merge has to expect, and capping only the request's half would leave the same deletion running the other way.
- Counterfactual added and killed (4 applied, 4 killed overall): removing `capNoGoSource` fails with the 1,021-character merge. Both halves are now asserted whole in both builders across three shapes — an oversized request half, a legacy oversized profile, and both — and the merge's two no-op cases (nothing to add; an empty request half taking the profile's without a stray separator) are pinned beside them.
- Non-claim: this is a floor, not the boundary these fields should have. The honest answer for the request's half is a continuation route that refuses it the way genesis does, which is the same open follow-up recorded for `continuationBrief` below; until that exists, truncating at the merge is strictly better than deleting the reader's standing boundaries.
...
Review round 3 (Codex on `9078fc4`) — **one more regression, fixed; and the stopping rule stated last round now binds**:

- **HTML's raw-text rules were applied inside foreign content.** `<h3><svg><title/></svg>Real Title</h3>` — SVG and MathML honour a self-closing `/` where HTML ignores it, so that `title` closes immediately. Entering raw-text mode there hunts for a `</title>` that never comes and abandons the scan; the heading was lost and the append path emitted a duplicate. `FOREIGN_CONTENT_TAG_NAMES` and a depth counter now suspend the raw-text rules inside `<svg>`/`<math>`.
- **Tracking the namespace rather than honouring `isSelfClosing` everywhere is the point.** A bare `<title/>` in HTML *does* open a raw-text element that runs to the end of the content, and a browser shows no heading after it — which this reader already answered correctly and the pattern it replaces got wrong. The two readings are opposite and only the namespace separates them, so the general fix would have broken a case already right.
- **Two of the four new counterfactuals survived their first drafts, and the surviving pair exposed a real weakness in how the obvious cases were chosen.** `<h3><svg/>Real Title</h3>` and a nested `<svg><svg><title/></svg></svg>` pass whether the depth arithmetic is right or wrong, because nothing downstream of them depends on the depth. Discriminating it needs an *unclosed* raw-text element after the foreign content — `<svg/><title>unclosed<h3>Heading</h3>` — where a wrong depth walks into text a browser never shows and reports a heading. That is **seven** surviving mutations across this slice, every one evidence about a test.
- Sixteen counterfactuals now, each confirmed failing a suite.

**Where this stops.** Three review rounds, each finding a further layer of HTML that this walk did not have: the tag-open state, three comment terminators, raw-text elements, the closing-tag boundary, and now foreign content. Every one was a real regression against the reader being replaced and every one is fixed — but the trend is the finding. Each round's markup is adversarial input that the generator, prompted for a bare `<h3>Chapter N: Title</h3>`, will never emit. **So is the defect this slice repairs.** That symmetry is #302's own consistency argument turned on this change: it cannot be right to keep buying browser parity on inputs of exactly the likelihood that was used to justify the work in the first place. The next finding of this class is documented and handed over rather than revised, and the question of whether this reader should be a partial HTML parser at all is put to the author on the PR.

Review round 2 (Codex on `d78ff3d`) — **two further regressions, both reading as markup what a browser reads as text**:

- **Raw-text and RCDATA elements were walked into.** `<script><!-- legacy\n</script><h3>Real Title</h3>` — inside `script`, `style`, `textarea`, `title` and their kin nothing is a tag and nothing is a comment until that element's own closing tag, so the `<!--` is script text. Reading it as a comment found no `-->`, and the walk abandoned the scan and lost the heading after it. The pattern being replaced found it. `RAW_TEXT_TAG_NAMES` and `findRawTextEnd` now skip such an element whole, matching its closing tag case-insensitively and only where the name ends at whitespace, `/` or `>` — so `</scriptx>` is more script text, as HTML has it. A side effect worth stating: `<textarea><h3>Not a title</h3></textarea>` no longer reads that heading as the title, which is what a browser shows and what neither this reader nor the pattern did before.
- **A closing tag was accepted on `findTagEnd`'s fallback boundary.** `<h3>Real Title</h3 data-x="a>b"class=x>` has no well-formed reading — the closing quote is followed by a word character — so the fallback ended the tag at the `>` inside the quoted value, and the body then *began* `b"class=x>`: a markup fragment emitted as visible prose, in both the title path and the continuation append. The pattern being replaced required a literal `</h3>` and never produced it. `findWellFormedTagEnd` is now exported from the scanner alongside `findTagEnd` — same reading, minus the fallback — and a closing heading tag must sit on a boundary it vouches for. **The opening tag is deliberately not held to the same rule**, and the asymmetry is the point: there the fallback decides only where the *title* starts and reproduces exactly what `[^>]*>` did, so it is coverage forgone rather than a regression introduced.
- `nextHeadingTag` split three ways as it grew — `nextTag` (the next thing that really is a tag, with comments, stray `<` and non-tag constructs skipped in one place), `isHeadingBoundary` (the rule above), and the heading walk itself. Cognitive complexity had reached 25; all three are now well under the gate, and the local SonarJS reproduction reports nothing on any line this branch adds except the declined test oracle.
- **The counterfactual sweep is twelve**, each confirmed failing a suite: the first-`>` boundary, the tag-open check, the `/` skip inside it, each of the three comment closings, first-opening-wins, the leading-only guard, raw-text skipping, the raw-text name's case-insensitivity, the raw-text name's boundary, and the closing-tag fallback rule.
- **Three of them survived their first drafts**, and all three were the tests' fault rather than safe mutations: the raw-text name's case-insensitivity and its boundary had no discriminating input at all, and one assertion was simply wrong — `body.includes(fragment)` cannot fail when the body is the untouched content, so it was replaced with the property that actually broke, the body *beginning* on the fallback boundary. That makes five surviving mutations across this slice, every one of them evidence about a test.
...
- Instead the analyzer was reproduced locally: `eslint-plugin-sonarjs` installed **outside the repository**, in a scratch directory, and run against copies of the changed files. It reported exactly three findings on lines this diff adds — `findChapterHeading` at cognitive complexity 19, and the test's `referenceTagEnd` at 79 and `walkInteriors` at 19 — matching SonarCloud's count of 3. Its other four findings are on pre-existing lines (`extractCharacterNames`'s speaker-tag pattern, `stripSpeakerTagsForDisplay`) that this diff does not touch, which is consistent with them not being counted as new. No dependency was added to the repository.
- **Two fixed.** `findChapterHeading` split into `nextHeadingTag` (walk to the next `<h3>`/`</h3>`, skipping comments and reading every intervening tag to its true end) and the heading assembly itself; both now well under the threshold, and the split names the reason the walk reads non-heading tags at all. `walkInteriors` split into `measureInterior` and the recursion.
- **One declined, and stated on the PR rather than left silent.** `referenceTagEnd` is a state-by-state transcription of the WHATWG start-tag states, and its readable one-to-one correspondence with the specification is the entire reason it is a valid oracle. Splitting it would obscure that correspondence and risk a silent error in the thing that verifies everything else. Cognitive complexity is the wrong measure for a transcription.
- **A fourth counterfactual survived the refactor's test sweep**, and it found a genuine gap rather than a mutation-testing artifact: `opening ??= tag` replaced by `opening = tag` — "first `<h3>` wins" becoming "last wins" — passed everything. Nothing asserted the two-openings case at all, though the reader's own comment claimed the behaviour. `<h3>First<h3>Second</h3>` now pins it, including the assertion that the replaced pattern took the same span. That is the second time in this slice that a surviving mutation was evidence about the tests; the first was the comment-skip case above.
- Re-validated after the refactor: `npm run test:all` exits 0, all four counterfactuals now fail a suite, API function typecheck clean, `shared/` clean under the Angular build's strict config.
...
Review round (Sourcery and Codex on `a155b30`) — one declined, **two real regressions found and fixed**, and the fuzzing that missed them corrected:

- **Sourcery, declined — a value containing a literal `<`.** `<p title="a<b>c">Hello.</p>` still leaks `c">Hello.`, and their suggested repair (let a quoted run cross `<`) does fix it. Measured rather than judged: on well-formed markup lifting the bound is right by a factor of 227 (46 losses against 10,444 recoveries over 300,000 inputs), and it inverts once a quote is unterminated (24,815 against 14,995), worst case a silent 26-word deletion. The newline-bounded alternative this issue proposed costs *identically to the digit*, so it is the same trade relabelled. Reproduces byte-identically on `main`, so it is row 4 of #296 rather than a defect in this diff; re-priced there and the thread left open.
- **Codex P2, fixed — attributes with no whitespace between them.** `<p title="a>b"class=x>Visible " prose > after</p>` lost `Visible " prose` that `main` kept. The real closing quote is followed by `c`, which the follow-set cannot accept, and the scan then hunted on and found a quote in the sentence. Fixed by making **the first candidate closing quote decide**: a rejected candidate means the value has no well-formed end, not that the end is further on — "further on" is where the story text is.
- **Codex P2, fixed — a quote inside an unquoted value.** `<p data-x=a"b>Visible text">After.</p>` likewise lost prose `main` kept. Fixed by opening a quoted value only where one may begin: directly after the `=` of an assignment.
- **Found while fixing those, not reported by either reviewer:** the same class of defect twice more. An `=` with no attribute name before it (`<v =">…">`) is not an assignment — HTML reads it as the attribute's *name* — and an `=` welded to the tag name (`<e=">…">`) is more tag name. Both were opening quoted values and swallowing sentences. Fixed by requiring a name before an `=` counts, and by starting the attribute scan past the tag name.
- **Why the fuzzing missed all four: the generator only ever emitted well-formed attributes** — always ` name="value"`, always whitespace-separated, always terminated. Every shape the reviewers found was one it could not produce. Rebuilt to emit unquoted values, values containing quotes, missing separators, bare attributes and prose carrying quotes and angle brackets; the regressions reproduce in it now.
- Also corrected a **measurement** error, not just a code one: the harness counts a missing vocabulary word as lost prose, and once markup is malformed a word can legitimately be absorbed *into* an attribute value. Minimising the remaining "losses" produced `<v e=">openedx">`, which is well-formed markup where this reader matches a browser and `main` leaks — the metric was wrong, not the code. The twin comparison is the authoritative one and is unaffected.
- Two of the eight rules stopped pinning once the new ones landed, because the new rules subsumed them on the inputs the tests used. Rather than trust that, both were removed and re-fuzzed: prose losses went from 34 to **5,728**, so they are load-bearing and the tests were at fault. Replaced with inputs that discriminate (`<p title='>' She wept for him<em>then left</em>` and a run reaching a following `title=x" >`). Two other guards turned out to be genuinely redundant and were deleted instead.
- **All eight rules now fail their counterfactual.** Re-validated: `npm run test:all` exits 0, API typecheck clean, function count 8/12, `git diff --check` clean. Over 400,000 well-formed inputs the fix is inert on all 104,101 where no quoted value carries a `>`, and twin-correct on all 295,899 where one does, with 0 disagreements.

Second review round (Codex, on `5262435`) — three more P2s, **all three real, including one that falsified a published non-claim**:

- **The security one, and the reason this round mattered.** `<script data-x="a>b"/>stealPrivateStory()</script>` put the script body into every export as story text. Reading the tag whole — which is this slice's whole point — preserves the trailing `/`, so `parseHtmlTag` marked it self-closing and `removeNonStoryHtml` never entered block-skipping. `main` was safe here only *by accident*: its truncation ate the slash. **In HTML the slash is ignored on `<script>`, `<style>`, `<iframe>` and every other non-void container** — only foreign content (`svg`, `math`) and void elements really self-close — so `SELF_CLOSING_DROPPED_TAGS` now names the three that do and the rest always enter block-skipping. This directly falsified the non-claim "the sanitizer's security boundary does not move", which had been asserted by a test using a *non*-self-closing `<script>`; the assertion did not cover the spelling that broke.
- **An `=` embedded in an unquoted value** (`<p data-x=a=b="c>…`). The assignment gate added last round could be re-entered while still inside an unquoted value, so the second `=` read as a fresh assignment and the quote after it opened a run into the prose. Fixed by giving the walk an explicit `unquotedValue` state — quotes and `=` inside one are characters. Codex's own example (`a="b`) turned out not to discriminate; the two-`=` form does, and that is what the test uses.
- **Constructs with no parseable tag name** (`< =a="b>…`, `<!x a="b>…`). **This was the guard I deleted last round as redundant** because no mutation of it failed a test. It was not redundant; my tests simply did not reach it. Restored, and now pinned. The lesson is the one that keeps recurring here: a mutation that fails no test means the tests are silent about the rule, not that the rule is dead — and last round I read it the second way.
- The `!`/`?` declaration check *is* genuinely redundant, and this time by an argument rather than by absence of evidence: neither character can ever be a tag-name character, so the tag-name requirement already refuses both. Removed, with the reasoning in the docblock.
- `findTagEnd` was rewritten as an explicit four-state walk (`beforeAttributeName`, `attributeName`, `beforeValue`, `unquotedValue`), which is what the unquoted-value fix needed and also cuts the cognitive complexity SonarCloud most likely flagged on `5262435`.
- **Re-fuzzed at 500,000 adversarial inputs**, with the generator extended again — embedded `=`, missing separators, bare `=`, declarations — and with dangerous containers carrying a detectable body. With every container properly closed: **#297 loses prose 0 times, and leaks dangerous content 0 times against `main`'s 55,125.** Containment is strictly better than `main`, not merely unchanged.
- One deliberate cost, asserted rather than left silent: a dangerous container written self-closing and never closed (`<iframe/>Story text.`) now takes the rest of the document with it. A browser does the same, and it is the safe direction for a sanitizer; the alternative is the leak above. That family accounted for the entire prose-loss figure before the containers were closed.
- Two measurement traps caught in this round, both the same shape as last round's: an early harness credited a leak to `#297` that came from the generator concatenating `style`+`title` into a tag named `styletitle`, and the prose-loss metric counted words the fixed reader correctly dropped from *inside* a dangerous container. Both vanished once the generator stopped producing them; neither was a defect in the code.

Third review round (Codex, on `631f43a`) — one P2, real, and the **same containment class as the last round's**:

- **`isSelfClosing` was inferred from the token's suffix**, not from where the `/` actually sits: `parseHtmlTag` asked whether the last non-whitespace character before `>` was a `/`. Two tags satisfy that without being self-closing, and HTML keeps the contents of both — `<svg title="a>b" data-x=y/>` (the `/` is the last character of the unquoted value `y/`; nothing ends an unquoted value but whitespace or `>`) and `<svg title="a>b"/ >` (a marker is the two characters `/>`; whitespace between them makes the `/` a stray). Both put the SVG's contents into the export as story prose. Reproduced against `main` first: `main` keeps them contained, again only because its truncation removed the `/` before `parseHtmlTag` ever saw it.
- Fixed by locating the slash with the attribute walk rather than by reading the end of the string — `closesWithASelfClosingSlash` reuses `findAttributesStart`, `findAttributeValueEnd` and `nextAttributeState`, and the state type is now shared as `AttributeScanState`. A `/` closes the tag only where it sits immediately before the `>` and outside an attribute value; markup the walk cannot read keeps the older suffix reading.
- **This is the second round in a row where the finding was "reading the tag whole removed an accident that was protecting containment".** The pattern is worth naming: every place `main` was safe *because* of the truncation is a place this slice has to make safe on purpose, and both the tag's `>` and its `/` turned out to be such places. Anything else inferred from a tag's raw text is the next candidate.
...
Twelfth round (Codex, on `14912e2`) — **the breakout the eleventh round added leaks the container it is skipping**, and the round that added it measured containment with a corpus that could not see it:

- **The defect.** While skipping `svg` or `math`, the breakout ends the skip at any HTML start tag on the spec list. Inside an *integration point* that is wrong: the children of `<foreignObject>`, of SVG's `<desc>` and `<title>`, and of MathML's `<mi>`/`<mo>`/`<mn>`/`<ms>`/`<mtext>` are HTML **within** the foreign subtree, and a parser stays inside it. So `<svg><foreignObject><p>secret</p></foreignObject></svg>` ended the skip at the `<p>` and exported `secret` in all five formats. `main` drops it, so this was a containment regression against the reader this module replaces — the same category as the eleventh round's, in the fix for the eleventh round's.
- **Fixed** by counting open integration points while skipping and breaking out only at depth 0. Counted rather than flagged, because they nest: an `<svg>` inside a `<foreignObject>` may carry another, and a flag cleared on the first closing tag breaks out inside the outer one. A self-closing `<desc/>` opens nothing and must not hold the skip open; both are asserted.
- **The breakout itself is correct and stays.** A browser really does put `<svg><p>text</p></svg>`'s paragraph *outside* the SVG. That is worth stating because my first repair here was to delete the breakout entirely and fall back to ending the skip at the container's closing tag. It removed the leak and cost 56 of the 56 stories the eleventh round had recovered — I had written the corpus with a leak metric that scored `<svg><p>x</p></svg>` as a containment failure, when parse5 says the paragraph is a sibling of the `<svg>` and exporting it is right. **The metric was wrong before the code was.** Caught by checking the DOM shape rather than the rendered text, and the blunt fix was discarded.
- **`annotation-xml` is the case that cannot be decided from a tag name**, and both spellings are asserted rather than one being quietly preferred. It is an integration point only when its `encoding` is `text/html` or `application/xhtml+xml`. Carrying that encoding, breaking out exports MathML content `main` drops — a containment regression. Unadorned, holding the skip drops text a browser shows — but `main` drops it too, so only an improvement is forgone. One spelling costs containment against `main`, the other costs an improvement over it; this module drops `math` for containment, so it is counted as an integration point unconditionally and the forgone improvement is the accepted cost.
- **Measured against `parse5`, 1,188 inputs**, leak defined as exporting text a browser keeps *inside* the dropped element and loss as dropping prose a browser shows outside it. 996 place the marker strictly inside a dropped subtree.

  | | leaks | drops prose a browser shows |
  | --- | --- | --- |
  | `main` | 96 | 108 |
  | branch before the breakout (`416d2e0`) | 0 | 216 |
  | branch with the breakout (`14912e2`) | 24 | 72 |
  | **here** | **0** | **72** |

  Strictly better than all three predecessors on both axes: no reader of this module has leaked less, and none of the eleventh round's recovered stories is given back.
- **The eleventh round's containment claim was false when written.** It reported *"1,980 adversarial containment inputs: 0 new leaks introduced ... the fixed reader leaks in 0 cases"*, and the commit it reported on leaks in 24 of the 996 inputs above. That harness was not committed, so I cannot say what it contained — only that it did not contain an integration point, because the construct the new code turns on is the one thing it had to include and a leak would have been unmissable. **A containment corpus that cannot express the construct under review reports a clean zero and measures nothing.** This is the fourth time in this slice a corpus's blind spot has hidden the defect under review, and the second time a *metric* rather than a sample was the thing at fault — mine, this round, before the code was.
- **Four counterfactuals, three of which discriminate**: removing the integration-point check reproduces the reported defect; counting a self-closing `<desc/>` holds the skip open for the rest of the container; dropping the non-negative clamp lets a stray `</desc>` push the count below zero, so the next real `<desc>` reads as depth 0 and leaks. The fourth — resetting the count when a skip *begins* — no test could fail, and rather than keep an unfalsifiable guard I checked why: a skip ends either on its own closing tag, which already zeroes the count, or on a breakout, which can only fire at zero. It is redundant on the invariant, not merely untested, so it was removed and the invariant written down where it holds. That distinction is the one this log recorded getting wrong in the other direction two rounds ago.
- Non-claim: the `annotation-xml` encoding case is decided by policy, not detected. Comments inside tags and CDATA are still unhandled, and the `<` -in-a-quoted-value limit (row 4 of #296) is unchanged in both directions.
- **SonarCloud's quality gate failed on the first push, and the regression was mine.** `B Maintainability Rating on New Code`, with no annotations and `output.text` empty; `sonarcloud.io` is egress-blocked here, so the finding could not be read. Rather than guess — the failure mode recorded two rounds ago — I reproduced the metric locally with `eslint-plugin-sonarjs`, which implements Sonar's own rule. `removeNonStoryHtml` cognitive complexity: **36 on the branch before this change, 47 after it.** Mine, and a fair complaint: the integration-point counting went into a loop body that was already three levels deep.
- Fixed by extracting the skip into a `SkipState` and three named functions — `advanceSkip`, `countIntegrationPoint`, `countSkippedBlock`, `openSkipIfBlock` — which drops `removeNonStoryHtml` below the threshold entirely. **Proved a pure refactor rather than assumed one**: 14,112 inputs across both exported readers, 0 differences against the pre-refactor implementation. The parse5 table above is unchanged by it, and a fourth counterfactual (never counting integration points) now discriminates alongside the other three.
- The one remaining `sonarjs` finding in the file, `sanitizeStoryHtmlForExport` at 17, is **pre-existing on the branch and on `main`** and is not in any line this diff touches. Left alone rather than widening the PR.
- Validation: `npm run test:all` exits 0; API function typecheck clean under preflight's exact command; function count 8/12 unchanged; `git diff --check` clean. Tests extend `tests/export-sanitizer.test.ts`, so `test:all` gains no entries.
- **Recovery CI does not run on this PR**, because `recovery-ci.yml` is scoped to `pull_request: branches: [main]` and this targets a `claude/*` branch; the same base choice disables CodeRabbit. `scripts/recovery/preflight.sh --skip-status` was therefore run locally instead and exits 0. Recorded because four green checks on a PR whose validating workflow never fired is exactly the kind of thing this log exists to keep honest.

## 2026-08-28 UTC - Story Lab Multi-Chapter Batch Generation Vs. The 60-Second Function Budget

Actions:

- Made both chapter-generation loops (`generateChaptersForStory` for genesis, the offset loop in `continueChapter`) check `getRemainingRequestBudgetMs` before starting any chapter after the first. `xaiConfig.ts` built this function-budget system against the 60-second window `vercel.json` kills a function at, and it had been wired into exactly two places — `XaiTextClient`'s fast-profile retry and continuity extraction — never into the chapter loops themselves. Each extra batch chapter got a fixed 9-second timeout (`EXTRA_BATCH_CHAPTER_TIMEOUT_MS`) with no check of real elapsed time: if chapter 1 needed its own fallback retry, it alone could burn most of the 60-second budget, and the loop started chapter 2 anyway with a fresh timeout the platform would never let finish. That is not a catchable failure — a platform-level SIGKILL terminates the process mid-await, so `finishJob` never runs and a durable (Postgres) job is left at `status: 'running'` forever with no expiry/GC anywhere in `postgresStoryLabJobStore.ts`.
- Below the floor a fast-profile call needs to be worth starting (`MIN_XAI_FALLBACK_TIMEOUT_MS`, reused from `xaiTextClient.ts` rather than re-declared), the loop now stops itself and records the remaining chapter numbers into the existing `failedChapters`/`partialFailures` mechanism with a "Skipped: insufficient time remaining..." message, instead of attempting a call that cannot safely finish. Each attempted batch chapter's own timeout is now `Math.min(EXTRA_BATCH_CHAPTER_TIMEOUT_MS, getXaiFastTimeoutMs(), remainingBudgetMs)` instead of the static ceiling alone.
- Sourcery review caught that the *first* chapter's own primary call was exempted from all of this — it still got `getXaiPrimaryTimeoutMs()`/`getXaiFastTimeoutMs()` unconditionally, so a deployment configuring a smaller `STORY_LAB_FUNCTION_BUDGET_MS` (or any meaningful pre-call overhead) left it exposed to the same platform SIGKILL. `callGrokAI`/`callGrokAIForContinuation` now take the request start time directly, refuse before ever calling the provider once the remaining budget is below the safe floor (rather than risk passing `timeoutMs: 0`, which axios reads as "no timeout" rather than "expired"), and cap both the primary and fast-fallback timeouts to the actual remaining budget.
- Codex review then caught that `StoryService.generateStory`/`continueChapter` were resetting their own `Date.now()` as that request clock, but the Story Lab job routes reach them well after the invocation actually began — job-store creation, an owner lookup, and content-boundary loading all run first in `jobRouteHandlers.ts`. `generateStoryLabGenesis`/`continueStoryLab` already captured their own `requestStartedAtMs` ahead of that work (the same timestamp already fed to continuity extraction's budget) but never passed it down. Both `StoryService` methods now take an optional `requestStartedAtMs` (defaulting to `Date.now()` for direct/test callers), and the engine passes its own timestamp through.
- Follow-ups flagged rather than fixed here: job-store staleness/GC for already-stuck durable rows from *before* this fix (a bigger, cross-cutting change to the store interface — this PR prevents new stuck jobs, it doesn't reap old ones); and the remaining gap between the true Vercel invocation start and the engine's own timestamp, which is `jobRouteHandlers.ts`'s own pre-engine job-store work (job creation, owner lookup, content-boundary loading) — threading a route-level start time that far down would mean changing `StoryLabJobRouteContext`'s interface, a larger change than this slice.
- Also flagged, not fixed here: the Clerk auth port (`configuredAuthPort.ts`/`clerkAuthPort.ts`) is permanently inert even when `STORY_LAB_AUTH_PROVIDER=clerk` is set, because no real token verifier is ever wired into the production singleton — silently deadening the entire Postgres-backed account/project/job/profile stack behind it. This is the same "Cloud Account/Profile system" area a previous slice already flagged as needing a human product/security decision (device-auth vs. documentation-only) rather than something to resolve unilaterally in one cycle.

Self-review:

- Added `tests/story-service-batch-chapter-budget.test.ts`, driving a fake `Date.now()` clock to simulate a slow chapter 1 without a real ~55-second wait: covers the genesis and continuation loops both degrading to a partial success within budget instead of attempting a doomed later chapter, skipped chapters reported with correct chapter numbers and a "skipped" message rather than a generic failure, an attempted-but-tight batch chapter getting a timeout capped to its actual remaining budget, the first chapter refusing cleanly (not with a degenerate `timeoutMs: 0`) once the configured budget is already exhausted, the first chapter's own timeout being capped under a tight-but-sufficient budget, and the Story Lab engine actually threading its own `requestStartedAtMs` into `StoryService` rather than leaving it to reset the clock.
- Updated `tests/verify-ai-fixes.test.ts`'s literal-string assertions (`apiUsesPrimaryTimeoutHelper`/`apiUsesFastFallbackTimeoutHelper`), which had asserted the old unconditional `: getXaiPrimaryTimeoutMs()`/`fallbackTimeoutMs: getXaiFastTimeoutMs()` substrings; they now assert the budget-capped form, preserving the test's original intent (both helpers are still read and still feed the request) rather than its literal shape.
- `npm run test:all`'s relevant suites (`story-service-improved`, `xai-fast-path-review`, `verify-ai-fixes`, `story-continuation-cliffhanger-variety`, `story-service-prompt-guards`, `story-route-contracts`, `trope-subversion`, `cliffhanger-service`, `story-service-batch-chapter-budget`, `story-lab-real-engine`, `story-quality-evals`, `story-lab-continuity-merge`, `story-lab-continuity-prompt`, `story-lab-continuation-guidance`, `story-lab-state-builder`) all pass. The full `npm run test:all` script itself could not be run in this environment (missing `tsx`/`@neondatabase/serverless`/`typescript` in `node_modules`, confirmed pre-existing on unmodified `main` via `git stash`), so each suite was run directly via `npx tsx`.
- No contract/frontend changes — `failedChapters`/`partialFailures` were already part of the response shape and already rendered.
- Rebased onto #290's correlation-id work, which landed on `main` while this was in review and added its own second parameter (`correlationId?: string`) to these same two `StoryService` methods. Resolved by keeping `correlationId` as the second parameter (matching #290's call sites) and adding `requestStartedAtMs` as a third, defaulted parameter after it, rather than either PR's shape winning outright.
- Codex review then caught a deeper version of the same clock problem, inside `XaiTextClient` itself: `resolveFallbackTimeoutMs`'s own `startedAtMs` was captured fresh at the top of `generateText`, not passed in from the caller — so even with `StoryService` now capping the primary attempt's `timeoutMs` correctly, a primary attempt that spent its own (correctly capped) timeout failing could still be followed by a fast-profile retry that `resolveFallbackTimeoutMs` believed had most of a fresh 60-second window to work with, because its call-local clock had no way to see the pre-call overhead `StoryService`'s own `requestStartedAtMs` already accounted for. Concretely: 50 seconds already elapsed, `StoryService` correctly caps the primary attempt to the ~5000ms truly left, the primary attempt spends that whole 5000ms failing, and `resolveFallbackTimeoutMs` — measuring from its own start rather than the true one — computes `60000 - 5000 - 5000 = 50000ms` "remaining" and grants a retry with none of the true budget left. `XaiTextRequest` now carries an optional `requestStartedAtMs`, `generateText` uses it in place of its own `Date.now()` when the caller supplies one, and both `StoryService` call sites now pass their own true `requestStartedAtMs` through. Added `assertFallbackRetryIsSkippedWhenTheTrueBudgetIsAlreadyGone` to `tests/story-service-batch-chapter-budget.test.ts`, driving `XaiTextClient` directly with a mocked `axios.post`; counterfactual (reverting `startedAtMs` to `Date.now()`) run and confirmed failing at "no fallback retry should be attempted... (2 !== 1)" before the fix was reapplied.

## 2026-08-27 UTC - A Cliffhanger Signal That Could Not See Its Own Name, An Ending Read With Whitespace Nothing Else Keeps, And The One Classic Theme Its Own Scan Filtered Out

Actions:

- Replaced the two entries of `EXPLICIT_CLIFFHANGER_PATTERN` that named ordinary narrative vocabulary. Between them they inverted the signal they exist to raise. **`cliff` could not match `cliffhanger`**: `wordFormAlternationPattern` accepts an inflection from `WORD_INFLECTION_SUFFIXES`, and `hanger` is not one, so the boundary at the end of the alternation rejected the compound outright — which left the rock face as the only thing the entry ever matched, in a genre that writes coastlines. And **`continued` is one of the commonest verbs in this prose**: `he continued down the hall` collected the signal in full, and the entry bought nothing besides, because the phrase `to be continued` beside it already contains it. So the lexicon is `['cliffhanger', 'to be continued']` — the two spellings that actually announce an ending as a cliffhanger. Measured: `they stood on the cliff` and `he continued down the hall` each scored `Ending uses explicit cliffhanger language.` before this and score nothing now; `the chapter ends on a cliffhanger` scored nothing before and scores it now. This is the class of defect `EMOTION_FAMILIES` above it was fixed for — `anger` inside `danger`, `hope` inside `hopeless` — arriving through a lexicon entry rather than through a substring match.
- Made `scoreCliffhangerQuality` read its paragraph with the whitespace collapsed, like the other six dimensions. The final paragraph came straight out of `splitStoryIntoTextBlocks`, whose blocks keep whatever whitespace the generator wrote inside them, so this was **the one scanning dimension of the seven that had never been through `collapseWhitespace`** — and the inconsistency was inside the one function, since the fallback for a story with no blocks at all *is* the collapsed text and the branch every real story takes was not. What it costs is the multi-word entry: `to be continued` is escaped literally, single spaces and all, so an ending the generator wrapped between the words — `To be\ncontinued` — was invisible to the pattern named for it. Same failure `CLIFFHANGER_HOOK_PATTERNS` collapses its scanned paragraphs for, and same one `extractPlotThreads`'s `UNRESOLVED_QUESTION_PATTERN` names. It was unobservable until the entry above came out, because `continued` matched either spelling for the wrong reason.
- Lowered the theme-word floor in `scoreContinuity` by one letter, which is what `sin` needed to be reachable. A theme arrives as an id and is split on its separators, so the filter exists to drop the connective the split exposes — the `to` in `enemies_to_lovers`, which appears in every story ever written. Every connective in either vocabulary the seams accept is two letters; the floor was `word.length > 3`, one letter above that, and **`sin` is exactly three letters and is one of the eighteen classic themes**. It was therefore filtered out of its own word list, and `[].some(…)` is `false`, so a story of sin, sinners, and damnation scored `Few configured story anchors were detected` and lost the twelve points a matched theme is worth — on the one dimension that exists to say the story kept its configuration's promise. Same shape as the six themes `extractThemesFromContent` could not report before they were given keywords: a member of a closed set that the scan reading it cannot reach.
- Extended `tests/story-quality-evals.test.ts` with three functions and registered all three in `main()`. No new test file, so `test:all` gains no entries.

Self-review:

- All three have semantic counterfactuals, all three were run against the suite rather than against a scratch harness, and all three were reverted immediately. Restoring `'cliff', 'continued'` fails at `an ordinary "continued" is not a cliffhanger announcement`. Dropping the `collapseWhitespace` call fails at `the same phrase wrapped between its words is the same ending (signals=[])` — with the inline spelling still passing in the same test, which is what makes the whitespace and not the phrase the thing being proved. Restoring `word.length > SHORTEST_MEANINGFUL_THEME_WORD` fails at `the shortest classic theme is still a theme`. None of the mutations is committed.
- The whitespace fix is written on the line the previous entry explicitly declined to touch. That non-claim said `scoreCliffhangerQuality`'s assign-once-per-paragraph loop "is a `paragraphs.at(-1)` written the long way and it is correct; changing it is a readability edit with no defect behind it." The loop was correct and still is — reaching the last block is not what was wrong. What was wrong was what the function then did with the block, and the defect is in the same expression, so the readability edit comes along with it rather than being done for its own sake.
- The theme fix is a floor on length, not a stop-word list, and the test pins both directions: `sin` echoes, and `enemies_to_lovers` does **not** echo on a story that merely says `to`. Non-claim: a caller-sent theme phrase containing a three-letter function word (`cat and mouse`) can now echo on that word. Neither vocabulary the app itself sends — the eighteen classic themes or the twelve Story Lab seeds — contains one, and the field is typed `string[]` for callers outside them.
- Non-claim: nothing here changes a prompt, a generated story, the routing table, or the deployable function count. `buildStoryQualityHeuristicReport` is advisory and its output travels as `heuristicReport` on `/api/story-lab/evaluate`; it is read by the Proving Grounds, not by any generator. What changes is the number two prompt variants are compared by there, which is exactly what an advisory scan is for.
- Non-claim: the remaining `EXPLICIT_CLIFFHANGER_PATTERN` entries still run through `WORD_INFLECTION_SUFFIXES`, so `cliffhangers` matches and `cliffhanging` does not. That is the same tolerance every other fixed list in the file gets and it is not widened here.
- Non-claim: this slice does not touch `UNRESOLVED_HOOK_WORD_PATTERN` beside it, whose eight hook words are single words already read at the right boundary, nor the score arithmetic of any dimension. Only what the two scans can see changes.
- Validation: `npm run test:all` exits `0` across every registered suite. The Vercel API typecheck is clean (`exit 0`) under the repo's pinned `tsc` with the exact file list and flags `scripts/recovery/preflight.sh` uses. `scripts/recovery/check-vercel-function-count.sh` reports 8/12, unchanged from `main`. `git diff --check` is clean over the files this slice touches. No test here reaches a store, an auth port, or a model provider: `buildStoryQualityHeuristicReport` is a pure function of a string and a configuration object. A root install was needed to run any of it; the tracked `node_modules` tree the install dirtied was restored and is not in this diff, as the previous entries record.
- Conflict note: this slice touches neither `tests/request-correlation-id.test.ts` nor any file open PRs #290 and #292 change, so it can merge in any order relative to them.

Review follow-up (Codex, three findings on `30bc976`; all three were real as *facts*, two were fixed in `5f3b168`):

- **P2, fixed — a label a `<br>` splits across blocks.** Introduced by this slice, and the same defect as the whitespace fix arriving through a door I had not checked. `BLOCK_LEVEL_TAG_NAMES` lists `br`, so `<p>To be<br>continued</p>` arrives as the two blocks `To be` and `continued` and a final-block scan sees only the second. That scored before this slice **only** because the bare-verb entry matched it, on the same word that was also scoring `he continued down the hall` — so removing the false positive is what exposed the real one under it. My non-claim for the whitespace fix was narrower than the claim I made for it: a raw newline is the half of the failure that stays *inside* a block, and no amount of collapsing reaches a break *between* two. `readEndingLabelText` walks back through the trailing blocks shorter than the longest label — the fragments a break leaves — and stops on the first block long enough to be prose. A story whose last block is an ordinary paragraph stops on the first step and reads exactly as before. Counterfactual, run and reverted: putting the label test back on the final block alone fails at `a label a <br> split across two blocks is the same ending (signals=[])`. The widening is pinned in the other direction too, because a short final block is what makes the walk take in the paragraph before it: `<p>He continued down the long dark hall.</p><p>She ran</p>` must not score, and the assertion says so.
- **P1, fixed — the documentation map.** `AGENTS.md` L114 routes story-quality eval changes to `GROK_MULTIAGENT_STORY_LAB_POLISH_EXEC_PLAN.md`, and this slice had recorded itself only here. The three repairs are now in that plan's Progress list, with the evaluator limit below as a Surprises entry, since that plan is where `AGENTS.md` sends the next reader of the evaluator.
Second review round (Codex, on `5f3b168`); both fixed in the same commit as this entry:

- **P2, fixed — my own `<br>` fix bought its true positive with a false one.** Judging the join by block length alone — "the final block is too short to be prose, so read the one before it too" — means `<p>The chapter was a cliffhanger.</p><p>She ran</p>` scores the signal and its sixteen points, on a label sitting wholly inside the *previous* paragraph and an ending that announces nothing. I had spotted this widening while writing the fix, decided it was acceptable, and wrote it up as a non-claim; that was the wrong call, and the review was right to press it. Reconstruction is now defined as what it always should have been: the join is read **only for what no single block already says**, so a match present in the joined text and in none of the blocks it was joined from is the only thing that counts as a label a boundary broke apart. A label inside one block stays that block's business and is still reported by the final-block test wherever in the block it sits. Both counterfactuals run and reverted: dropping the "in no single block" rule fails at `a label in the paragraph before the ending is not this ending's`, and removing the reconstruction entirely fails at `a label a <br> split across two blocks is the same ending (signals=[])`.
- **P1, fixed — `[x]` on work that is not merged.** `AGENTS.md` L34 is explicit that "merged to `main`" is the only meaning of `Done` and that open-PR work must be labelled `In review`. Both entries this slice added to the plan's Progress list were checked. They are now `- [ ]` and read `` `In review` (PR #293, not merged) ``. The publication discipline is the one rule in that file whose whole point is that a plan should not be able to overstate what has shipped, and I broke it in the same commit that was answering a finding about that plan.

Third review round (Codex, on `74947f5`), fixed in the same commit as this entry:

- **P2 — a reconstructed label has to reach the ending, not merely sit near it.** My second rule ("the match is in no single block") closed the label-in-the-previous-paragraph case and still let through `<p>To</p><p>be continued</p><p>She ran</p>`, where the label genuinely is split across blocks, genuinely is in none of them alone, and stops one block short of the ending. Reproduced first: signal present, dimension at 66. The condition that actually says what a closing label *is* — it does not merely come near the end, it **is** the end — is that the last fragment is load-bearing: the join carries a label and the join without its final piece does not. A label that survives dropping the ending was never the ending's. That **subsumes** the previous rule (a label wholly inside an earlier block survives the drop), so it replaces it rather than stacking a third condition on two, and the function is shorter than the version it corrects. Twelve label cases are pinned now, including a label broken into three blocks — the case easiest to lose while excluding the one above. Counterfactuals run and reverted: dropping the load-bearing rule fails at `a label in the paragraph before the ending is not this ending's`; removing the reconstruction entirely fails at `a label a <br> split across two blocks is the same ending (signals=[])`.
- Worth recording as a process note rather than a defect: three rounds of review on one twelve-line function, each round correct and each finding a case the round before had not. The pattern was mine, not the reviewer's — twice I proved a rule against the examples I had thought of and stopped, rather than asking what the rule *claims* and testing that. "The ending announces itself" is a claim with a sharp edge, and each weaker rule was a proxy for it (block length, then block membership) instead of the thing itself.

Fourth review round (Codex, on `50d6e62`), fixed in the same commit as this entry:

- **P2 — the collection step was still a proxy, and it was the last one.** Rounds two and three both corrected what *counts* as a reconstruction while leaving *how far back to look* as a walk that stopped at the first block long enough to read as prose. A fragment is free to carry text of its own, so length never distinguished the two: `<p>To be<br>continued in Chapter Two</p>` leaves `continued in Chapter Two` as the final piece, the walk stopped there, never reached `To be`, and the split spelling scored nothing — while the identical inline sentence scored. Same sentence to a reader, two different answers. The span is now read off the labels themselves (`MOST_BLOCKS_ONE_LABEL_CAN_SPAN`, from their word counts: a boundary falls only between words, so a three-word label survives at most three pieces), which deletes the character-length constant entirely. Fifteen label cases pinned. Counterfactuals run and reverted: restoring the length-bounded walk fails at `a <br> before a long remainder is still the same label (signals=[])`, and dropping the load-bearing rule still fails at `a label in the paragraph before the ending is not this ending's`.
- Recorded as a known limit rather than fixed, in the safe direction: an unrelated earlier label defeats the drop test, so `<p>The cliffhanger was real.</p><p>To</p><p>be continued</p>` reports nothing. A missed signal rather than a false one, which is the error an advisory scan should prefer, and it is in the docblock rather than only here.
- Amending the round-three process note now that there were four: every one of the four findings was a proxy failing, and the same proxy twice. `EXPLICIT_CLIFFHANGER_PATTERN`'s original `cliff` entry was a proxy for `cliffhanger`; block length was a proxy for "this is a fragment"; block membership was a proxy for "this label is the ending". Each time the fix was to state the thing itself and delete the proxy, and each time the resulting code was *shorter*. The review did not find four unrelated bugs — it found one habit four times.

Sixth review round (Codex, on `489ab5f`) — **the `<br>` reconstruction is withdrawn**, and the slice is better for it:

- **P2, and the one that settles the mechanism.** `<p>She wanted to be</p><p>continued through the next trial.</p>` scored the explicit-cliffhanger signal and its sixteen points. Two ordinary paragraphs of prose, and joining them *synthesises* a label out of words that were never one. Reproduced before changing anything, on two separate sentences. This is not the missed-signal category of round five: it is a **false positive on ordinary prose**, which is precisely the defect fix 1 of this slice removed the bare `continued` entry for — arriving by a different route, through a mechanism I added.
- **It cannot be repaired at this level, and the reason is structural.** The join is only ever safe across a `<br>`; by the time blocks reach the scan, `splitStoryIntoTextBlocks` has erased which boundary was which, because a `<p>` break and a `<br>` break are both `\n\n`. Codex's own suggestion — restrict to fragments from the same rendered paragraph — requires boundary provenance the splitter does not carry, and adding it changes a module every scanner in the repository reads. That is a slice of its own, with its own blast radius.
- **So the reconstruction is removed rather than patched a fourth time.** The tally it earned: four findings against it across rounds two, three, four and six, three of them false positives, in exchange for catching one markup shape (`<p>To be<br>continued</p>`) there is no evidence the generator emits — a shape I introduced the mechanism for only because removing the bare `continued` exposed it. The dimension is back to reading the final block, which is what it did before this slice and has none of these failure modes. `hasReconstructedCliffhangerLabel`, `MOST_BLOCKS_ONE_LABEL_CAN_SPAN` and `EXPLICIT_CLIFFHANGER_LABELS` are gone with it; the file is shorter than at any point since round one.
- **The gap is now an asserted limit rather than a silent one.** A `<br>`-split label is a *missed* signal — the direction an advisory scan should err in — and `testTheScanDoesNotSynthesiseALabelFromProse` pins both halves: prose must not be joined into a label, and the torn label is a miss. Reintroducing the join now has to argue with a test rather than slip past one. The half of fix 2 that never depended on joining blocks is untouched: a break *inside* one block is still handled by `collapseWhitespace`, and `To be\ncontinued` still scores.
- Counterfactual, run and reverted: restoring the reconstruction fails at `two prose paragraphs must not be joined into a label`.
- Correction to the record: round one's finding was answered "fixed", and that answer no longer holds. The `<br>` case is now a stated limit. Said plainly on that thread rather than left to be inferred from a later commit.

Fifth review round (Codex, on `6e9cfa9`) — one fixed, two declined, and the round where the findings stopped converging:

- **P1, fixed — `In review` on work this PR does not do.** The parked theme-vocabulary limit was labelled `In review (PR #293)`, which after this merges would read as "reviewed in that PR" for a repair the diff deliberately leaves alone. `AGENTS.md` L32-34 has the right state for it and it is `Parked`. Relabelled with what it actually is: a follow-up slice, not started, surfaced here.
- **P2, declined — restore a bare `continued` for label-position uses.** Factually right: `<p>Continued in Chapter Two</p>` and `<p>Continued next week</p>` score 50 with no signal. But acting on it means partially reinstating the entry whose removal *is* fix 1 of this slice, and the distinguisher offered — "label position" — does not exist in this code or anywhere near it. Deciding what makes a paragraph a label rather than prose is a design question about the scan, not a defect in it.
- **P2, declined — permit a suffix block after a reconstructed label.** Also factually right, and the reason it is declined is exact rather than a matter of taste: **it asks for the opposite of round three on structurally identical input.**

  | blocks | round 3 required | round 5 requires |
  | --- | --- | --- |
  | `["The door opened.", "To", "be continued", "She ran"]` | reject | — |
  | `["To be", "continued", "in Chapter Two"]` | — | accept |

  Both are label fragments followed by a trailing block, and in both, dropping the trailing block leaves the label intact — which is precisely the load-bearing test round three demanded. Nothing structural separates them. What separates them is whether the trailing block *continues the annotation* or *begins a new ending*, which is a semantic judgement the scan has no notion of. A rule satisfying both would have to be a notion of what an annotation looks like, i.e. the same design question the finding above raises.

**Stopping here rather than pushing a sixth time.** Four rounds converged: each fix was narrower than the last, and round four deleted the mechanism that generated rounds two and three. Round five does not — it reopens fix 1 and contradicts round three. That is the "findings stop converging" condition, so what is still flagged is raised on the threads and left open for the author instead of coded around. Both are real observations about an advisory heuristic and neither is a defect this slice introduced; the endings they name score 50 rather than 66 on a dimension that is one seventh of an advisory score.

- **P2, answered not fixed — the `sin` theme's vocabulary.** The facts check out, measured word by word: with `themes: ['sin']` the scan echoes `sin` and `sins`, misses `sinful`, `sinner`, `damnation`, `damned` and `penance`, and newly admits `sines`, which is a real if tiny new false positive. But it is not about `sin`: the same run with `themes: ['betrayal']` misses `betrayed`, because `WORD_INFLECTION_SUFFIXES` is inflectional **by design** — the docblock excludes `less` and `ly` for building different words, and `ful` and `ner` are that same category. So `sin` is a regular case of a deliberately narrow rule, not an irregular case forced through a regular one; before this slice it was the one theme unmatchable at all, and it is now matched exactly as well as its seventeen siblings, which is the whole of what was claimed. The proposed fix — read `storyContentAnalysis.ts`'s `THEME_KEYWORDS` — covers the eighteen classic themes only, while `scoreContinuity` reads `configuration.themes` as an un-normalised `string[]`: the picker sends twelve *seed* ids, seven of them on no classic list, and `/api/story-lab/evaluate` accepts any short string. It would therefore change matching for eighteen values, do nothing for the other twelve or for arbitrary input, and cross into story-quality behaviour well past a slice about three blind spots. Filed in the plan doc as its own slice with what it has to settle first; the thread is left open, because spending a slice on it is the author's call.

## 2026-08-27 UTC - The Two Routes That Could Not Have A Correlation Id Without Also Becoming POST-Only (#291)

Actions:

- Split the id-settling half out of `beginPostRoute` as `settleRequestCorrelationId`. Reading the caller's `X-Request-ID` and echoing it back were the opening two lines *inside* `beginPostRoute` and reachable only through it, which made a correlation id something a route could have only by also adopting POST-only method dispatch — the helper answers `405` for anything but `POST` and pairs its `Allow` header with the CORS preflight list. `/api/story-lab/jobs` serves a `GET` event stream and `/api/story-lab/account` serves `GET`, `PUT`, `POST` and `DELETE`, so neither could adopt it without changing what it serves, and both were therefore left as **the only two routes in `api/` that settled no id at all**: neither read `x-request-id`, neither set `X-Request-ID`, and neither had a value in scope to log under. This is option (1) of the two #291 names, and the one `postRoutePreamble.ts`'s own docblock argues for — option (2) would have spelled the read-and-echo pair a third and fourth time, which is the shape the duplication gate already objected to once. `beginPostRoute` is now composed of it plus the POST rules, so every route that had an id keeps the same one by the same rules. `X-Request-ID` is named once as `REQUEST_CORRELATION_ID_HEADER` beside the rules for what may be written into it; `corsPolicy` already lists that name twice, as a request header a caller may send and as a response header a browser may read back, and a route spelling it a fourth time could disagree with all three.
- Settled the id once per request on both routes, at the composite entry each one's paths are reached through. Both files dispatch internally — the jobs route on `OPTIONS`/`POST`/events/`GET`, the account route on profile/projects/project — and on both deployments every path arrives through the one entry function: `vercel.json` rewrites `/api/story-lab/jobs/:jobId[/events]` and the account paths onto these route files, and `API_ROUTES` mounts all three job paths and all three account paths on the same two handlers. Settling again inside a sub-handler would mint a **second** id for the same request and overwrite the header with the one the log lines do not use, so the id is settled at the entry and handed down as an argument. The three individually-exported job handlers settle their own, since each is an entry when called alone. It is settled *before* each route's CORS branch, matching `beginPostRoute`'s order, so an `OPTIONS` answer carries the id too — and the jobs route answers its preflight from a branch of its own, which is the case that ordering exists for.
- Stamped the jobs route's two log lines with it. `runJobWork`'s `logError` (the throw out of job work that a durable job's `running` row is recovered from) and `jobStoreUnavailable`'s `logWarn` (the `503` a reader most often comes back asking about on this route) both carried an `endpoint` and no `requestId`. Both do now, threaded from the settled id rather than read again. Those are every line either file writes.
- Extended `tests/request-correlation-id.test.ts` rather than adding a file, so `test:all` gains no entries: both routes echo a usable id and replace an unbounded one, the jobs preflight carries it, and the store-unavailable warning is asserted — through the logger's own buffer — to be filed under the id the caller was echoed.

Self-review:

- Four semantic counterfactuals, each run and reverted immediately; none is committed. Dropping the jobs route's settle for a bare `readRequestCorrelationId` fails at `/api/story-lab/jobs should echo a real correlation id back to the caller`. Dropping the account route's settle fails the same assertion for `/api/story-lab/account`. Removing `requestId` from the store-unavailable warning fails at `the store-unavailable warning should be filed under the echoed id, got undefined`. Moving the jobs route's settle below its `OPTIONS` branch fails at `the jobs route preflight should carry the correlation id as well` — the ordering claim, stated separately from the echo claim because the preflight returns before every other path.
- Both routes are driven through their own pre-auth, pre-store refusal — a jobs `GET` with no job id, an account `GET` naming no resource — which is the shortest path that still reaches the echo and touches no store, no auth port and no provider. Each assertion also names the refusal code it was driven through, so a route that starts answering that request some other way fails here rather than quietly proving the echo on a path the test did not intend to exercise.
- The store-unavailable case is reached with an injected `createJobStoreConfig` rather than by unsetting environment, so the test states which branch it is proving and leaves no process state behind for the suites after it.
- Review follow-up, and a correction to this entry's own first non-claim. Codex raised (P2) that `clerkAuthPort.warnAuthVerificationFailure` calls `readRequestCorrelationId(req)` again on the request it was handed. Settling wrote the id to the *response* only, so `req.headers['x-request-id']` was unchanged and that re-read minted a **second** `req_<uuid>` whenever the caller supplied none — the common case, since no client here sends the header. It is real and it is the sharper half of what I had written down: I claimed the account route "writes no log lines today", which is true of the file and false of the request. An auth failure on the account route, and on the jobs route whenever the store is durable and `requireUser` runs, produces exactly one log line — that warning — and it was filed under an id appearing in no response. The echoed header named the one diagnostic a reader would come back asking about and could not find, which is worse than no header, because it reads as a trace. Fixed by making the verb true: `settleRequestCorrelationId` now writes the settled id back onto the request as well as out to the caller, mutating `req.headers` in place rather than rebinding it so a derived request built from the same object — `withEventStreamAuth` on the jobs route builds one — cannot keep pointing at an old copy. That fixes the auth-port seam, the jobs route's durable path, and all five `beginPostRoute` routes at once, without touching the `AuthPort` interface or any of its callers. A supplied id was never affected: both reads return the same trimmed value, and only the minted case drifted. Pinned by a new assertion driving the account route against a Clerk verifier stubbed to throw — the provider-failure seam, proved without a provider. Counterfactual, run and reverted: removing the write-back fails it with the two ids printed side by side.
- Non-claim, narrowed accordingly: the account route file still writes no log lines **of its own**, so the id in that file is passed to nothing. Its store failures answer `503` and `403` silently, and giving them lines is a logging slice, not this one. What changed is that a line written *for* one of its requests, by the auth port, now carries the id the caller was echoed.
- Non-claim: nothing changes what either route serves, its method dispatch, its `Allow` headers, its CORS policy, its auth, or its rate limits. No route file is added, retired or renamed. This is a correlation and log-hygiene change; it is not an authentication or rate-limiting boundary and does not claim to be one.
- Non-claim: `Access-Control-Expose-Headers` already listed `X-Request-ID`, so browser callers on these two routes can read the new header without a CORS change. That list was not touched.
- Validation: `npm run test:all` exits `0` across every registered suite. The Vercel API function typecheck is clean under the repo's pinned `tsc` with the exact command `scripts/recovery/preflight.sh` uses. `scripts/recovery/check-vercel-function-count.sh` reports 8/12, unchanged from `main`. Root install was needed to run any of it; the tracked `node_modules` tree the install dirtied is restored and is not in this diff, as the previous entries record.

## 2026-08-27 UTC - Two Fields A Type Check Never Reached, A Correlation Id That Stopped At The Route, And The Minting Method Behind It With No Caller

Actions:

- Gave `storyId` and `content` the `typeof` check every field beside them in `ImageService.validateImageInput` already had. They were the last two in that validator guarded by truthiness alone, while `creature`, `themes`, and `imagePrompt` each got an explicit type check as the defect behind them was found. The gap is not cosmetic, because `length` is `undefined` on a number and `undefined < 10` is `false`: a body sending `content: 1234567890123` satisfied `!input.content || input.content.length < 10`, satisfied the route's own guard — `!input.content`, a truthiness test too, so **neither layer caught what the other missed** — and reached `stripStoryHtmlToText`, which throws `storyContent.replace is not a function` into the catch that answers `IMAGE_GENERATION_FAILED`. The caller was told the image service had failed and that the failure was the service's, for a malformed request only the caller can fix. `storyId` failed more quietly: nothing downstream calls a string method on it, so a numeric id was copied into the response envelope's `storyId` and the caller was handed a **success** whose id contradicts the type the contract declares for it. Both are read as `unknown` first, for the reason the `imagePrompt` check beside them already gives: the contract types them as strings and the wire does not, and these checks are precisely about the values the type says cannot arrive. The bare `10` became `MIN_IMAGE_CONTENT_LENGTH`; the number is unchanged and is a floor against an empty body, not a judgement about story length.
- Made the correlation id reach the generation it names. `beginPostRoute` settles an id for every paid POST route — the caller's `X-Request-ID` when it is usable, a minted one otherwise — echoes it back as a header, and stamps it on every line the route writes. It stopped there. `generateStoryLabGenesis` and `continueStoryLab` were called with the request body alone, and `StoryService` opened **both** entry points with an unconditional `logger.generateRequestId()`, so every line describing the work a reader would actually be asking about — the prompt sizes, the provider call this app pays for, the failure that made them ask — was filed under a second id that appears in no response, no header, and nothing the caller was ever told to keep. A reader quoting the id they were handed was quoting the one id that names none of the work. `StoryService.generateStory` and `continueChapter` take the id as an optional second argument and resolve it through `resolveRequestId`, the engine carries it as `StoryLabEngineOptions.requestId`, and both Story Lab routes pass the id they already had in scope. This is the shape `ImageService` and `ExportService` were each given for this same reason, which had left `StoryService` the last of the three still minting its own.
- Retired `StoryService.generateRequestId()`. It was byte-identical to `logger.generateRequestId()` — `` `req_${randomUUID()}` `` in both — and **called from nowhere**: both entry points went to the logger's copy, so the private method beside them had no reader at all. `resolveRequestId` replaces it, which is the same method with the argument that gives it a reason to exist.
- Extended two suites rather than adding files, so `test:all` gains no entries. `tests/image-service.test.ts` drives seven malformed `content` values and eight malformed `storyId` values through the service and pins the length floor from both sides. `tests/request-correlation-id.test.ts` drives both Story Lab handler factories — which take the engine function as their parameter — and asserts the settled id reaches the generation, that a minted id reaches it too, and that `StoryService` reports the id it was handed and mints one only when it has none.

Self-review:

- Four semantic counterfactuals, each run and reverted immediately; none is committed. Restoring `!input.content || input.content.length < 10` fails `image-service` at `content=1234567890123 is a caller error, not a service failure (got IMAGE_GENERATION_FAILED)` — the defect, named by the answer it produced. Restoring `!input.storyId` fails it at `storyId=12345 is a caller error, not a generated image (got success=true, code=undefined)`, which is the quieter half: a success, not an error code. Making `resolveRequestId` ignore its argument fails `request-correlation-id` at `generateStory should report the id it was given`. Dropping `{ requestId }` from the genesis route's engine call fails it at `should hand the settled id to the generation, got undefined`.
- The `generateStory` cases are driven through `validateStoryInput`'s refusal, which returns before any generation. `continueChapter` has **no input validation at all**, so there is no early return to drive it to and it runs a whole generation for whatever it is handed — which is why `tests/request-correlation-id.test.ts` now clears `XAI_API_KEY` at module scope, before any service is constructed, the way `tests/image-service.test.ts` already did. Codex raised this as a P1 on the first push and was right: with a key present the continuation case was a real, billable provider call. Counterfactual, run and reverted: restoring the key attempts 1 live call; with the fix it attempts 0.

Answering the Codex review on the first push:

- **P1, the correlation test could call the live provider.** Fixed as above. The claim in the first PR body that both services were driven through an early refusal was wrong for `continueChapter`, and the docblock that repeated it has been corrected rather than left to mislead the next reader.
- **P2, the correlation id did not reach continuity extraction.** Also correct, and the same defect one call later. A successful Story Lab generation makes *two* paid provider calls — the chapters, then `extractContinuity` — and only the first was correlated. `XaiTextClient` already logs every call's start, latency and failure through `request.context`, and this one passed none, so the timeout or provider error that most often degrades a batch was recorded under no request at all. `ContinuityExtractionInput` now carries `requestId`, `enrichContinuity` forwards it, and both engine call sites pass `options.requestId`. The context names its own endpoint, `story-lab/continuity-extraction`, so the continuity call stays tellable from the chapter calls sharing its id. Counterfactual: dropping the context fails `request-correlation-id` at `the continuity provider call should carry the request's own id, got undefined`.
- Non-claim on that P2: forwarding the id is all that changed. The continuity call's model, prompt, timeout, budget check, and heuristic fallback are untouched.
- `resolveRequestId` trims and falls back on a blank id, so a caller passing `''` or whitespace still gets a usable id rather than an empty `requestId` field. This matches `ImageService.resolveRequestId` exactly; only the minted prefix differs, and `req_` is the prefix `StoryService` already minted.
- The engine's `requestId` is optional and every existing caller still compiles without it: `StoryServiceLike` is a `Pick` of the two methods, and adding an optional parameter leaves the test doubles that implement them assignable. Job execution and direct test calls keep minting, which is why the fallback stays.
- Non-claim: nothing here changes a prompt, a generated story, the routing table, or the deployable function count — `check-vercel-function-count.sh` reports 8/12, unchanged from `main`. What changes is which id a log line is filed under, and which error code two malformed fields are answered with.
- Non-claim: this is a correlation and log-hygiene fix. It is not an authentication or rate-limiting boundary and does not claim to be one.
- Known gap, not fixed here: `/api/story-lab/jobs` and `/api/story-lab/account` still settle no correlation id at all and echo no `X-Request-ID`. Both serve more than `POST`, so `beginPostRoute` does not apply to them unchanged, and giving them one is a slice of its own rather than a line in this one.
...
- Gave the five heat levels a table below both trees and made the classic seam check membership in it. `SPICY_LEVELS` was declared in the Angular contract when the three numeric blueprint vocabularies were given tables, and everything on the Story Lab side has read it since — the picker, `FormValidationService`, `parseStoryLabBlueprint`, the Proving Grounds slider, the continuity merge. The classic side kept its own copy, and it is the copy that decides what the wire accepts: `VALIDATION_RULES.spicyLevel` was `{ min: 1, max: 5 }`, **the only rule in that object describing its closed set as a range instead of naming it**, while `themes`, `wordCount`, `imageStyle` and `aspectRatio` beside it all read their tables and say so. `StoryService.validateStoryInput` read it as a range and paired it with a separate `Number.isInteger` guard — the two together answering, in two checks that have to agree, what membership answers in one. Each direction of drift fails silently in its own way. A sixth level added to the table is offered by the picker, accepted by the parser, and then refused as `INVALID_INPUT` by the seam the Story Lab hands its blueprint to, naming a ceiling nobody told it had moved. Narrow the table without narrowing the rule and it is quieter: the request is accepted at a level no rung of `SPICE_LEVEL_PROMPT_RUNGS` describes, so `readSpiceLevelPromptLabel` falls back to `Spicy` and the model is sent a `SPICE LEVEL:` line naming a rung the system prompt beside it never defines. `shared/spiceLevelVocabulary` is the table now, `SpicyLevel` is derived from it rather than declared beside it in the API contract, and the refusal renders the list rather than the ends of a range, the way `formatChapterBatchSizeList` already does for the batch sizes. `SpiceLevelPromptRung.level` is typed from the scale rather than left `number`.
- Gave the reasoning efforts a table and retired the reader that never had a caller. The five efforts were written **four** times with nothing tying any two together: `XaiReasoningEffort` in `xaiConfig`, the `VALID_REASONING_EFFORTS` `Set` beside it (which exists because a union is not something `XAI_STORY_REASONING_EFFORT` can be checked against, and which needed two `as XaiReasoningEffort` casts to get back to the union it was copied from), and an inline copy on `ApiResponseMetadata.reasoningEffort` and again on the Angular `GenerationTelemetry.reasoningEffort` — the one a reader sees, since `app.ts` prints it in the telemetry panel. Two identical unions are structurally assignable, so nothing would have reported the drift; the `Set` is the half that fails at run time, since an effort dropped from it is silently replaced by the default on a deployment that configured it. `shared/reasoningEffortVocabulary` is the table, `isXaiReasoningEffort` is the guard, and both casts are gone because the guard narrows. Retired with it: `isHighAgentEffort`, exported so that choosing `high` or `xhigh` could be logged — the logging was never written, so **nothing in the repository, tests included, ever called it**, and what it left behind is a second place deciding what "high effort" means with no caller to hold it to account. `getXaiReasoningEffortForModel` is where every real decision about effort is made and it names the values it acts on itself.
- Stopped two keyword scans rebuilding their tables on every chapter. `api/_lib/utils/wholeWord.ts` says what `wholeWordAlternationPattern` is for — "compiled once per table by the callers that scan repeatedly, never per call" — and `CLIFFHANGER_HOOK_PATTERNS` and `PRESSURE_KEYWORD_PATTERNS` already do exactly that. `storyContentAnalysis` and `storyQualityHeuristics` did not: both asked `containsWholeWord` (or `containsWordForm`) one keyword at a time, and each of those builds a `RegExp` per call. Worse, three of the tables were **literals written inside the function that reads them** — `extractThemesFromContent`'s eighteen-theme, ninety-eight-keyword record, `extractSpicyLevelFromContent`'s four ladders, and `extractSensoryTextures`' seven-entry lexicon — so both the table and a pattern for every word in it were constructed again for every chapter of every continuation. Measured through the `RegExp` constructor: a single story-quality scan built **48** whole-word patterns and `extractThemesFromContent` built **36** per chapter; both are 0 now, and what the continuity dimension still builds is bounded by the words the *request* carried (the creature and the theme words, which arrive from the blueprint and have no list to compile ahead of time) rather than by the file's own tables. The reading is unchanged: the alternation puts the boundary at its ends, which is the boundary each keyword carried on its own. Folded in: `extractSpicyLevelFromContent`'s four `return N as SpicyLevel` assertions, which existed because a bare numeric literal has no relationship to the scale — the rungs are a table typed from `SpicyLevel` now, so `6` is a type error rather than an assertion nobody would question.
- Added three suites and registered all three: `tests/spice-level-vocabulary.test.ts` (one table, both contracts and the validation rule point at the same array object, nothing respells the list or the min/max pair, the refusal renders the table's contents, the guard answers on membership for every value the integer-plus-range pair answered on, and the prompt ladder covers the scale exactly), `tests/reasoning-effort-vocabulary.test.ts` (one table, the env var is checked against it with padding and capitals, an unusable value falls back to the default, no file respells the union or the list, and the retired reader stays retired), and `tests/keyword-scan-compilation.test.ts` (no scan builds a whole-word pattern while it runs — asserted by counting through a replaced `RegExp` constructor, with a sentinel construction first so a zero cannot mean the counter is blind — and every keyword in every moved table still reaches the answer its table names).

Self-review:

- All three have counterfactuals and all three were run, then reverted immediately and not committed. Narrowing `SPICY_LEVELS` to `[1, 2, 3, 4]` makes the classic seam refuse level 5 with `Invalid spicy level (must be 1, 2, 3, or 4)` — the refusal following the table, which is the whole claim — and fails `spice-level-vocabulary` at `the scale should still be five levels`. Dropping `xhigh` from the effort table makes a deployment configuring `xhigh` resolve to `medium` and fails `reasoning-effort-vocabulary` at `the ladder should still be five efforts`. Moving `extractThemesFromContent`'s compilation back inside the scan fails `keyword-scan-compilation` at `extractThemesFromContent built 36 whole-word patterns while scanning` — the defect, counted — while `story-content-analysis` stays green, which is the point of adding the counter rather than trusting the existing suite to notice.
- One message changes text and the change is a no-op in meaning today: `Invalid spicy level (1-5)` becomes `Invalid spicy level (must be 1, 2, 3, 4, or 5)`, rendered from the table. It is a list rather than a range for the reason `formatChapterBatchSizeList`'s docblock gives — a range is only honest while the table is contiguous.
- One guard changes shape and refuses exactly what the pair it replaces refused. `Number.isInteger(x) && x >= 1 && x <= 5` and `SPICY_LEVELS.includes(x)` agree on every value, including `4.0` (which is the integer 4 and is accepted by both) and `'3'`, `2.5`, `NaN`, `Infinity`, `null` and `[3]` (refused by both). The suite pins all of them through the seam rather than only through the guard.
- The keyword change is behaviour-preserving by construction and is asserted that way: `wholeWordAlternationPattern(terms)` and `terms.some(t => containsWholeWord(text, t))` ask the same question, because the helper escapes each term and puts the boundary at the ends of the alternation. What the reading *does* — the whole-word boundaries, the substrings that must not match, the past-tense inflections that must — stays proved where it already was, in `story-content-analysis`, which is green unchanged.
- `containsWholeWord` still exists and still has callers; what changes is that neither of these two modules is one of them. `containsWordForm` also stays, for the continuity dimension's request-carried words, which is the one scan here with no fixed list to compile.
- Non-claim: `story-generator/src/app/contracts.ts` still declares `WORD_BUDGETS`, `NARRATIVE_TONES`, `HEAT_TENSION_MODES` and `HEAT_INTIMACY_BOUNDARIES` union-first with `as const satisfies`, which the previous entry already recorded as arguing against this repository's own `WORD_COUNTS` and `IMAGE_STYLES` docblocks. Spice level is derived here because it had to move to `shared/` regardless; the other four are the same conversion and belong together in their own slice.
- Non-claim: `VALIDATION_RULES.userInput.maxLength` and `VALIDATION_RULES.audioSpeed` are still bare numbers with no table behind them. `userInput` is a length cap rather than a closed set, so it has no vocabulary to read; `audioSpeed` has no reader at all in either tree, which is a retirement question rather than a vocabulary one.
- Non-claim: `ImageService`'s `STORY_LAB_SEED_VISUAL_ELEMENTS` is still a `Record<string, string>` holding seven of the twelve picker seeds, with the other five reached through the classic-theme table beside it. It is not a live gap — `tests/image-service.test.ts` already drives every seed from `STORY_LAB_THEME_SEED_IDS` and fails on the generic fallback or on a repeated element — so tightening the key type would move an existing runtime proof to compile time and nothing else.
- Non-claim: nothing here touches `scoreCliffhangerQuality`'s loop that assigns `finalParagraph` once per paragraph to reach the last one. It is a `paragraphs.at(-1)` written the long way and it is correct; changing it is a readability edit with no defect behind it.
- Validation: `npm run test:all` exits `0` across all 75 registered suites, including the three added here. `tsc --noEmit` is clean against `story-generator/tsconfig.app.json`, `story-generator/tsconfig.spec.json`, and the Vercel API file list. `scripts/recovery/check-vercel-function-count.sh` reports 8/12, unchanged — no route file is added, retired or renamed. `git diff --check` is clean over the files this slice touches. No Angular spec is added: the one Angular file this slice touches is a contract declaration, pinned from the root suite instead. Root and Angular installs were needed to run any of it; the tracked `node_modules` tree the install dirtied was restored and is not in this diff, as the previous entries record.

## 2026-08-27 UTC - Six Blueprint Fields The Routes Accept And The Prompt Halved, A Prompt Cut Made With `slice` And A Number, And A Trope Range With No Reader

Actions:

- Gave each blueprint free-text field the prompt the cap its own route publishes. `formatStoryLabContext` measured six of them — the logline, the tone, both character names, the world details, and the narrative directives — against a single `STORY_LAB_CONTEXT_VALUE_MAX_LENGTH = 320`, a bare number predating the shared limits. `parseStoryLabBlueprint` accepts a **420**-character logline, **600** characters of world details, and **1200** of narrative directives, and `FormValidationService` reads those same numbers out of `STORY_BLUEPRINT_LIMITS` to tell the reader what the field will take. Every one of them then arrived at the prompt and was cut to 320 with nothing anywhere saying so: **narrative directives lost three-quarters of an accepted field, world details nearly half, and a logline written to the limit the form states lost its last hundred characters**. The block those lines are written into ends `- Treat these blueprint fields as binding story intent, not as optional flavor`, so the prompt was asserting that a quarter of what it had dropped was binding. The cap is not removable and that is why these are numbers rather than nothing — `/api/story/generate` takes a `generationContext` too and `validateStoryInput` does not measure its fields, so this is the only boundary on that path — it just has to be the number that path's sibling already publishes, one field at a time. `noGoContent` keeps 320, which was always its own cap (`STORY_LAB_PROFILE_LIMITS.maxNoGoContentLength` reads it as one); `tone` takes `STORY_EVALUATION_LIMITS.maxConfigurationValueLength`, documented for exactly this shape of field.
- Stopped that funnel cutting with `slice` and a number. `limitStoryLabPromptText` is the one door **nine** prompt fields pass through — the six above, the no-go list, and each theme seed's label and description — and it ended `compacted.slice(0, maxLength).trim()`. That is the defect `api/_lib/utils/textExcerpt.ts` was written for and names three earlier instances of: "Three places still cut with `String.prototype.slice` and a number, and all three feed a model rather than a screen." This is the fourth and the widest. `slice` counts UTF-16 code units, so a cut landing between the halves of a surrogate pair leaves a **lone surrogate in the provider request** — nothing throws, because `JSON.stringify` escapes it rather than refusing it, so the model is simply shown a character the reader never typed in place of the emoji or astral-script character that was there — and a cut at an arbitrary offset ends mid-word, handing the model a fragment of a world-detail sentence. `capAtWordBoundaryWithinCodeUnits` rather than `capAtWordBoundary`, and deliberately: the caps are the route's, and the route measures them with `.length`, so cutting in code points here would let a field of astral characters be measured one way at the route and another at the prompt — the drift the shared limits exist to prevent. A code point is still never split. The trim-and-`\s+` beside it is `collapseWhitespace` now, read from the module that holds it rather than spelled out a fifth time, and `tone`'s inline `split('_').join(' ')` reads the same `formatBlueprintIdLabel` the two Heat Contract fields one line below it already did.
- Gave the trope-subversion range back its reader. `TropeSubversionService` owns how many tropes one story is asked to subvert — `minTropes`, `maxTropes`, and `getRandomTropeCount` drawing from them, with `selectTropesForSubversion` falling back to it when the caller names no count. `StoryService.selectTropeSubversions` is that service's only caller in the app, and it named a count on every call: `randomInt(2, 4)`, which is the same two-or-three drawn the same way, written out again on the other side of the seam. So all three declarations had **no reachable reader at all** — a named range nothing consults is not a range, it is a note, which is the argument `estimateReadTimeMinutes` was extracted under for `READING_SPEED.WORDS_PER_MINUTE` — and widening `maxTropes` would have changed nothing about any generated story, because the caller decided. The caller omits the count now. The draw is unchanged: `randomInt(this.minTropes, this.maxTropes + 1)` **is** `randomInt(2, 4)`.
- Extended two existing suites rather than adding a file: `tests/story-service-prompt-guards.test.ts` (each field reaches the prompt at its own route's cap and is still cut past it; a cut inside an astral character leaves no lone surrogate; a capped field ends on a whole word) and `tests/trope-subversion.test.ts` (an unspecified count stays inside the service's range across 25 draws per creature, and the app is asserted to pass no `tropeCount` at all, by observing the options the seam receives). No `package.json` script changes were needed.

Self-review:

- All three have counterfactuals and all three were run, then reverted immediately and not committed. Putting `STORY_LAB_LOGLINE_MAX_LENGTH`, `STORY_LAB_WORLD_DETAILS_MAX_LENGTH` and `STORY_LAB_NARRATIVE_DIRECTIVES_MAX_LENGTH` back to `320` fails `story-service-prompt-guards` at `a logline should reach the prompt at the length the blueprint route accepts`. Restoring `compacted.slice(0, maxLength).trim()` fails the same suite at `a cut inside an astral character must not leave a lone surrogate in the prompt`. Restoring `tropeCount: randomInt(2, 4)` at the call site fails `trope-subversion` at `the app should let the service draw the count from its own range, not name one`.
- The trope assertion is on the **options the seam receives**, not on the count that comes back, and that is the point: the two ranges agree today, so no assertion about the selection itself could distinguish a caller that reads the service's range from one that restates it. Nothing about the distribution of generated stories changes.
- The astral fixture prefixes one ASCII character deliberately. Every cap in play is even, so without it the cut falls on a character boundary and the old `slice` happens to be safe — the test would have passed against the defect.
- Behaviour changes for a reader in exactly one direction: more of what they wrote reaches the model. Nothing that was refused before is accepted now — the route caps are unchanged and the parser still refuses past them — and nothing that was accepted before is refused.
- `capAtWordBoundaryWithinCodeUnits` can return the empty string for a cap smaller than the field's first word, where `slice` returned a fragment of it. Every caller here checks the result for truthiness before writing a line, so the field is omitted rather than rendered broken; the smallest cap in play is 80, so no field this app can send reaches that corner.
- Non-claim: `formatThemeContext`'s seed caps are unchanged in value — they already read `maxThemeLabelLength` and `maxThemeDescriptionLength`. What changes for them is only the *cut*, which they share with the six fields above through the same funnel.
- Non-claim: `validateStoryInput` still does not measure any `generationContext` field on `/api/story/generate`. That is why the prompt-side caps stay, and closing it properly means running that route's context through `parseStoryLabBlueprint`'s reading — a route change, not a quick win.
- Non-claim: `TropeSubversionService.serializeTropeSelection` still writes a `timestamp` that `deserializeTropeSelection` never reads, and `getTropeStatistics`/`getAllTropesForCreature` still have no caller in either tree. Recorded because they were found while tracing the trope count; retiring dead surface is the shape of #281, not of this slice.
- Validation: `npm run test:all` exits `0` across all 72 registered suites. `tsc --noEmit --strict` is clean over the four files this slice touches. No Angular file is touched, so no `ng test` run is claimed for this slice. No route file is added, retired or renamed, so `check-vercel-function-count.sh` is unchanged at 8/12. Root install was needed to run the suite; the tracked `node_modules` tree and the `package-lock.json` `name` field the install dirtied were both restored and are not in this diff, as the previous entries record.

## 2026-08-27 UTC - The Same Batch-Size Bound Six More Times On The Classic Path, A Theme Cap Three Modules Kept Their Own Copy Of, And A Private HTML Stripper That Decoded No Entities

Actions:

- Gave chapter batch size one home below both trees and made the classic seam read it. The previous entry moved the *Story Lab* half of this bound onto `CHAPTER_BATCH_SIZES` after both continuation routes were caught writing `[1, 2, 3].includes(size)` by hand. The classic half kept every copy it had — and these are the same value, not two that happen to agree: `toClassicGenerationInput` and its continuation sibling in `storyLabEngine` pass `input.chapterBatchSize` straight into `requestedChapterCount`, so a Story Lab batch size **is** the number `StoryService` is asked to write. Six copies of it, all inside the seam the Story Lab hands its blueprint to: the inline `1 | 2 | 3` on `StoryGenerationSeam['input']` and again on `ChapterContinuationSeam['input']`; `isValidRequestedChapterCount`, a bare `[1, 2, 3].includes`; the refusal `requestedChapterCount must be 1, 2, or 3` sent from `continueChapter` and again from `validateStoryInput`; and `expectedType: '1 | 2 | 3'` beside each of those. A seventh was the Angular form's own `Choose 1, 2, or 3 chapters per batch.`, written out beside a `VALID_BATCH_SIZES` set built from the table. **A fourth batch size fails twice over**: the picker offers it, the parser accepts it, and both continuation routes now accept it — all four reading the table — and then `StoryService` refuses it as `INVALID_INPUT` naming a range nobody told it had moved. Remove that refusal and it is worse, because `normalizeChapterCount` answers `3` for it instead: a validated request for four chapters silently generating three, with `chaptersRequested` in the response metadata reporting the number the reader asked for. `shared/chapterBatchVocabulary` is the table now, `ChapterBatchSize` is derived from it rather than declared beside it, and `clampToChapterBatchSize` replaces the hardcoded `<= 1 → 1`, `>= 3 → 3`, otherwise `2`.
- Made the theme cap one number. How many themes one story may weave was written three times: `STORY_BLUEPRINT_LIMITS.maxThemes`, which the Story Lab form and `parseStoryLabBlueprint` enforce; `VALIDATION_RULES.themes.maxCount`, which `validateStoryInput` enforces on the array **that same blueprint becomes**; and a bare `5` inside `toClassicThemes`, the function that turns the first into the second. `STORY_EVALUATION_LIMITS.maxThemes` sitting beside the first already read it and says why — these fields "name the same things" — while its two neighbours in `VALIDATION_RULES` both read their tables and say so. Each direction of the drift fails silently in its own way. Raise the shared limit alone and the picker offers a sixth seed, the form takes it, the parser takes it, and the classic route answers `INVALID_INPUT` for a blueprint the app assembled for itself. Lower it alone and the refusal never arrives: `toClassicThemes` cuts the sixth seed on the way to the generator and **the story is written without a theme the reader chose**, with nothing in the response saying which one went missing.
- Retired the last private HTML stripper in the repository. `storyLabEngine` carried its own `stripMarkupTags` — a character loop dropping everything between `<` and `>` — beside `shared/storyTextBlocks`, the module written for exactly this and already read by every other scanner here. `shared/whitespace`'s docblock records that this file's *whitespace* half was moved onto the shared reading; the markup half was left behind. The difference is the one hardest to see in the code and easiest to see in the product: **the loop decoded no entities**, so `&nbsp;`, `&quot;`, `&#39;` and `&amp;` survived it as their literal source text, and both readers hand that straight to the reader. `summarizeHtml` is the chapter excerpt shown in the library and the workbench, cut to 28 words, so a chapter opening on `she said &quot;no&quot;` was previewed with the entities showing and spent two of those 28 words on markup. `countWords` is the `wordCount` on a chapter the classic service did not count for itself, and `intense&nbsp;passion` is one token to a scan that cannot see the entity as the space it is. The speaker-tag pass stays in the engine: `[Narrator]:` is this app's own convention rather than markup, and the shared module deliberately knows nothing about it.
- Added three suites and registered all three: `tests/chapter-batch-vocabulary.test.ts` (one table, both contracts point at the same array object, nothing respells the list or the prose range, the service's refusal renders the table's contents and its `expectedType` is joined from it, and the clamp answers what the hardcoded form answered), `tests/theme-count-limit.test.ts` (the three numbers are one number, `toClassicThemes` cuts at the shared limit, and the conversion cuts to exactly the count the classic validator accepts), and `tests/story-lab-chapter-text.test.ts` (the excerpt shows no entity as source text, a non-breaking space separates two words in the count, and a paragraph boundary is not welded).

Self-review:

- All three have counterfactuals and all three were run, then reverted immediately and not committed. Restoring `stripMarkupTags` fails `story-lab-chapter-text` at `the chapter excerpt should not show &quot; as source text`, printing the excerpt with all four entities intact — the defect as the reader would have seen it. Putting the literal `.slice(0, 5)` back and raising `maxThemes` to 6 fails `theme-count-limit` at `toClassicThemes should cut at the shared limit rather than at a literal`. Restoring `'requestedChapterCount must be 1, 2, or 3'` fails `chapter-batch-vocabulary` at `the classic story service should name the accepted batch sizes from the list it checks, not from memory`.
- One behaviour changes, and it is the corner the old form got wrong for the same reason it hardcoded the rest. `normalizeChapterCount`'s `NaN` satisfied neither `<= 1` nor `>= 3`, so an unreadable count fell through to the literal `2` — a two-chapter batch conjured from a value that was not a number. It now answers the smallest size, which is what an absent count already meant. On every path the app can take this function is unreachable with anything but a table member or `undefined`, because `isValidRequestedChapterCount` runs first; the suite pins `0`, `4`, `2.5` and `NaN` anyway, since what the clamp is *for* is the path where that guard is skipped or moved.
- The two refusal messages change text and the change is a no-op in meaning today: `requestedChapterCount must be 1, 2, or 3` is what `formatChapterBatchSizeList()` renders for the current table. `expectedType` moves from `'1 | 2 | 3'` to the same three numbers joined with `' | '`, which is character-for-character identical while the table holds them.
- The spicy-level refusal in `FormValidationService` was folded in with the batch-size one because it is the same defect two lines away — `Spicy level must be between 1 and 5.` beside a `VALID_SPICY_LEVELS` set built from `SPICY_LEVELS`. It reads the ladder's own ends now. It is named here rather than counted as a fourth win because it is one line and no behaviour of it changes.
- Non-claim: the entity decoding does not change any *classic* chapter's `wordCount`. `toStoryLabChapters` keeps `chapter.wordCount` when the classic service supplied one, and that service has counted through `stripStoryHtmlToText` since the block-splitter was written. What changes is the excerpt, which is always computed here, and the count on the fallback path where no chapter carried one. The suite pins both.
- Non-claim: `SPICY_LEVELS`, `WORD_BUDGETS`, `NARRATIVE_TONES`, `HEAT_TENSION_MODES` and `HEAT_INTIMACY_BOUNDARIES` are still declared union-first and checked with `as const satisfies`, which this repository's own `WORD_COUNTS` and `IMAGE_STYLES` docblocks argue against — a `satisfies` clause catches a wrong entry and every copy of these lists could only ever go wrong by being short one. Batch size is derived because it had to move to `shared/` regardless; converting the other five is a change to both contracts and belongs in its own slice.
- Non-claim: nothing here reads `active_skin`, `tone`, `spicy_level`, `creature` or `latest_revision` back out of `story_projects` — every statement in `postgresStoryProjectStore` selects only `project_json` and the row's identity columns, and `active_skin` in particular is written from a hardcoded `'writing-desk'` that no `SavedStoryProject` field can influence. That is a schema question, not a quick win, and is left where it is.
- Non-claim: `StoryLabProfilePreferences.defaultHeatContract`, `favoriteCreatures` and `favoriteTones` are stored, validated, redacted and persisted with no reader in either tree — only `librarySort` is read, by `handleProjectsRoute`. Recorded here because it was found while tracing the blueprint defaults; applying or retiring those three is a feature, not a cleanup. **Superseded by the 2026-09-06 UTC entry above (PR #342)**, which gave the profile a frontend editor and applies these three to a fresh blueprint.
- Validation: `npm run test:all` exits `0` across all 72 registered suites, including the three added here. `tsc --noEmit` is clean against `story-generator/tsconfig.app.json`, `story-generator/tsconfig.spec.json`, and the Vercel API file list. `npx ng test --watch=false --browsers=ChromeHeadlessNoSandbox` passes 198 of 198 specs, unchanged from the previous entry — this slice adds no Angular spec, because both Angular files it touches are pinned from the root suite instead. `scripts/recovery/check-vercel-function-count.sh` reports 8/12, unchanged. `git diff --check` is clean over the files this slice touches. `CHROME_BIN` had to be pointed at the preinstalled Chromium (`/opt/pw-browsers/chromium`, the symlink rather than the directory) for Karma to launch. Root and Angular installs were needed to run any of it; the tracked `node_modules` tree the install dirtied was deliberately left unstaged, as the previous entries record.

## 2026-08-27 UTC - A Batch-Size Bound Both Continuation Routes Wrote Out By Hand, Five Save-Request Fields Admitted By Cast, And A Job Step Vocabulary Whose Fallback Could Not Fail

Actions:

- Made the two routes that run continuations read the batch-size table the rest of the app reads. `CHAPTER_BATCH_SIZES` has been the list since the blueprint pickers were given one, and its docblock claims that everything checking one of these three fields reads it — true for spicy level and word budget, and **not true for batch size**, which has a checker the blueprint parser never sees. A continuation does not arrive through `parseStoryLabBlueprint`; it arrives at `POST /stories/:storyId/continue` or at the continuation half of `POST /jobs`, and both of those declared their own local `isValidBatchSize` over a literal `[1, 2, 3]`. Two identical copies in two files, neither reading the table beside them, and each refusal then explained itself as "a chapterBatchSize of 1-3" — a third and fourth spelling of the same bound, in the sentence whose whole job is to state it. The failure a fourth batch size buys is worth naming because it is not symmetrical: the picker offers the new size and genesis accepts it, both reading the table, and the reader's *next* chapter request is refused by a route insisting on a range nobody told it had moved — **a story generated at a size it cannot be continued at**. `isChapterBatchSize` is the predicate and `formatChapterBatchSizeList` renders the refusal from the same list, as a list rather than a range because a range is only honest while the table stays contiguous. The `batchSizeNumber as ChapterBatchSize` cast in `continue.ts` is gone with it: the predicate narrows, so the refusal that guards the line is now also what types it.
- Made the project save route check the five fields it was writing down as casts. `readProjectFromBody` hand-checked eight fields and then admitted `chapters`, `telemetry`, `continuityExtraction`, `pinnedMemoryCardDraftIds`, and `acceptedMemoryCards` with `as`. `chapters` got as far as `Array.isArray` and its entries were never looked at; the other four were not looked at at all. **The Angular tree already knew this was unsafe** — `normalizePinnedMemoryCardDraftIds` and `normalizeAcceptedMemoryCards` in `app.ts` are both declared over `unknown` and both filter entry by entry, written for exactly two of the five fields this route was casting, and run on the way *out* of a project the route let in without looking. So the client was defending itself against data its own server had accepted, which is the wrong half of the seam to be doing it on, and the one reader that is not the Angular tree was not defended at all: `toStoryProjectListItem` computes `acceptedMemoryCardCount` as `acceptedMemoryCards?.length ?? 0`, so a project saved with `"acceptedMemoryCards": "none"` became **a library card announcing four memory cards for a story that has none** — `.length` of a string, counted and rendered. Two rules now, and the split is deliberate: the story is refused rather than repaired (`chapters` joins `summary`, `state`, and `blueprint`, because storing the chapters that passed and dropping the rest loses a chapter the caller believed they saved), and the annotations are filtered rather than refused (refusing an entire novel over one malformed memory card would lose the story to save a note about it).
- Gave the job step vocabulary a table and took away the fallback that could not fail. `status` is where a job is in its lifecycle and has had a union and a table since the terminal-status slice; `currentStep` is what the job is *doing*, and it had neither — declared `string` on the job, `string | undefined` on the store's create input, and `string` on its update input, for a set with exactly five members, every one of them a literal written by hand at the five places `jobRouteHandlers` moves a job along, plus `'queued'` twice more as the two stores' default. The reader is where an open type costs: `formatJobStage` is the only one, and it switched on the five literals to choose the sentence the reader watches, with `humanizeIdentifier` beneath. **That fallback cannot fail visibly** — it is built to make any identifier presentable — so a sixth step would have reached the reader as its own title-cased wire name, a written sentence silently missing, and both sides saying `string` meant TypeScript had nothing to compare. `STORY_LAB_JOB_STEPS` is the table and `STORY_LAB_JOB_STEP_LABELS` is a total `Record` over it; `UpdateStoryLabJobInput.currentStep` is a `StoryLabJobStep`, which is what makes the set closed — nothing in the repository can *write* a step the reader has no sentence for. Folded in with it: `StoryLabJobCreationRequest`'s deferred variant, which was still spelling `kind: 'export' | 'audio'` by hand — the fifth spelling of that pair and the last one left after the previous slice took the other four — now reads `StoryLabDeferredJobKind` off the table.
- Extended three existing suites rather than adding a file: `tests/story-lab-blueprint-vocabulary.test.ts` (both continuation routes read the table and restate neither the list nor the prose range; the rendered refusal names every size the table holds), `tests/story-lab-job-contracts.test.ts` (labels total and distinct, every step the route writes is one the reader has a sentence for, read out of the route's own source so a future literal is caught, and the deferred variant no longer respells the pair), and `tests/story-lab-account-routes.test.ts` and `story-generator/src/app/app.spec.ts`. No `package.json` script changes were needed.

Self-review:

- All three behavioural changes have counterfactuals and all three were run, then reverted immediately and not committed. Restoring `readAcceptedMemoryCards`'s cast fails `story-lab-account-routes` at `the library card should report no memory cards, not the length of a string (got 4)` — which is the defect itself, counted and reported. Putting `[1, 2, 3].includes(...)` and the `1-3` prose back into `continue.ts` fails `story-lab-blueprint-vocabulary` at `the continue route lists the batch sizes again; there should be one table`. Dropping `formatJobStage` back to the bare `humanizeIdentifier` fails the new Angular spec with `Expected 'Generating story.' to be 'Grok is writing your first chapter.'` — the fallback rendering a step that had a sentence written for it — and takes an *existing* spec with it (`Expected 'Continuing story.' to contain 'Grok'`), which is worth recording: the reader's copy was partly pinned already, and only for the two steps a spec happened to exercise.
- The save route's two rules are the only changes here that alter what gets stored, and the asymmetry between them is deliberate rather than an oversight. A value that is not a list at all is stored as **absent** rather than as `[]`, because absent is a state the optional field already has and every reader already handles, and because the client's normalizers turn `undefined` back into `[]` on load — so the two sides agree at the only place it shows. A list *with* entries in it is the caller trying to report annotations and getting the shape wrong, so what survives is kept; the suite pins both directions.
- `readChapters` requires the four fields readers dereference — `chapterId`, `chapterNumber`, `title`, `htmlContent` — and not all eight `GeneratedChapter` declares. Demanding all eight would refuse projects saved by an earlier client over a field nothing reads, which is a worse trade than the one this check exists to make. Named because it is a judgement call and a future reader may want the other one.
- The two refusal messages change text, and the change is a no-op in meaning: "a chapterBatchSize of 1-3" becomes "a chapterBatchSize of 1, 2, or 3". `tests/story-lab-job-routes.test.ts` asserts the error *code*, not the sentence, and was not touched.
- No claim is made that the job steps disagreed today. They did not — `formatJobStage` had a case for all five. What changes is that a sixth is now a compile error in the reader instead of a silently humanized wire name, and that the writers can no longer invent one.
- Non-claim: `StoryLabJob.currentStep` is still `string` and not `StoryLabJobStep`, deliberately. It is a snapshot off the wire, and `postgresStoryLabJobStore`'s `jobFromRow` reads it out of `current_step` where a durable row written by an older deployment may name a step this build has retired. Narrowing it there would be a promise the network cannot keep, so the reader keeps its fallback — the same arrangement `formatBatchStatus` already uses for `BatchProgressState['status'] | string`. What the table guarantees is the write side.
- Non-claim: nothing here validates `summary`, `state`, or `blueprint` beyond `isObjectRecord`, which is where they already were. They are large nested contracts and checking them properly is a parser, not a guard; this slice closes the gap between the fields that were checked and the fields that were cast, and does not move that line.
- Non-claim: `StreamingProgressChunk` in the Angular contract is still a four-member union with no writer and no reader in either tree, as the previous entry recorded. Retiring dead contract surface is the shape of #281, not of this slice.
- Validation: `scripts/recovery/preflight.sh --quick --skip-status` exits `0` (`git diff --check`, the Vercel function-count check at 8/12, and `tsc --noEmit` against `tsconfig.app.json`, `tsconfig.spec.json`, and the Vercel API file list). `npm run test:all` exits `0` across all 71 registered suites. `npx ng test --watch=false --browsers=ChromeHeadlessNoSandbox` passes 198 of 198 specs, two more than the previous entry recorded because this slice adds two; `CHROME_BIN` had to be pointed at the preinstalled Chromium for Karma to launch. Root and Angular installs were needed to run any of it. `npm install` rewrote the `name` field of the root `package-lock.json` to match this checkout's directory casing; that was reverted and the lockfile is unchanged. The tracked `node_modules` tree the install dirtied was deliberately left unstaged, as the previous entries record.
