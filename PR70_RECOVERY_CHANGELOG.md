...
## 2026-08-27 UTC - The Tag That Ended At The First `>` Rather Than Its Own

Actions:

- Made `shared/storyTextBlocks.ts` read a tag past its quoted attribute values. A tag does not end at the first `>` — it ends at the first `>` that is not inside a quoted attribute — and both readers in the module used `[^>]*`, which stops early. On `<p title="a>b">Hello.</p>` the boundary pattern matched `<p title="a>` and the tag's own remaining text came back as prose: `["b\">Hello."]`. That is reader-visible rather than internal, through the module's other export, `stripStoryHtmlToText`: the Angular client's plain-text story download (`buildPlainStoryText`) renders it, and `continuityExtractor`'s chapter excerpts, `storyLabEngine`'s reader text and `ImageService`'s scene sentences are all built from it — so the attribute remnant is both shown to the reader and sent to the model as part of the story so far.
- Fixed at both readers, because fixing one only moves the remnant to the other: `BLOCK_BOUNDARY_PATTERN` for block-level tags, and `stripInlineTags` for what is left, so `<em title="a>b">` no longer leaves `b">` either. The attribute fragment is spelled once, in `tagAttributesPattern`, parameterised by the unquoted-run class the two readers already differ on (`[^>"']` and `[^<>"']` — the inline one keeps excluding `<` for the run-of-`<` cost the docblock records).
- Two deliberate limits, each of which is what keeps the change from reaching prose. A quoted run stops at `<`, so an unterminated quote cannot swallow the story's own dialogue looking for a partner; and each pattern keeps its older `[^>]*>` reading as a *fallback* alternative rather than an equal, so markup with no well-formed reading is answered exactly as before. The inline reader offers the tag-shaped reading only to a `<` that begins a tag name, because prose is where it runs: in `Price < 5 "a > b" > done.` there is no tag, and letting a quoted run reach across the sentence would delete more of it than the older reading did.
- Extended `tests/story-text-blocks.test.ts`. No new test file, so `test:all` gains no entries.

Self-review:

- Four semantic counterfactuals, each run and then reverted; none is committed. Restoring `[^>]*` on the boundary pattern fails at ``a `>` inside a double-quoted attribute should not end the tag``; restoring it on the inline stripper fails at `an inline tag carrying such an attribute should be stripped whole`; dropping the unterminated-quote fallback fails at `an unterminated attribute quote should still put a boundary between the words`; dropping the inline tag-name guard fails at ``a bare `<` in prose should not let a quoted run reach across the sentence``.
- The first two mutations are the fix itself. The second two are the limits, and both needed a sharper case than the one I first wrote: my initial fallback assertion passed under its own mutation, because the inline reader's fallback still deleted the malformed tag in place and the *blocks* only differ when that deletion welds two words (`Before.<p title="unclosed>After.` → `Before.After.`). A mutation that does not fail is a test that was not pinning the claim, so both assertions were replaced by ones that discriminate rather than left as decoration.
- Differential fuzzing against the pre-fix implementation, 200,000 inputs built from tags, wrapped attributes, entities, malformed `<`, bare quotes and prose: **0 differences** on a corpus containing no `>` inside a quoted attribute. That is the claim worth making — the reading changes for exactly the inputs it is about and for nothing else. On a corpus that does contain them, ~32,700 of 100,000 differ, which is the fix. On 200,000 well-formed inputs, **0** blocks come back carrying `<`, `=`, `">` or `'>`.
- Each position is still decided once. `tagAttributesPattern` is the unrolled "unquoted runs separated by quoted ones" form, and the unquoted run and the two quoted alternatives cannot start on the same character, so no input has two ways to match. Pinned by timing assertions on the three adversarial shapes the module's docblocks already worry about — a run of quotes, a run of spaces, a run of `<` — each of which has no match at all.
- `npm run test:all` exits `0`; the Vercel API function typecheck is clean under the pinned `tsc` 5.9.2 with preflight's exact command; `check-vercel-function-count.sh` is 8/12, unchanged; `git diff --check` is clean.
- Non-claim: no prompt, no generated story, no route, and no deployable function count changes. What changes is that a tag with a `>` inside a quoted attribute is read whole instead of leaving its tail in the text.
...
- Worth recording how the finding was read at all: `sonarcloud.io` is blocked by this environment's egress proxy, and the previous slice (#293) had to stand down on a Sonar count it could not open. The rule text and line numbers are available without it, through `GET /repos/{owner}/{repo}/check-runs/{id}/annotations` — the check run's id comes from `GET /commits/{sha}/check-runs`, so no webhook is needed either. **Corrected a round later:** this entry first said a *failed* check run is required, repeating #293's belief. It is not. The passing gate on `e32b1dd` returned its annotation the same way, which is how the `String.raw` finding below was read. Sonar's `N New issues` line is readable from this environment whether the gate passes or fails, and #293's stand-down was avoidable.

Review round — Codex, three P2 findings on `50080f5`, all three correct and all three fixed or corrected:

- **The slice did not fix the server exports, and this entry said it did.** `.txt`, `.pdf`, `.epub` and `.docx` all go through `ExportService.toPlainText` → `stripStoryHtmlForExport` in `exportSanitizer.ts`, which is a different reader with its own tokenizer — not `stripStoryHtmlToText`. Measured rather than argued: `stripStoryHtmlForExport('<p title="a>b">Hello.</p>')` still answers `b">Hello.`, so every server export keeps the defect this slice repairs elsewhere. The claim above is narrowed to the paths that do read this module — the Angular client's plain-text download, the continuity excerpts, the engine's reader text, and the image scene sentences — and the export sanitizer's copy of the defect is added to the follow-up issue rather than left implied as fixed. My own non-claim section had said "reader-visible", which was true, but the mechanism I named for it was wrong.
- **The `<` bound was not enough to stop the reading eating prose.** `Before.<p title='unclosed>It's dangerous >After.` came back as `['Before.', 'After.']`, where `main` answers `['Before.', "It's dangerous >After."]`: the run opened by the malformed attribute found its partner in the apostrophe of `It's`. That is the worse of the two failures — the defect being fixed leaves visible junk in the text, this silently removes words the reader wrote and the next continuation prompt is built from. Fixed with `ATTRIBUTE_VALUE_MUST_END_AT`, a lookahead requiring an attribute value's closing quote to be followed by whitespace, `/` or `>`, which is HTML's own syntax rather than a guess about content: in that input the candidate quote is followed by `s`, so there is no well-formed reading and the older fallback answers instead.
- **The wall-clock deadline was both flaky and not a proof.** Both halves are right. The adversarial inputs are now asserted on their *answer* first, which is deterministic and does not depend on machine load, and the timing check that remains is named for what it is — a tripwire against reintroducing a backtracking pattern — with a deadline raised to 30s, because these scans take about a millisecond and quadratic behaviour on that input takes minutes, so nothing in between is a loaded runner.

Review round — Codex, two more P2s, and the round that changed the design rather than adding to it. `Before.<p.foo="oops>Visible" >After.` and `Before.<p ="oops>Visible" >After.` both deleted `Visible`; checking the family rather than the two reports found a third, `<p x=y="oops>`, and the inline analogue of each. Four regressions, all the same failure as the two previous rounds.

**The lesson is the shape, not the shapes.** Each round I had matched the *inside* of a tag and added one more rule for a way prose could be mistaken for it — open only after `=`, close only before whitespace, never cross `<`. Every rule was correct HTML and every one closed a real hole, and the next round found another shape satisfying all of them. Enumerating what malformed markup looks like does not terminate; the repository already learned this on #294, where four rounds of rules died on the first case that separated each rule from the thing it stood for.

So the reading is inverted: `WELL_FORMED_TAG_ATTRIBUTES` describes a **whole well-formed tag** — a run of attributes, each a name with an optional `=` and a quoted or bare value, then an optional `/` and the `>` — and anything that does not parse as one falls to the `[^>]*>` reading the module has always used. Malformed markup is therefore answered exactly as it was before attributes were understood at all, for every malformed shape rather than one shape at a time. The three accumulated rules and their two unquoted-run constants are deleted; the grammar subsumes all of them.

Measured, not asserted: all six review findings to date answer identically to `main` (0 regressions across the round-4 and round-5 batteries), every repair still works, the no-trigger corpus still shows **0 differences** from `main` over 200,000 inputs, 0 blocks carry markup remnants over 200,000 well-formed ones, and on the malformed corpus the inputs where a word a reader would notice is dropped fall from ~30 per 300,000 to **11** — all of them now tags that genuinely are well-formed, which is the browser's reading and the documented residue. Counterfactual: restoring the previous inside-the-tag form fails at `a tag name run into punctuation is not a tag, so its prose should survive`. Adversarial timings unchanged (~1-10ms), since `\s+` and the name characters are disjoint and every attribute iteration consumes at least two characters.

Review round — Codex again, one P2, **declined with measurements and recorded as a known limit**: an attribute value legitimately containing `<` (`<p title="1 < 2 and 3 > 2">`) is not read whole, because quoted runs stop at `<`. Checked first whether this is a regression or an unfixed case, since only the first would be this PR's to fix: it is identical to `main` on every shape offered, the inline one included, so nothing here makes it worse. Then measured the proposed repair instead of reasoning about it — lifting the bound, and a newline-bounded variant of it, both take the inputs where a word a reader would notice is dropped from ~30 per 300,000 to 49, and the new losses reach *across* tags rather than staying inside the one they began in. That is a wider version of the exact failure the previous round was about. The generator emits `<p>`, `<em>` and `<h3>` with at most a class, so an attribute carrying a literal `<` is not a shape it produces, while a truncated tag is. Recorded in the module docblock with the numbers and added to the follow-up issue, rather than resolved away.

Also corrected in the same pass: the module docblock still carried the export claim that was narrowed everywhere else, and now names the paths that actually read it.

Review round — Codex, one P2, **a real regression and fixed**: `Before.<p ">Visible" >After.` came back as `['Before.', 'After.']` where `main` answers `['Before.', 'Visible" >After.']`. A stray quote loose in a tag — no `=` anywhere near it — opened a run that crossed the `>` and closed at the next quote, deleting `Visible`. The closing-quote rule from the previous round could not see this, because it governs only where a run *ends*; nothing governed where one was allowed to *begin*. Fixed by requiring a quoted attribute value to open after `=`, which is the other half of the same syntax, with `=` removed from both unquoted-run classes so the two cannot overlap. Counterfactual: reverting to the pre-round shape fails at ``a stray quote with no `=` before it should not open an attribute value``. Every earlier case — the `>` inside an attribute, the apostrophe case, the bare-`<` prose case, the unterminated-quote boundary — re-verified unchanged.

Follow-up on the same head: the gate passed but reported one new issue, and it was mine — `String.raw` on `tagAttributesPattern`'s return, which carries no backslash of its own. I had put it back during the round before this one, on the reasoning that adding `\s` made it necessary; the `\s` is inside `ATTRIBUTE_VALUE_MUST_END_AT` and reaches the literal through interpolation, so the reasoning was wrong even though the constant it named was real. Dropped; the three `String.raw` calls left all genuinely carry `\s`, `\b` or `\/`. Behaviour identical — the round-4 cases, the own suite, the no-trigger corpus and `test:all` all re-run unchanged.

That makes three rules, each closing a hole the previous ones left: a run may only open after `=`, may only close where an attribute value may end, and stops at `<` either way. The first two arrived as review findings on the same defect a round apart, which is worth recording as the shape of the mistake: I twice fixed one end of a construct and assumed the other end was covered by it.

Where the line now falls, stated as a decision rather than left to be discovered: where markup is well-formed, this module follows it; where it is not, it prefers keeping the reader's words over guessing. The lookahead is the well-formedness test. That means text inside a quote that *does* close where an attribute value may close is still dropped — matching what the browser does, and the app renders chapters through `[innerHTML]`, so that text is text the reader never saw. Measured on a 300,000-input corpus seeded with unterminated quotes, contractions and dialogue: 130 differences from `main`, 103 of them differing only in punctuation or markup residue, and 27 in which a word differs — every one of those a case where the new reading is the browser's. Pinned by three assertions covering both sides of the line, and a counterfactual: removing the lookahead fails at `a malformed attribute quote should not consume prose at the next apostrophe`.
