...
## 2026-08-27 UTC - The Splitter Threw Away Which Boundary It Split On, And Four Review Rounds Were Spent Reasoning About A Structure It Had Erased

Stacked on the PR #293 branch, and the follow-up its own withdrawal names: "Closing the gap properly needs boundary provenance in `splitStoryIntoTextBlocks`, which every scanner reads — a slice of its own."

Actions:

- Added `splitStoryIntoRenderedParagraphs` to `shared/storyTextBlocks.ts`: the same text blocks, grouped by the paragraph a reader sees. `splitStoryIntoTextBlocks` is now **defined as its flattening**, which is the part that matters for a module every scanner in the repository reads — there is one definition of where a boundary falls, and the flat list is derived from the grouped one rather than computed a second way, so the two cannot drift. `br` moved out of the tag list into its own boundary applied one level down; the flat output is byte-identical, and a flattening-equivalence assertion across every input shape that module's tests document pins it.
- Restored the `<br>` label reconstruction in `scoreCliffhangerQuality`, reading the final **rendered paragraph** rather than the last N blocks of the flat list. None of the rules PR #293's rounds two, three and four arrived at changed: how far back to look (the labels' own word counts), and what counts as a reconstruction (the last fragment is load-bearing). What changed is the input they had always assumed.
- Re-pinned those three rules as assertions, spelled with `<br>` rather than `</p><p>`, since the mechanism they constrain is back and they were correct.

Self-review:

- **Why this is a slice and not a re-litigation of #293's withdrawal.** That withdrawal was right on the evidence it had: at that level the defect is unfixable, because a `<p>` break and a `<br>` break are the same `\n\n` by the time blocks reach the scan. Three of the four findings against the reconstruction were false positives, and the fourth — prose synthesised into a label — is the same defect the slice existed to remove. Removing a mechanism that cannot be made correct where it sits is the right call. This slice does not disagree with any of that; it goes one level down and returns the information whose absence made it unfixable.
- **The withdrawal's own tests are the strongest evidence for this one.** `testTheScanDoesNotSynthesiseALabelFromProse` was written so that "reintroducing the join has to argue with a test rather than slip past one". Both of its prose cases pass here **unmodified**. The only assertion in it that changed is the one stating the cost of the withdrawal — the `<br>`-split label going unrecognised — which is exactly what this slice buys back. A third case was added beside them: `<p>The door opened.</p><p>She wanted to be</p><p>continued through the trial.</p>` is the *same two-block shape* as the torn label and must not score, so the pair asserts that only the boundary tells them apart.
- **Counterfactuals, all run against the suite and reverted; none committed.** Putting the reconstruction back on the flat block list fails at `two prose paragraphs must not be joined into a label` — the withdrawal's own message, which is the point. Removing the reconstruction entirely fails at `a label broken into three fragments is still the ending`. Adding `br` back to the paragraph-level tags fails at both `a <br> should end a block without starting a new rendered paragraph` and the label case.
- **Cost, asserted rather than implied:** a label spelled across separate `<p>` elements — `<p>To</p><p>be</p><p>continued</p>` — is not reconstructed, where the pre-withdrawal mechanism did score it. A reader sees three paragraphs there, not one label. The narrower input is the fix, so its cost belongs in a test.
- **On the premise "one markup shape there is no evidence the generator emits".** Not relied on either way, but worth recording: `exportSanitizer.ts` lists `br` in `VOID_BREAK_TAGS` and `sanitizeStoryHtmlForExport` collapses runs of boundaries to a single `<br>`, so `<br>` is markup the export path handles as story content rather than a hypothetical. That is evidence about handling, not a measurement of generator output, and the case for this slice does not rest on it: the shared module's boundary provenance is worth having for any scanner that joins blocks, and the reconstruction is a consumer of it.
- Non-claim: nothing here changes a prompt, a generated story, the routing table, or the deployable function count. `buildStoryQualityHeuristicReport` is advisory.
- Non-claim: the two findings PR #293 declined in its fifth round are still declined and untouched. `<p>To be<br>continued<br>in Chapter Two</p>` still scores 50, for the reason given there — the load-bearing rule refuses it, and this slice does not change that rule.
- Non-claim: `splitStoryIntoTextBlocks` has one caller change — none. Its output is unchanged and every existing consumer (`imageService`, `cliffhangerService`, `continuityExtractor`, `storyContentAnalysis`, `exportSanitizer`, `storyLabEngine`, the Angular app) reads it exactly as before.
- Process note, extending PR #293's: its four review rounds found one habit four times — a proxy standing in for the thing. This is the same habit one level lower, and the reason it survived all four rounds is that it was never written as a rule. It was the *shape of the data* every rule downstream had to accept. Worth asking of any pipeline stage: what did the earlier stage throw away, and does anything after it need to know?
- Validation: `npm run test:all` exits `0` across every registered suite. The Vercel API typecheck is clean under the repo's pinned `tsc` 5.9.2 with the exact file list and flags `scripts/recovery/preflight.sh` uses. The Angular app and spec typechecks are clean too, which this slice checked because `shared/` is compiled by both and the API typecheck alone would not have caught a break there.

Review follow-up (Codex, two P2 findings on `cc2777a`). One was a real regression in the flat splitter and is fixed; the other is a limit inherited from that splitter, now asserted rather than described:

- **P2, fixed — marking the two boundary kinds in two passes tore tags in half.** The first version replaced paragraph tags, split on blank lines, and only then looked for `<br>` inside each piece. So the second pass was searching text the first had already cut up, and a `<br>` whose attributes contained a blank line — `<p>A<br\n\nclass="x">B</p>` — was split down the middle by the paragraph pass before the `<br>` pattern could see it. Its halves survived into the blocks as raw markup: `["A<br", "class=\"x\">B"]` where the splitter had always answered `["A", "B"]`. That is a change in `splitStoryIntoTextBlocks` itself, which every scanner in the repository reads, and it falsified this slice's central non-claim. Reproduced against the pre-slice implementation before changing anything. Fixed by matching **every** boundary tag in one pass over the untouched markup, with the same pattern the flat splitter always used, and varying only the *replacement* by kind — a `\0` marker for a line wrap, `\n\n` for a paragraph. `[^>]*` spans newlines, so one pass matches such a tag whole. The marker is stripped from caller input first, so nothing outside the module can forge a boundary with it, and the tag classifier is deliberately **not** global: `.test()` on a `g` pattern advances `lastIndex` and answers differently on the same input from one call to the next.
- **The equivalence claim is now proved rather than sampled.** The first version supported "flat output unchanged" with a handful of hand-picked shapes, and the defect above lived exactly in the gap between them. Differential fuzzing against the pre-slice implementation over **200,000** generated inputs — assembled from block tags, `<br>` spellings with and without attributes, wrapped attributes, malformed `<`, inline tags, entities, and whitespace runs — reports **0** differences. The specific shapes it would have caught are now assertions in `tests/story-text-blocks.test.ts`, stated as the invariant rather than as examples: where a tag's attributes wrap cannot change what comes back, and no block may carry a fragment of a boundary tag.
- **P2, not fixed, inherited and asserted — a raw blank line inside an open `<p>`.** `<p>To be<br>\n\ncontinued</p>` is grouped as two rendered paragraphs, where a browser collapses that blank line as ordinary whitespace, so the reconstruction misses the label. Confirmed. But this is the flat splitter's own long-standing conflation rather than something this slice introduced: blank lines have always ended a block, which is precisely what plain-text callers depend on, and telling "blank line in plain text" from "blank line inside an open `<p>`" needs paragraph-depth tracking that would change `splitStoryIntoTextBlocks`'s output for every scanner reading it. That is a slice of its own, and the tempting shortcuts — treating a blank line adjacent to a `<br>` as a line wrap, or detecting "is this HTML?" at the document level — are proxies of exactly the kind the four earlier rounds on this function each died of. It errs safely: extra boundaries make the final paragraph *smaller*, so a caller joining within one can only miss a repair, never invent one. Pinned as an assertion with that reasoning attached, so it is a stated limit and not a silent one.
- Counterfactuals run and reverted; none committed. Restoring the two-pass order fails at `a blank line inside a tag's attributes is part of the tag`, printing the torn `["A<br", "class=\"x\">B"]`. Making the classifier global fails at `a label broken into three fragments is still the ending` — in the *evals* suite rather than the splitter's own, which is what shows the non-global choice is load-bearing rather than stylistic. Dropping the marker strip fails at `a caller-supplied \0 should not forge a block boundary`.

Second review round on this slice (Codex, on `063e551`) — one P2, fixed, and it landed on the exact sentence I had talked myself into the round before:

- **P2, fixed — the boundary classifier searched the whole matched tag instead of reading its name.** `[^>]*` stops at the first `>`, so a quoted attribute carrying a tag hands the classifier a *truncated* match: `Prelude<div title="Use <br> here">A</div>` presents it with `<div title="Use <br>`. An unanchored search finds the attribute's `<br>` and calls the `<div>` a line wrap, so `Prelude` and the div's content come back in **one** rendered-paragraph group — a consumer joining across a genuine block boundary, which is the single thing the grouping exists to prevent. Reproduced before changing anything. Fixed by classifying from the tag's own leading name, `/^<\s*\/?br\b/i`, which is what the finding asked for and what the previous version only assumed.
- **The docblock had asserted this could not happen.** Its words were "there is nothing else in the string for an unanchored test to find" — written in the previous round while deciding *not* to anchor. That is a property I claimed instead of enforced, and a truncated match is precisely the case that breaks it. Same habit the four rounds on #293's reconstruction each died of, one level further down and dressed as a comment rather than as code.
- **The mirror direction is pinned too**, because a fix that only handles the reported case is the proxy again: a real `<br>` carrying a `<p>` in its attributes must still be a line wrap. Both directions are asserted as the *rule* — a tag's attributes do not change what kind of boundary it is — by comparing the **shape** of the grouping (how many blocks in how many groups) against the same markup with a bare tag, since the text differs where a truncated attribute leaves a remnant but the structure must not.
- **Worth recording about the method, not just the code:** the 200,000-input differential fuzz from the previous round could never have caught this, and neither could the grouped/flat invariant. A misclassification swaps one boundary kind for another and both still end a block, so the *flat* output is identical either way — the defect is invisible to any property stated over the flat list. Fuzzing proved the thing it was pointed at and said nothing about the thing beside it. The corpus was extended with quoted attributes carrying tags (0 flat differences, 0 grouped/flat mismatches over 200,000 inputs) and that is worth keeping, but the assertion that actually holds this rule is the shape comparison above. A test suite is only as good as the properties it names.
- Counterfactual, run and reverted: restoring the unanchored classifier fails at `a tag's attributes must not change what kind of boundary it is`, printing `grouped [2], bare grouped [1,1]` — the two paragraphs merged into one group.

Third round on this slice, and this one was not a review finding — it came out of following the SonarCloud issue count, and it removed the technique rather than the symptom:

- **SonarCloud went from `1 New issue` to `2` at exactly the commit that introduced the `\0` marker** (`063e551`), which turned the previous round's guess into evidence. `sonarcloud.io` is egress-denied here, so the count is still the only observable — but the increment landing on that commit is what a control-character regex (`/\0/g`) would look like from the outside, and acting on it was already promised on the PR thread.
- **Swapping `\0` for a private-use code point looked right and was not.** `U+E000` is what Unicode reserves for a marker whose meaning is settled privately, and it is not a control character, so it answers the Sonar rule. Fuzzing said otherwise. Extending the corpus with the characters it had never contained — `\0`, `U+E000`, a neighbouring private-use point, `U+FFFD`, a zero-width space — reported **17,036 flat differences** against the pre-slice splitter. Several printed as visually identical strings, which is what an invisible-character difference looks like.
- **The cause was the stripping step, and it was there in the `\0` version too.** A sentinel has to be removed from caller input or the input can forge a boundary with it — and removing it *is* a change to what `splitStoryIntoTextBlocks` returns. Measured against the pre-slice implementation: `<p>To be\0continued</p>` returned `["To becontinued"]` under the `\0` marker where the original returns `["To be\0continued"]`. So the round-1 commit had shipped a third small equivalence break, and its 200,000-input fuzz missed it for the same reason the earlier corpus missed everything else — the corpus did not contain the character the code was special-casing.
- **In-band signalling cannot avoid that trade**: either the sentinel is removable from input, which alters the text, or it is not, which lets input forge boundaries. Both `\0` and `U+E000` were tried and the problem is the technique. `splitStoryIntoRenderedParagraphs` now **walks** `BLOCK_BOUNDARY_PATTERN`'s matches and appends the text between them, classifying each match by its leading tag name and ending either a block or a paragraph. Nothing is written into the text to stand for a boundary, so there is no character to strip and none to forge. Re-fuzzed on the corpus that had just produced 17,036 differences: **0 flat differences, 0 grouped/flat mismatches** over 200,000 inputs.
- The two sentinel characters are ordinary text now, asserted as a loop over `\0`, `U+E000`, `U+E001` and `U+FFFD` rather than as one example, so the property is "no character is a boundary" rather than "this character is not".
- Counterfactuals run and reverted: making every boundary end a paragraph fails at `a <br> should end a block without starting a new rendered paragraph`; restoring the unanchored classifier still fails at `a tag's attributes must not change what kind of boundary it is`.
- Worth recording, because it is the third time in this slice the same thing happened: each fuzz run proved exactly the inputs its corpus contained and said nothing about the rest, and each time the gap was a shape the code specifically handled — wrapped attributes, then attribute-carried tags, then the sentinel character itself. A corpus assembled from the code's *happy* vocabulary tests the code against its own assumptions. The rule worth keeping: whenever an implementation singles out a character or shape, that character or shape belongs in the corpus before the run is worth quoting. `scripts/recovery/check-vercel-function-count.sh` reports 8/12, unchanged. `git diff --check` is clean. No test reaches a store, an auth port, or a model provider. A root and `story-generator` install was needed to run any of it; the tracked `node_modules` tree the install dirtied was restored and is not in this diff.
