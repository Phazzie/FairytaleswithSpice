...
- Extended `tests/story-quality-evals.test.ts`, `tests/export-service.test.ts`, and `story-generator/src/app/form-validation.service.spec.ts`. All three files are already registered in `test:all` / `ng test`.
...
- Non-claim: none of this changes a generated story, a prompt, a stored contract, or a route's success shape. It changes one advisory score's inputs, adds two client-side refusals that match refusals the route already makes, and changes the wording of two lines in an exported document.
