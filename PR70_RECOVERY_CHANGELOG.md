...
- **P2, real — the click-to-dismiss backdrop and the panel it wraps had no keyboard equivalent, and opening the panel never moved focus into it.** `aria-modal="true"` alone doesn't move focus, trap it, or support Escape (SonarCloud's Quality Gate independently failed "B Reliability Rating on New Code" on the same two `<div>`s, for the same underlying gap). Rather than inventing a second focus-trap implementation, matched `StoryLabProfilePanelComponent`'s existing one exactly: `role="presentation"` backdrop, a `tabindex="-1"` `<section role="dialog">` focused via an `effect()` when the panel signal turns non-null (this component's own equivalent of that panel's `ngAfterViewInit`, since the preview is a conditional block on an already-running component rather than its own routed one), `onPromptPreviewKeydown`'s Tab-wrap trap and Escape-to-close, and focus restored to whatever was focused before `viewPrompts()` opened it.

Validation: `ng test` full Angular suite green (including new specs for the baseline-sends-no-override, experimental-payload, custom-prompt-overflow, and prompt-preview-panel behaviors); `npm run test:all` (backend/shared) green, including `test:proving-grounds-production-prompt`; `ng build --configuration production` compiles clean.
