...
exports.TropeSubversionService = void 0;
const tropeDatabase_1 = require("../data/tropeDatabase");
class TropeSubversionService {
    constructor() {
        this.MIN_TROPES = 2;
        this.MAX_TROPES = 3;
    }
     ...
     */
    selectTropesForSubversion(options) {
        const { creature, preferredIntensity, avoidCategories = [], tropeCount } = options;
        const creatureTropes = tropeDatabase_1.TROPE_DATABASE[creature];
        if (!creatureTropes) {
     ...
     */
    enhancePromptWithSubversions(basePrompt, tropeSelection, creature) {
     ...
     */
    enhanceContinuationPrompt(basePrompt, previousTropeIds, creature) {
     ...
     */
    serializeTropeSelection(tropeSelection) {
     ...
     */
    deserializeTropeSelection(serializedData, creature) {
        try {
    ...
    createWeightedTropePool(creatureTropes, avoidCategories) {
        const pool = [];
    ...
    selectRandomTropes(pool, count, preferredIntensity) {
        const selected = [];
    ...
    formatSubversionInstructions(tropeSelection, creature) {
    ...
    generateContinuationInstructions(tropeIds, creature) {
        const creatureTropes = tropeDatabase_1.TROPE_DATABASE[creature];
        const allTropes = [...creatureTropes.common, ...creatureTropes.subversive];
     ...
     */
    getAllTropesForCreature(creature) {
        return tropeDatabase_1.TROPE_DATABASE[creature];
    }
    /**
     * Get trope statistics for monitoring subversion effectiveness
     */
    getTropeStatistics(creature) {
        const creatureTropes = tropeDatabase_1.TROPE_DATABASE[creature];
        const allTropes = [...creatureTropes.common, ...creatureTropes.subversive];
...
exports.TropeSubversionService = TropeSubversionService;
