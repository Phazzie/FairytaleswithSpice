...
const tropeSubversionService_1 = require("./tropeSubversionService");
class StoryService {
    constructor() {
        this.grokApiUrl = 'https://api.x.ai/v1/chat/completions';
        this.grokApiKey = process.env.XAI_AI_KEY;
        this.tropeService = new tropeSubversionService_1.TropeSubversionService();
        if (!this.grokApiKey) {
    ...
    async callGrokAI(input, tropeSelection) {
        if (!this.grokApiKey) {
    ...
    buildStoryPrompt(input, tropeSelection) {
...
Format the story with HTML tags for structure (h3 for chapter titles, p for paragraphs).`;
        ...
        if (tropeSelection && tropeSelection.selectedTropes.length > 0) {
            basePrompt = this.tropeService.enhancePromptWithSubversions(basePrompt, tropeSelection, input.creature);
        }
        return basePrompt;
    }
...
Write approximately 400-600 words for this chapter. Format with HTML tags.`;
        ...
        if (input.tropeMetadata && input.creature) {
            const tropeSelection = this.tropeService.deserializeTropeSelection(input.tropeMetadata, input.creature);
            if (tropeSelection) {
                basePrompt = this.tropeService.enhanceContinuationPrompt(basePrompt, tropeSelection.selectedTropeIds, input.creature);
            }
        }
        return basePrompt;
    }
    ...
    generateMockStory(input, tropeSelection) {
        const creatureName = this.getCreatureDisplayName(input.creature);
        const spicyLabel = this.getSpicyLabel(input.spicyLevel);
...
<p><em>This is a mock chapter generated without AI.</em></p>`;
    ...
    getMockCharacterTraits(creature, tropeSelection) {
    ...
    getMockPlotElements(creature, tropeSelection) {
        const subversions = tropeSelection?.subversionInstructions || [];
