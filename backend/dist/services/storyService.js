...
class StoryService {
    ...
    async callGrokAIForContinuation(input) {
        if (!this.grokApiKey) {
...
DIALOGUE FORMAT REQUIREMENT:
Format ALL dialogue using [Speaker]: "dialogue text" format for multi-voice audio generation.
Examples:
- [Lady Arabella]: "You shouldn't be here in the shadows."
- [Count Dracula]: "Yet here you are, drawn to the darkness like a moth to flame."
- [Narrator]: The tension between them was palpable.
...
- Natural dialogue with emotional subtext
- Include both dialogue and narrative sections

Format the story with HTML tags for structure (h3 for chapter titles, p for paragraphs).
Ensure dialogue uses the [Speaker]: format consistently throughout the story.`;
    }
...
Format ALL dialogue using [Speaker]: "dialogue text" format for multi-voice audio generation.
Examples:
- [Lady Arabella]: "What have you done to me?"
- [Count Dracula]: "I have awakened what was always within you."

Write approximately 400-600 words for this chapter. Format with HTML tags.
Ensure dialogue uses the [Speaker]: format consistently throughout the chapter.`;
    }
    validateStoryInput(input) {
        ...
        if (input.themes.length > contracts_1.VALIDATION_RULES.themes.maxCount) {
            return {
                code: 'INVALID_INPUT',
                message: `Too many themes (max ${contracts_1.VALIDATION_RULES.themes.maxCount})`,
                field: 'themes',
                providedValue: input.themes,
                expectedType: 'ThemeType[]'
            };
        }
        if (input.userInput && input.userInput.length > contracts_1.VALIDATION_RULES.userInput.maxLength) {
            return {
                code: 'INVALID_INPUT',
                message: `User input too long (max ${contracts_1.VALIDATION_RULES.userInput.maxLength} characters)`,
                field: 'userInput',
...
<p>[Lady Arabella]: "You shouldn't be here," she whispered, her voice trembling with a mixture of fear and desire.</p>

<p>[Count Dracula]: "Yet here you are, drawn to the darkness like a moth to flame," his voice was like velvet over steel.</p>
...
<p>[Lady Arabella]: "What are you doing to me?" she breathed, her corset straining against her rapidly beating heart.</p>

<p>[Count Dracula]: "I am showing you who you truly are beneath all that propriety," he purred, his fingers tracing the line of her throat.</p>
...
<p>[Lady Arabella]: "I should run," she gasped against his lips.</p>

<p>[Count Dracula]: "But you won't. You can't. The pull is too strong now."</p>

<p>The ${spicyLabel.toLowerCase()} intensity of their encounter left her breathless, her skin flushed and marked by his passionate embrace. She knew she should scream for help, but the words died in her throat.</p>
...
<p>[Lady Arabella]: "What have you done to me?" she whispered to the empty room, her voice carrying a new, sultry quality.</p>
...
<p>Her reflection in the mirror showed a woman transformed. Her skin glowed with an otherworldly luminescence, her eyes held a predatory gleam.</p>

<p>[Lady Arabella]: "The creature has given me a gift," she murmured, running her fingers along her throat, "or is it a curse?"</p>

<p>As night fell once more, she waited impatiently for his return. When he finally appeared at her balcony, silent as a shadow, Arabella knew there was no turning back.</p>

<p>[Count Dracula]: "You have awakened beautifully, my dear," his voice carried approval and desire.</p>

<p>[Lady Arabella]: "I can feel everything so intensely now. Is this what you experience every night?"</p>

<p>[Count Dracula]: "This is only the beginning. Come, let me show you the true extent of your transformation."</p>
...
<p>But in the aftermath, as they lay entwined in sweat-dampened sheets, Arabella began to question the true cost of her transformation.</p>

<p>[Lady Arabella]: "What price will I pay for eternal passion?" she asked, her voice barely a whisper.</p>
...
<p><em>This is a mock chapter generated without AI.</em></p>`;
    ...
    generateChapterTitle(input) {
        return 'The Deeper Shadows';
    }
    countWords(content) {
        return content.replace(/<[^>]*>/g, '').split(/\s+/).filter(word => word.length > 0).length;
    }
    detectCliffhanger(content) {
    ...
    extractSpicyLevelFromContent(content) {
