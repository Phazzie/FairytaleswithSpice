import { DialogueSegment, CharacterVoiceType, EmotionalContext, CreatureType } from '../types/contracts';

export class DialogueParsingService {
  
  /**
   * Parse story content into dialogue segments with character identification
   */
  parseDialogue(htmlContent: string, creatureType?: CreatureType): DialogueSegment[] {
   ...
   */
  mapCharacterToVoice(characterName?: string, creatureType?: CreatureType): CharacterVoiceType {
    if (!characterName) {
      return 'narrator';
    }
    ...
    if (creatureType) {
      ...
      if (this.isMainCharacter(lowerName)) {
        return `${creatureType}-${gender}` as CharacterVoiceType;
      }
    }
    
    // Check for supernatural indicators in name or context
    if (this.isVampireName(lowerName)) {
      return `vampire-${gender}` as CharacterVoiceType;
    }
    
    if (this.isWerewolfName(lowerName)) {
      return `werewolf-${gender}` as CharacterVoiceType;
    }
    
    if (this.isFairyName(lowerName)) {
      return `fairy-${gender}` as CharacterVoiceType;
    }
    
    // Default to human
    return `human-${gender}` as CharacterVoiceType;
  }
  
  /**
   * Extract emotional context from dialogue and surrounding narration
   */
  private extractEmotionalContext(content: string, fullContext: string): EmotionalContext {
   ...
   */
  private containsDialogue(paragraph: string): boolean {
   ...
   */
  private splitDialogueFromNarration(paragraph: string): Array<{content: string, isDialogue: boolean}> {
    const parts: Array<{content: string, isDialogue: boolean}> = [];
   ...
   */
  private extractCharacterName(dialogue: string, allParagraphs: string[], currentIndex: number): string | undefined {
   ...
   */
  private isFeminineName(name: string): boolean {
   ...
   */
  private isVampireName(name: string): boolean {
   ...
   */
  private isWerewolfName(name: string): boolean {
   ...
   */
  private isFairyName(name: string): boolean {
