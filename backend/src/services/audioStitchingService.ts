import { DialogueSegment, AudioFormat } from '../types/contracts';
...
export class AudioStitchingService {
  ...
  async stitchAudioSegments(
    audioSegments: Array<{ segment: DialogueSegment; audioBuffer: Buffer; type: 'dialogue' | 'narrative' }>,
   ...
   */
  estimateCombinedDuration(segments: DialogueSegment[]): number {
    let totalDuration = 0;
   ...
   */
  estimateCombinedFileSize(durationInSeconds: number, format: AudioFormat): number {
   ...
   */
  validateAudioSegments(audioSegments: Array<{ segment: DialogueSegment; audioBuffer: Buffer }>): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
   ...
   */
  createAudioMetadata(segments: DialogueSegment[], totalDuration: number) {
   ...
   */
  private createSegmentTimeline(segments: DialogueSegment[]): Array<{
    segmentId: string;
    speaker: string;
    startTime: number;
    duration: number;
  ...
  private calculatePauseDuration(
    currentSegment: { segment: DialogueSegment; type: 'dialogue' | 'narrative' },
    nextSegment: { segment: DialogueSegment; type: 'dialogue' | 'narrative' }
  ): number {
   ...
   */
  private generateSilence(durationInSeconds: number, format: AudioFormat): Buffer {
   ...
   */
  optimizeAudioQuality(audioBuffer: Buffer, format: AudioFormat): Buffer {
   ...
   */
  async convertAudioFormat(audioBuffer: Buffer, fromFormat: string, toFormat: AudioFormat): Promise<Buffer> {
