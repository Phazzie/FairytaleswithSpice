...
7. **What `redactBearerTokens` treats as a credential.** `bearer` is both the RFC 6750 scheme keyword and an ordinary English noun, and in a dark-fantasy story generator the noun reaches the logger constantly — a bearer of a seal, an oath, or bad news, inside a prompt, a story excerpt or an error message. The run after the keyword is redacted when **either** the character immediately before the keyword (skipping whitespace) is `:` or `=`, putting it in a header value position, **or** the run is credential-shaped: at least 8 characters, or carrying a digit or one of `b64token`'s `._~+/=-`, which an English word cannot. Neither test is sufficient alone and both directions are asserted — a header position catches a credential short and alphabetic enough to pass for a word, and the shape test catches one written bare into a sentence.
   ...
   **The introducer set is deliberately two characters.** What this app logs is story content, so `>` (HTML), `"` and `'` (dialogue), `(`, `[` and `|` all sit immediately before an ordinary capitalized noun far more often than before a credential; admitting any of them reintroduces the defect on the most common shape story text takes. Nothing is lost, because a credential appearing after a quote or a tag is still caught by the shape arm. A configured `API_KEYS` entry is always redacted on shape alone, since Note 6's floor (16) is above the 8 here — `tests/log-redaction.test.ts` pins that relationship so neither can be moved into a gap.

   **Known gap, unchanged by this contract:** the scheme must be followed by whitespace to be read at all, so the query-string form `?auth=1&bearer=xyz` is not redacted. A full URL is redacted whole by `redactUrls`; a bare query fragment is not. Closing that is a separate change.
