...
- `api/_lib/story-lab/jobs/postgresStoryLabJobStore.ts` **does exist** and is more than scaffolding: it has real `INSERT`/`UPDATE`/event-append SQL, owner-scoped reads, and a terminal-status list driven off `STORY_LAB_TERMINAL_JOB_STATUSES`. It had two confirmed gaps; one is now closed:
  - `createJob()` (lines 162-200) does two separate, unwrapped queries — insert the job row, then `insertEvent()` — with no transaction around them. A crash between the two leaves a job with no snapshot event. **PR #357** (`***WORST TO BEST*** Story Lab durable job store`) rewrites `createJob()` and `updateJob()` to each write their job-row mutation and event-append as one atomic `WITH ... AS (...)` CTE statement instead — the exact single-round-trip design Pass 1 below already required for any outbox-style write against this driver — but that PR is **in review as of this note, not yet merged to `main`**. Treat this gap as open until it merges; once it does, this specific "crash between the two leaves a job with no snapshot event" gap is closed for the store's existing create/update path. Even merged, it is **not** a transactional outbox and does not by itself satisfy Phase B: no claim/dispatch/idempotency semantics were added, the idempotency gap immediately below is still open, and it does not resolve the separate, pre-existing ambiguity of a Neon HTTP `.query()` call whose response is lost after the server has already committed (ambiguous-outcome reconciliation remains unaddressed, related to but broader than the idempotency-key work below).
