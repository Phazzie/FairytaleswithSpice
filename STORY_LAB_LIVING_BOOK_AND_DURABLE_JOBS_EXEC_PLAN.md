...
- `api/_lib/story-lab/jobs/postgresStoryLabJobStore.ts` **does exist** and is more than scaffolding: it has real `INSERT`/`UPDATE`/event-append SQL, owner-scoped reads, and a terminal-status list driven off `STORY_LAB_TERMINAL_JOB_STATUSES`. It had two confirmed gaps; one is now closed:
  - ~~`createJob()` (lines 162-200) does two separate, unwrapped queries — insert the job row, then `insertEvent()` — with no transaction around them. A crash between the two leaves a job with no snapshot event.~~ **Fixed in PR #357** (`***WORST TO BEST*** Story Lab durable job store`, merged after this plan's last revision): `createJob()` and `updateJob()` each now write their job-row mutation and event-append as one atomic `WITH ... AS (...)` CTE statement — the exact single-round-trip design Pass 1 below already required for any outbox-style write against this driver. This closes the "crash between the two leaves a job with no snapshot event" gap for the store's existing create/update path. It is **not** a transactional outbox and does not by itself satisfy Phase B: no claim/dispatch/idempotency semantics were added, and the idempotency gap immediately below is still open.
