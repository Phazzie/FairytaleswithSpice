...
- Later follow-up (2026-08-27): this plan's invariant has an outbound half it never stated.
  - The work above closed the *inbound* streaming gap — what a caller may put in the query string, and which origins may read the response. It said nothing about what a stream *writes back* on a failure, and the genesis stream's throw branch was quoting the raw exception: `/api/story-lab/stream/genesis` sent `error.message` verbatim in its SSE `error` frame, so whatever threw inside the engine — a driver quoting a failing statement, a client naming an internal host or upstream URL — reached the browser and, through `StoryService`, the reader. `redactSensitiveLogData` covers this class of text on the way into the log; nothing covered it on the way to the client. The route also logged nothing for that branch, so the operator never saw the failure the reader did.
  - That specific route no longer exists: it was retired outright (fake per-chapter progress over a blocking call, reachable only from an unmounted demo component). The gap it exposed in this plan is not retired with it, and the surviving job event stream is the route the invariant now applies to.
  - Recorded here because "no private text leaves through the stream" is this plan's invariant and only its inbound half was ever written down. Read any future streaming route against both directions: what the caller may send in, and what a failure sends back. The POST routes' pattern — a fixed client-facing message plus `logError` with the real exception — is the shape to copy.
