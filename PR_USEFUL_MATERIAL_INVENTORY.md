...
| #63 | Database investigation | Large documentation set comparing Postgres, Redis, MongoDB, MySQL, SQLite, schemas, ROI, decision tree, and implementation roadmap. DigitalOcean-oriented but generally useful for storage planning. | `DATABASE_*.md`, `README.md` | **Reference/recreate for Vercel storage**. Do not merge as-is if Vercel is canonical. |
| #56 | DigitalOcean backend services research | Researches DO Spaces, Redis, Postgres, monitoring, cost/ROI; includes a `cacheService.ts` implementation for story/audio caching and rate limiting. | `DIGITAL_OCEAN_*.md`, `DO_*.md`, `api/lib/services/cacheService.ts` | **Reference/recreate**. Cache concept is useful; provider assumptions are not. |
| #55 | Voice evolution and emotion support | Enhances story/audio prompts with 90+ emotions, voice evolution tags, narrator atmosphere cues, and intelligent voice parameter assignment. Mostly audio, but prompt-format ideas may help future story metadata. | `api/lib/services/storyService.ts`, `api/lib/services/audioService.ts`, audio docs, lockfiles | **Defer/recreate later**. Useful audio/prompt concepts; avoid now. |
| #54 | DigitalOcean deployment infrastructure | Adds Dockerfiles, `server.js`, DO `app.yaml`, deploy scripts, env examples, and deployment docs. Explicitly moves away from Vercel. | Docker files, `server.js`, `app.yaml`, `deploy.sh`, docs, package files | **Do not merge for Vercel**. Reference only for traditional hosting ideas. |
| #53 | Documentation/serverless cleanup | Archives docs, updates serverless references, rewrites changelog/agent docs, and removes committed dependencies in concept. However the PR itself touches hundreds of `node_modules` files. | `AGENTS.md`, `CHANGELOG.md`, archived docs, many `node_modules/*` | **Reference only**. Useful intent; recreate cleanly. |
...
| #47 | Audio pipeline investigation | Research/test artifacts for speaker tag parsing, voice assignment, emotion mapping, sound effects, Web Audio player, and practical enhanced audio service prototypes. | `audio-pipeline-investigation.md`, `audio-pipeline-test.js`, `enhanced-audio-service*.ts`, `sound-effects-investigation.ts` | **Reference for later audio rebuild**. Not current merge material. |
| #45 | Emotion mapping and character consistency | Implements 81-emotion audio parameter mapping, character memory, fuzzy emotion matching, enhanced speaker tags, docs, demo, and tests. | `api/lib/services/audioService.ts`, `backend/src/services/audioService.ts`, emotion docs/tests/demo | **Defer/recreate later**. Good audio subsystem material, old structure. |
