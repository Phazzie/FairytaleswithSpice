...
export const RATE_LIMITS = {
  ...
  /**
   * The Story Lab job event stream.
   *
   * `/api/story-lab/jobs/:jobId/events` replays the events a job has recorded
   * so far and then *ends the response* — every time, for a job that is still
   * running — so the browser fires `error`, reopens the connection, and asks
   * again. That is the documented design on both sides: `shared/eventStreamRetry.ts`
   * exists to tell the reader to keep the subscription alive through it, and
   * `StoryService.streamStoryLabJobEvents` treats a reconnect as normal. So one
   * reader watching one job is not one request here; it is a request every
   * time the browser retries, roughly every three seconds.
   *
   * A five-per-fifteen-minutes tier (sized for a single held-open connection)
   * was spent about fifteen seconds into the first generation, and every
   * reconnect after it was answered 429 for the rest of the window: the job
   * kept running on the server while the reader was told "Story generation
   * updates stopped", and could not get them back for fifteen minutes. The
   * route spends nothing — it reads the job store and replays recorded
   * snapshots — so the cap belongs on the polling, not on the generation.
   * Fifteen minutes of uninterrupted three-second reconnects is three hundred
   * requests, which is what this allows.
   */
