...
- Deleting a tag is not the same as removing it. `content.replace(/<[^>]*>/g, '')` closes the gap the tag held open, so `door.</p><p>Blood` becomes the single token `door.Blood`: word counts lose one word per boundary, sentence splits that need whitespace after `[.!?]` stop finding any, and paragraph-shaped measures see one paragraph. Three separate slices have now hit this in three different files (cliffhanger scan, quality heuristics, story service). `api/_lib/utils/storyTextBlocks.ts` is the shared answer — `splitStoryIntoTextBlocks` for anything that reasons about paragraphs, `stripStoryHtmlToText` for anything that wants the reader's plain text. Reach for it instead of writing a fourth local strip.
