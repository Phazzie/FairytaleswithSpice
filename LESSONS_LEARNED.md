...
- Deleting a tag is not the same as removing it. `content.replace(/<[^>]*>/g, '')` closes the gap the tag held open, so `door.</p><p>Blood` becomes the single token `door.Blood`: word counts lose one word per boundary, sentence splits that need whitespace after `[.!?]` stop finding any, and paragraph-shaped measures see one paragraph. Four separate slices have now hit this in four different files (cliffhanger scan, quality heuristics, story service, image service). `api/_lib/utils/storyTextBlocks.ts` is the shared answer — `splitStoryIntoTextBlocks` for anything that reasons about paragraphs, `stripStoryHtmlToText` for anything that wants the reader's plain text. Reach for it instead of writing another local strip, and when a slice fixes one site, sweep for the rest in the same pass: `grep -rn '<\[^>\]\*>' --include='*.ts' api shared story-generator/src` finds every remaining in-place strip in one command, and as of this entry it returns nothing. Three of the four sites were found one slice at a time because nobody ran it.
