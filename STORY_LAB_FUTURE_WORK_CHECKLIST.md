...
- [ ] **Coverage proof:** root/API coverage tooling and repo-wide coverage evidence do not exist. Angular coverage invocation exists, but this Mac currently cannot run ChromeHeadless reliably.
...
- [x] **1.5 Make Angular coverage invocation explicit**
  ...
  - Status: completed in the first implementation wave; `test:coverage` exists and uses the no-sandbox Karma launcher. Local ChromeHeadless still times out on this machine, so CI or a supported browser environment is required for coverage evidence.
