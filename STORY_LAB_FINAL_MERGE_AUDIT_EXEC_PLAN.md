...
- As of 2026-09-03, PR #328 went through an extended run of Codex review rounds — see `PR70_RECOVERY_CHANGELOG.md`'s dated, numbered "Clerk auth hardening round N" entries for the current count, rather than restating a number here that the next round would make stale (Codex flagged this exact staleness twice in a row). Every round but the first landed its comment(s) on a still-open PR rather than after merge, and each was replied to with the fixing commit and resolved before the next round started, consistent with this file's "every new completion PR has its own late-thread audit" requirement.
