...
> **Routing update (2026-07-31):** Keep this document as the source for
> completed audit evidence and the stricter claim-based Definition of Done.
> Execute remaining work through
> `STORY_LAB_FOUNDATION_AND_LIVING_BOOK_EXEC_PLAN.md`, including merged P1-06
> activation, and then `STORY_LAB_PRODUCTION_COMPLETION_EXEC_PLAN.md` from
> current `origin/main`; Plan 2 owns current final closure, production evidence,
> rollback, and reporting.

> **Status: HISTORICAL / READ-ONLY.** Every unchecked `Progress` item,
> Definition-of-Done row, gate, and work-plan instruction below is historical
> evidence, not current execution authority. Do not use it to start or close
> work. The two current ExecPlans govern unfinished execution and Plan 2 owns
> required durable Queue work even where older text called that work optional.
...
- broader historical review-thread cleanup beyond the required issue #153 baseline and its recorded dispositions;
